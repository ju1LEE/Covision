/*
 * sharedBox.js (based on mydrive.jsp)
 * use: SharedBoxList.jsp (공유받은 폴더) 
 */
...
var boxinit = function(){
	...
	// title text setting.
	$("#driveTitle").text( Common.getDic("lbl_sharedFolder") );		// 공유받은 폴더
	
	// 공유받은 폴더에서는 '복사', '이동', '삭제', '이름바꾸기' 버튼 안보이게 함. dafault is show.
	$("#btnCopy, #btnMove, #btnDelete, #btnRename").hide();

	//contents init draw
	webhardView.init().setHideImportant();		// 중요표시(북마크) 숨김(setHideImportant는 list type 만 적용).
	
	// 데이터 조회.
	this.getData = function(pFolderID, pSearchText, pStartDate, pEndDate) {
		var folderID = CFN_GetQueryString("folderID") === "undefined" ? pFolderID : CFN_GetQueryString("folderID");
	...
	$("#btnSearch").off("click").on("click", function(){
		$("#nameSearchText").val("");	// 상세 조건 진행 시, 상단 우측 검색창 초기화.
