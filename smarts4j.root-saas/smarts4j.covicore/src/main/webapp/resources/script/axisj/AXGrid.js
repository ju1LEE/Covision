...
var AXGrid = Class.create(AXJ, {
    initialize: function (AXJ_super) {
        ...
        this.config.resizeable = true; // 2016-06-12 resize 안되는 옵션 추가.
        ...
        this.config.notFixedWidth = -1;			// Header Resize 고정(default) 해제할 column seq
        this.colHeadResizerMoveFg = false;		// 헤더 드래그 작업 여부
        this.config.overflowCell = [];			// overflow를 visible로 할 cell array     
    },
    /* 공통 영역 */
    defineConfig: function (rewrite, _test) {
        ...
        var notFixedCG = "";			// 고정 해제할 컬럼 열
        var isFirstCallGrid = false;	// 그리드 처음 호출 여부
        var notFixedWidth = this.config.notFixedWidth;
        $.each(cfg.colGroup, function(i, v) {
        	if (v.display != false) {
        		if (i == notFixedWidth) notFixedCG = v;
        		if (typeof(v._owidth) == "undefined") {
        			isFirstCallGrid = true;
        		}
        	}
        });
            ...
            if (CG.display) {
                if (!rewrite) {
                    ...
                    if (CG.widthAstric) {
                        ...
                        CG._owidth = CG.width;
                        astricCount++;
                    }else if(!cfg.fitToWidth){ // 자동 너비 맞춤이 아닌 경우, 수동으로 컬럼width 변경 후 resize시 전체 width 및 스크롤 오류
						CG._owidth = CG.width;
					}
                }
...
/*                var notFixedCG = "";			// 고정 해제할 컬럼 열
                var isFirstCallGrid = false;	// 그리드 처음 호출 여부
                var notFixedWidth = this.config.notFixedWidth;
                $.each(__arr, function(i, v) {
                	if (v.display != false) {
                		if (v._owidth == v.width) isFirstCallGrid = true;
                		if (i == notFixedWidth) notFixedCG = v;
                	}
                });*/
                ...
                for (var CG, cidx = 0, __arr = cfg.colGroup; (cidx < __arr.length && (CG = __arr[cidx])); cidx++) {
                    if (CG.display) {
                    	if (isFirstCallGrid || this.colHeadResizerMoveFg || notFixedCG == "") {
                ...
                if (notFixedCG != "" && notFixedCG.display == true && !this.colHeadResizerMoveFg && !isFirstCallGrid) {
                	$.each(__arr, function(i, v){
                    	if (i == notFixedWidth) {
                    		if (_bodyWidth < 25) {
                    			v.width = 25;
                ...
                this.colHeadResizerMoveFg = false;
                this.colWidth = colWidth;
/*            } else {
                colWidth = 0;
        ...
        if (cfg.colHead.rows) {
            ...
            for (var CG, cidx = 0, __arr = cfg.colGroup; (cidx < __arr.length && (CG = __arr[cidx])); cidx++) {
                ...
                //[2016-11-09] 이연재 : 헤더 변경시 발생하는 오류 수정
                //기존: adder.sort = cfg._colHead_rows[0][cidx].sort; // redrawGrid 호출된 경우 예외처리
                if (cfg._colHead_rows) adder.sort = cfg.colGroup[cidx].sort; // redrawGrid 호출된 경우 예외처리
 ...
 * }
    ...
    init: function () {
        ...
        /*colHead setting */
        if(cfg.displayColHead != false){  //2017.09.21 yjlee : 헤더 숨김 설정 추가
        	this.setColHead();
        }
        ...
        this.target.bind("click.axgrid", (function (e) {
            ...
            //this.gridFocus.focus();		//[khbaek] 특정 브라우저에서 팝업이 뒤로 숨는 현상으로 인해 주석처리 
        }).bind(this));
        ...
        // 2017-09-28 ywcho - resize 수정
        //axdom(this).bind("resize", this.windowResizeApply.bind(this));
        $(window).off("resize");
    ...
    windowResizeApply: function () {
        ...
        if (this.config.resizeable === false) return; // 2016-06-12 resize 안되는 옵션 추가.
    ...
    redrawGrid: function (changeGridView) {
        ...
        if (typeof changeGridView == "undefined" || changeGridView) {
            ...
            if (cfg.viewMode == "grid") {
                ...
                if(typeof this.ajaxInfo != "undefined" && typeof this.ajaxInfo.onLoad != "undefined" && typeof this.ajaxInfo.onLoad == "function"){		// 20160525 안기현 수정 리사이즈시 그리드 body onload함수실행
                	this.ajaxInfo.onLoad();
                }
                
                if(typeof coviInput == "object") {			// 20160525 안기현 수정 리사이즈시 switch setting
                	coviInput.setSwitch();
                }
            }
        }
            ...
            this.contentScrollResize();
            if(typeof this.ajaxInfo.onLoad == "function"){		// 20160525 안기현 수정 리사이즈시 그리드 body onload함수실행
            	this.ajaxInfo.onLoad();
            }
    ...
    getColHeadTd: function (arg) {
        ...
        po.push("class=\"colHeadTd" + arg.bottomClass + sortClass + "\" style=\"height:"+(arg.tdHeight < 40 ? 40 : arg.tdHeight)+"px;\">");//(2016-03-28 안기현 수정, td 40px로)
        //po.push("<div class=\"tdRelBlock\" style=\"height:"+arg.tdHeight+"px;\">"); 
		po.push("<div class=\"tdRelBlock\" style=\"height:"+(arg.tdHeight < 40 ? 40 : arg.tdHeight)+"px;\">");//(2016-03-28 안기현 수정, td 40px로)
    ...
    setColHead: function () {
        ...
        if (cfg.viewMode == "grid")
        {
            ...
            this.colHead.find(".colHeadResizer").each(function () {
                ...
                cellMarginTop = "10";//(2016-03-28 안기현 수정, 헤더 높이 40으로 변경에 따라 )
            ...
            if (this.hasFixed) { //fixedColHead에 대한 바인딩 및 처리
                ...
                this.fixedColHead.find(".colHeadResizer").each(function () {
                    ...
                    if (valign == "bottom") cellMarginTop = (tdHeight - txtHeight) + 5;
                    if (valign == "middle") cellMarginTop = (tdHeight - txtHeight) / 2 + 5;
                    cellMarginTop = "10"; //(2016-03-28 안기현 수정)
                    axdom("#" + txtID).css({"padding-top": cellMarginTop + "px"});
                    axdom("#" + toolID).css({"top": (cellMarginTop - 5) + "px"});
                });
...
/*        if (!cfg.colGroup[this.colResizeTarget.colSeq].widthAstric) {
        ...
        this.colHeadResizerMoveFg = true;
    },
    /**
    ...
    colGroupListClick: function (event) {
       ...
       /* if (event.target.id == "") return;
        ...
        // [2020-05-12] COVISION 수정
        var myTarget; 
        if (event.target.id == "") {
        	if(axdom("#" + cfg.targetID + "_AX_colHeadMenu").length <= 0) return;
    ...
    setBody: function (list, rewrite) {
        ...
        if (this.list == undefined || this.list == "undefined" || this.list.length > 0) {
            var _this = this;
    ...
    setList: function (obj, sortDisable, rewrite, exts) {
        ...
        if (obj.ajaxUrl) {
            ...
            // 기본 소팅정보
            if (cfg.remoteSort 
            		&& (obj.ajaxPars == undefined || obj.ajaxPars["sortBy"] == undefined) ) {
            ...
            if (axdom.isArray(obj)) {
                ...
                if (!cfg.page.paging) {
                    this.pagingUnit.hide();
                    //2016-11-10 안기현 수정(redraw시 페이징 작동때문에 주석처리. 기본 axisj 페이징을 사용안하게 돼서)
                    //this.pageActive = false;
                }
            }
        }
        
        if(typeof coviInput == "object") {			// 20160525 안기현 수정  switch setting
        	coviInput.setSwitch();
        }
    },
    /**
    ...
    reloadList: function () {
        ...
        if (this.ajaxInfo) {
        	...
        	// 기본 소팅정보
            if (cfg.remoteSort && (obj.ajaxPars == undefined || obj.ajaxPars["sortBy"] == undefined)) {
                var sortParam = this.getSortParam("one");
                if (sortParam) {
                    appendPars.push(sortParam);
                }
            ...
            new AXReq(url, {
                ...
                onsucc: function (res) {
                    ...
                    if ((res.result && res.result == AXConfig.AXReq.okCode) || (res.result == undefined && !res.error)) {
                        ...
                        if (obj.onLoad) obj.onLoad.call(res);		// 20160511 안기현 추가. 정렬 후 reload시에도 onload함수 있으면 실행되도록
            ...
            for (; CHidx < cfg.body.rows[r].length; CHidx++) {
                ...
                if (CH.display && CH.colspan > 0) {
                    ...
                    if (printOk) {
                        ...
                        else if (typeof this.config.overflowCell == "object" && this.config.overflowCell.length > 0 && this.config.overflowCell.includes(CHidx) ) bodyNodeClass = " bodyTdFile";
    ...
    printList: function (args) {
        ...
        if (cfg.viewMode == "grid") {
            ...
            if (this.list.length == 0) { // empty list
                ...
                po.push("<td colspan=\"" + (this.showColLen) + "\">");
                po.push("<div class=\"tdRelBlock\">");
                po.push("<div class=\"bodyNode bodyTdText\" align=\"center\" style=\"width:"+this.gridBody.width()+"px;\">");
                po.push(cfg.emptyListMSG);
                po.push("</div>");
                po.push("</div>");
                po.push("</td>");
            ...
            if (cfg.__height != "auto" && this.list.length > 0) {
            	...
            	if(this.cachedDom.tbody.find("#" + cfg.targetID + "_AX_null_AX_0").outerHeight() != null){
            		this.virtualScroll = {
                            startIndex: 0,
    ...
    editCell: function (r, c, ii, times, event) {
        ...
        get_editor = function (cond, val) {
            ...
            if (cond.type === "select" || cond.type === "AXSelect") {
                ...
                else if (cond.type == "dictionary"){
		            po.push("<input type=\"hidden\" id=\"" + cfg.targetID + "\" name=\"inline_editor_item\" value=\"" + val + "\" />");
		            po.push("<input type=\"text\" id=\"" + cfg.targetID + "Visible\" value=\"" + CFN_GetDicInfo(val) + "\" class=\"AXInput\" style=\"width:60%\" disabled=\"disabled\" />");
		            po.push("<input type=\"button\" value=\"" + CFN_GetDicInfo("다국어;MultiLang;多言語;多语言;") + "\" class=\"AXButton\" \">")
				}
                else {
    ...
    contentScrollResize: function (resetLeft) {
		if(resetLeft === undefined) resetLeft = true;
        ...
        if(cfg.xscroll == false) {
	        scrollWidth = "100%";//2019.04 너비 강제조정
	        bodyWidth = "100%";//2019.04 너비 강제조정
        }
        ...
        if (resetLeft && (
			(_this.scrollContent.width() + _this.scrollContent.position().left) < _this.body.width() // 스크롤 위치가 본문 영역을 벗어난경우(ex. fitToWidth 가 아닐때 맨우측 컬럼 사이즈 축소)
				||
			(_this.scrollContent.position().left == 0 && (_this.colHead.position().left != 0 || _this.scrollXHandle.position().left != 0)) // // window가 변경되서 content만 다시그려진경우 (ex. fitToWidth 가 아닐때 스크롤 이동상태에서 window 사이즈 변경)
		)) {
        ...
        if ("수평스크롤 생성 조건 체크") {
            if (scrollWidth > (bodyWidth + 40) && cfg.xscroll) { // 2017-09-28 ywcho, 보정치 증가
                show_scroll_x = true;
            }
    ...
    bigDataSyncApply: function (reload) {
        ...
        if (VS.scrollTop != (scrollContentScrollTop = this.scrollContent.position().top) || reload) {
            ...
            if (VS.startIndex != newStartIndex || reload) {
                ...
                //20160511 안기현 추가   height:auto 등 리로드이벤트시 body onload 함수 호출하도록
                if(this.ajaxInfo != undefined && typeof(this.ajaxInfo.onLoad) == "function"){
                	this.ajaxInfo.onLoad();
                }
            ...
            for (var tri = 0; tri < rows.length; tri++) {
                for (var tdi = 0; tdi < rows[tri].length; tdi++) {
                    if (rows[tri][tdi].rowspan == 0) {
                        ...
                        //2021.08.12 nkpark, merge cell 케이스의 border solid 1px 스타일 안나와서 커스텀 처리.
                        if(!isNaN(Number(rows[tri][tdi].rowspan)) && Number(rows[tri][tdi].rowspan)>1) {
                            rows[tri][tdi].tdom.css({
    ...
    getEditorFormValue: function (form, dataSet, value, key, CH, idAttr) {
        ...
        else if (form.type == "dictionary"){
            var formValue = getFormValue(form.value, dataSet[key]);
            result.push("<input type=\"hidden\" id=\"" + formID + "\" name=\"" + key + "\" value=\"" + formValue + "\" />");
            result.push("<input type=\"hidden\" id=\"" + formID + "Visible\" value=\"" + CFN_GetDicInfo(formValue) + "\" class=\"AXInput" + formClass + "\" style=\"" + formWidth + formHeight + formStyle + "\" />");
            result.push("<input type=\"button\" value=\"" + CFN_GetDicInfo("다국어;MultiLang;多言語;多语言;") + "\" class=\"AXButton\" \">")
		}
        return result.join('');
    },
    /**
...
//            if (pgCount > 1000) {								// 20160511 안기현 수정. 1000페이지 넘어가는 경우 50페이지만 표시하도록 되있음. 테스트하면서 이유를 알아봐야 할 듯 함.
//                var oi = 0;
//                var pageStart = 1;
...
//                for (var p = 1; p < pgCount + 1; p++) {
//                    //mySelect.options[oi] = new Option(p, p.money());
//                    mySelect.options[oi] = new Option(p, p);    // 20160511 안기현 수정 p.money()형태로 value바인드시 컴마때문에 1000이 넘어가는 경우 페이징안됨
//                    if (pageNo == p) mySelect.options[oi].selected = true;
//                    oi++;
//                }
//            }
    ...
    setStatus: function (listLength) {
        ...
        // 2016-11-10 안기현 수정 (listCount 항상 총 갯수 가져오도록 주석처리)
    ...
    function boxFormatter(formatter, item, itemIndex, value, key, CH, CHidx) {
        ...
        if (CH.disabled) {
            if (CH.disabled.call(sendObj)) {
                ...
                if(!this.list[itemIndex].___disabled) this.list[itemIndex].___disabled = {};			//2016 0303 안기현 수정. 변수 선언이 disabled가 아니라 checked로 잘못 선언되어있었음.
...
AXGrid.prototype.inlineEditor = (function () {
    ...
    return {
        ...
        "dictionary": {
            init: function (inline_editor, AXBindConfig, CG, r, c, ii) {
                inline_editor.find("input[type='button']").unbind("click").bind("click", function () {
                   	if(typeof dictionaryLayerPopup === "function"){
						dictionaryLayerPopup(inline_editor.attr("id"));	
					}
                });
            },
            getValue: function (value) {
