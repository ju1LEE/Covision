...
var AXSelectConverter = Class.create(AXJ, {
	...
	appendAnchor: function (objID, objSeq) {
		...
		var css = { left: l, top: (t + 3), width: w, height: h }, objDom = axdom("#" + cfg.targetID + "_AX_" + objID);			// 박경연 [2016.04.12] Select Box의 top 조정
		objDom.css(css);
	...
	alignAnchor: function (objID, objSeq){
		...
		var css = { left: l, top: (t + 3), width: w, height: h };	// 박경연 [2016.04.12] Select Box의 top 조정
		obj.objDom.css(css);
		...
		obj.objDom.find("#" + cfg.targetID + "_AX_" + objID + "_AX_SelectTextBox").css({height:(h-(borderT+borderB)-2)+"px"});			// 박경연 [2016.04.12] Select Box의 height 조정
		...
		obj.objDom.find("#" + cfg.targetID + "_AX_" + objID + "_AX_SelectBoxArrow").css({height:h-2});		// 박경연 [2016.04.12] Select Box의 height 조정
	},
	bindSelect: function (objID, objSeq) {
		...
		po.push("<a " + obj.config.href + " class=\"selectedTextBox\" id=\"" + cfg.targetID + "_AX_" + objID + "_AX_SelectTextBox\" style=\"height:" + (h - (borderT+borderB)-2) + "px;\"");			// 박경연 [2016.04.12] Select Box의 height 조정
		...
		po.push("	<div class=\"selectBoxArrow\" id=\"" + cfg.targetID + "_AX_" + objID + "_AX_SelectBoxArrow\" style=\"height:" + (h-2) + "px;\"></div>");			// 박경연 [2016.04.12] Select Box의 height 조정
		po.push("</a>");
		po.push("</div>");
		...
		if (obj.config.ajaxUrl) {
			...
			if (obj.config.ajaxAsync != undefined) _async = obj.config.ajaxAsync;	//20160419 안기현 수정
		...
		if (obj.config.options) {
			...
			for (var opts, oidx = 0; (oidx < obj.config.options.length && (opts = obj.config.options[oidx])); oidx++) {
				...
				if (typeof opts[obj.config.reserveKeys.optionData] !== "undefined") po.push(" data-option=\"" + opts[obj.config.reserveKeys.optionData] + "\"");
		...
		if(!obj.config.positionFixed){
			if (bodyHeight < css.top.number() + expBoxHeight) {
				css = {
					...
					top: offset.top - expBoxHeight + 3,						// 박경연 [2016.04.12] Select Box의 top 조정
					left: offset.left
				}
			}
		}
