...
var AXTree = Class.create(AXJ, {
	...
	init: function () {
		...
		// tree 뼈대 그리기 -----------------------------------------------------------------------------------------------------
		var ol = [];
		ol.push("<a id=\"" + cfg.targetID + "_AX_tree_focus\"></a>");
		...
		if(event.target.id == "") {
            ...
            var myTarget = this.getEventTarget({
                ...
                find: function (evt, evtIDs) {
                    return (axdom(evt).hasClass("AXTreeColGroupListBoxItem") || axdom(evt).hasClass("colHeadTool")) ? true : false;
                }
            });
        }
			...
			axf.each(cfg.body.rows[r], function (CHidx, CH) {
				if (CH.display && CH.colspan > 0) {
					...
					if (isfix == "n" || (isfix != undefined && colCount < (cfg.fixedColSeq + 1))) {
						...
						if (item[cfg.reserveKeys.openKey]){			// 박경연 [2016.03.30] Tree 확장 이미지 사용
							expandNodeClass = " expand";
							iconClass = " expand";
						}
						...
						if (CH.indent) {
							...
							if(cfg.showConnectionLine){
								...
								if (item.__subTreeLength == 0 && item.haveChild != "Y"){ //[yjlee] 조직도 속도 개선
						...
						if (CH.getIconClass) {
							...
							iconClass = getIconClassValue(CH.getIconClass, item, itemIndex, item[CH.key], CH.key, CH) + iconClass;					// 박경연 [2016.03.30] Tree 확장 이미지 사용
	...
	printList: function () {
		...
		axf.each(this.list, function (lidx, L) {
			if(L.__checked){
				$("input:checkbox[id="+cfg.targetID +"_treeCheckbox_"+L.no+"]").attr("checked",true);	
	...
	expandToggleList: function (itemIndex, item, expandStat) {
		...
		if (item[cfg.reserveKeys.openKey] && (typeof expandStat == "undefined" || expandStat == false)) {
			...
			for (var r = 0; r < cfg.body.rows.length; r++) {
				...
				axdom("#" + cfg.targetID + "_AX_tr_" + r + "_AX_n_AX_" + itemIndex).find(".bodyNodeIcon").removeClass("expand");			// 박경연 [2016.03.30] Tree 확장 이미지 사용
			}
		...
		{ // 자식개체 열기
			...
			for (var r = 0; r < cfg.body.rows.length; r++) {
				...
				axdom("#" + cfg.targetID + "_AX_tr_" + r + "_AX_n_AX_" + itemIndex).find(".bodyNodeIcon").addClass("expand");			// 박경연 [2016.03.30] Tree 확장 이미지 사용
			}
			
			// 부서 확장 시, 해당 라인 선택 배경색 적용되도록 수정.
			this.selectedRow.clear();
			this.body.find(".gridBodyTr_" + itemIndex).addClass("selected");
			this.selectedRow.push(itemIndex);
	...
	gridCheckClick: function (event, tgId) {
		...
		//[yjlee] 체크 박스 개체 정보 조회 수정
		var prevItemID = $("#"+targetID).prev("a").attr("id");
		var itemIndex = prevItemID.split(/_AX_/g).last();
		var ids = prevItemID.split(/_AX_/g);
		var item = this.list[itemIndex];
		var colSeq = ids[ids.length - 2];
		var type = _body.find("input[id^="+cfg.targetID+"]").get(0).type == "radio" ? "Radio_" : "Checkbox_";
		...
		if( checkboxRelationFixed ){
			...
			var recursive = function( target ){
				...
				_list.filter(function( _cur ){ 
					...
					if(type == "Checkbox_" && item.chk == "N") return;
					if( item.open ) {
			...
			recursive( this.list[itemIndex] ).forEach(function(item){
				//this.updateTree(itemIndex, 	this.list[itemIndex], {__checked: checked});
				_body.find("#" + cfg.targetID + "_tree"+type + item.no).get(0).checked = checked;
			});
		}else{
			this.updateTree(itemIndex, 	this.list[itemIndex], {__checked: checked});
			_body.find("#" + cfg.targetID + "_tree"+type + item.no).get(0).checked = checked;			
	...
	contentScrollResize: function (resetLeft) {
		...
		var agent = navigator.userAgent.toLowerCase(); // 2016-05-27 박경연. ie에서 그리드형 트리 height 오류
		...
		if (scrollWidth > bodyWidth && cfg.xscroll) {
			...
			//x스크롤이 끝까지 이동하지 않을때 1보다 작은 값을 주어 조정한다.
			//AXTree 객체를 생성한 후, config 속성에 xscrollRatio 속성을 추가하여 사용 
			if(typeof cfg.xscrollRatio != 'undefined' && !isNaN(cfg.xscrollRatio)) scrollXHandleWidth = scrollXHandleWidth * cfg.xscrollRatio;
			...
			if (cfg.height == "auto") {
				...
				// 2016-05-27 박경연. ie에서 그리드형 트리 height 오류. 추가 수정 필요
			...
			if (cfg.height == "auto") {
				...
				if(agent.indexOf("chrome") != -1){
					...
					this.scrollBody.css({ height: (scrollBodyHeight + colHeadHeight + 25) }); //colhead + body height
					this.body.css({ top: colHeadHeight, height: (scrollBodyHeight + 25) }); // body Height
				}
				//trace(colHeadHeight + " " + scrollBodyHeight);
			}
		}
	...
	contentScrollTouchstart: function (e) {
		...
		this.contentScrollIDOffset = axdom("#" + cfg.targetID + "_AX_treeBody").offset();
		this.contentScrollXAttr = null;
		this.contentScrollYAttr = null;
