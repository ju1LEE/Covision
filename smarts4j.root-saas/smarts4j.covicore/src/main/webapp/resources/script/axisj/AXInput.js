...
var AXInputConverter = Class.create(AXJ, {
    ...
    appendAnchor: function (objID, objSeq, bindType) {
        ...
        //h = (iobj.css("box-sizing") == "content-box") ? iobj.outerHeight() : iobj.height();			[2016.05.12] 박경연 팝업 안의 달력 버튼 사이즈 조정
        h = iobj.outerHeight();
    ...
    alignAnchor: function (objID, objSeq) {
        ...
        if (obj.bindType == "placeHolder") {
            ...
            axdom("#" + cfg.targetID + "_AX_" + objID).css({"float": "none"});
    ...
    bindSwitch: function (objID, objSeq) {
        ...
        po.push("<div id=\"" + cfg.targetID + "_AX_" + objID + "_AX_SwitchDisplay\" class=\"AXanchorSwitchDisplay\" style=\"height:" + h + "px;line-height:" + h + "px;\"></div>");		// 박경연 [2016.03.29] Switch 안의 글 제거
      ...
      //20160516 안기현 수정. readonly 모드 추가함. ex)coviInput.setSwitch(true);
        if(!obj.config.readonly){
    ...
    bindSwitchClick: function (objID, objSeq, event) {
        ...
        if (obj.switchValue == "on") {
            ...
            //axdom("#" + cfg.targetID + "_AX_" + objID + "_AX_SwitchDisplay").html(obj.config.off);							// 박경연 [2016.03.29] Switch 안의 글 제거
        }
        else {
            axdom("#" + cfg.targetID + "_AX_" + objID + "_AX_SwitchBox").addClass("on");
            obj.switchValue = "on";
            axdom("#" + objID).val(obj.config.on);
            //axdom("#" + cfg.targetID + "_AX_" + objID + "_AX_SwitchDisplay").html(obj.config.on);							// 박경연 [2016.03.29] Switch 안의 글 제거
        }
        if (obj.config.onChange || obj.config.onchange) {
            var sendObj = {
                targetID: objID,
        ...
        if (obj.modal && obj.modal.opened) { /* mobile modal close */
            ...
            if (obj.config.onChange) {
                if (axdom.isFunction(obj.config.onChange)) {
                    ...
                    if (st_date != null && st_date != "" && ed_date != "" && ed_date != null) {
