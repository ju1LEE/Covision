...
var AXInputConverterPro = Class.create(AXJ, {
	...
	bindPatternGetValue: function(objID, objSeq, val, eventType) {
		...
		var getNumberApplyConfig = function(_val, valType) {
			...
			if(Object.isNumber(obj.config.num_max)) {	// 20161104 안기현 추가 (최대값 적용)
				if(_val > obj.config.num_max) _val = obj.config.num_max;
			}
			if(Object.isNumber(obj.config.num_min)) {
				if(_val < obj.config.num_min) _val = obj.config.num_min;
			}
