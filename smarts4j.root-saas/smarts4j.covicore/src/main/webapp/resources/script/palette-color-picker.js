...
(function($) {
    ...
    plugin.init = function() {
      ...
      .on(click_handler, '.'+ns+'-bubble span.swatch', function(e){
        ...
        if( plugin.settings.set_background === false ) {
        	var data = name+':'+col;
        	$('[name="' + $button.attr('data-target') + '"]').val(data);
        } else {
  ...
  $.fn.setColorPicker = function(option) {
	 var  ns = 'palette-color-picker', // Base attr / class
        col = option,
        name = $("[title="+option+"]").attr('data-color'),
        $button = $('.'+ns+'-button[data-target="colorPicker"]'),
        $bubble = $( this ).closest( '.'+ns+'-bubble' );

      $bubble.find('.active').removeClass('active');

      $('[title='+col+']').addClass('active');
      $button.css('background', name);
