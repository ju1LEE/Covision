...
(function () {
    ...
    var resizeTable = function (editor) {
      // [covieditor] START
      ...
      // };
      var calcFixedDeltas = function (sizes, index, next, delta, minCellSize) {
		if (isPreserveTableColumnResizing(editor)) {
    ...
    var getResizeHandler = function (editor) {
      ...
      var lazyResizingBehaviour = function () {
        return isPreserveTableColumnResizing(editor) ? preserveTable() : resizeTable(editor);
      };
      ...
      var execute = function (operation, guard, mutate, lazyWire, effect) {
        return function (table, target, noEvents) {
          ...
          var behaviours = {
            ...
            resize: isResizeTableColumnResizing(editor) ? resizeTable(editor) : preserveTable(),
    ...
    var getAdvancedTab = function (editor, dialogName) {
      ...
      var borderSelect = {
          type: 'label',
          label: tinymce.util.I18n.translate('Border') + ' ' + tinymce.util.I18n.translate('Selection'),
          items: [
              {
                  type: 'grid',
                  columns: 4,
      ...
      var items = dialogName === 'cell' ? [borderWidth].concat(advTabItems.concat(borderSelect)) : advTabItems;
      // [covieditor] END
      return {
        title: 'Advanced',
        name: 'advanced',
...
// [covieditor] START
    var extractAdvancedStyles = function (dom, elm) {
	  if (elm && elm.nodeName === "TABLE") {
        var cells = dom.select('td,th', elm);
        var firstChildBorderWidth;
        var firstChildBorderStyle;
        var firstChildBorderColor;
        var getFirstChildStyle = function (firstChildStyle, elms, property) {
          for (var i = 0; i < elms.length; i++) {
            var currentStyle;
            if (property === 'border-width') {
              currentStyle = getRaw$2(SugarElement.fromDom(elms[i]), 'border-width').getOr('');
            } else if (property === 'border-style') {
              currentStyle = getRaw$2(SugarElement.fromDom(elms[i]), 'border-style').getOr('solid');
            } else if (property === 'border-color') {
              currentStyle = getRaw$2(SugarElement.fromDom(elms[i]), 'border-color').map(rgbToHex(dom)).getOr('#000000');
            }
        ...
        return {
            borderwidth: getFirstChildStyle(firstChildBorderWidth, cells, 'border-width'),
            borderstyle: getFirstChildStyle(firstChildBorderStyle, cells, 'border-style'),
            bordercolor: getFirstChildStyle(firstChildBorderColor, cells, 'border-color'),
            backgroundcolor: getRaw$2(element, 'background-color').map(rgbToHex(dom)).getOr('#FFFFFF')
        };
      } else {
        var element = SugarElement.fromDom(elm);
        return {
          ...
          bordercolor: getRaw$2(element, 'border-color').map(rgbToHex(dom)).getOr('#000000'),
          backgroundcolor: getRaw$2(element, 'background-color').map(rgbToHex(dom)).getOr('#FFFFFF')
        };
      }
    };
    ...
    var extractDataFromTableElement = function (editor, elm, hasAdvTableTab) {
      var getBorder = function (dom, elm) {
        ...
        // [covieditor] START
        return getTDTHOverallStyle(editor.dom, elm, 'border-width') || dom.getAttrib(elm, 'border') || getTDTHOverallStyle(editor.dom, elm, 'border');
    ...
    // [covieditor] START
    var processBorderSelection = function (editor, cell, data) {
        var dom = editor.dom;
        var oldData = extractAdvancedStyles(dom, cell);
        var modifiedValue = {
        ...
        var makeBorderStyle = function (oldValue, newValue, defaultValue, data) {
            var top, right, bottom, left;
            var defaultValueArray;

            var format = oldValue.split(' ');
            if (oldValue.length == 0) {
                defaultValueArray = [defaultValue, defaultValue, defaultValue, defaultValue];
            } else if (format && format.length === 1) {
                defaultValueArray = [oldValue, oldValue, oldValue, oldValue];
            } else if (format && format.length === 2){
                defaultValueArray = [format[0], format[1], format[0], format[1]];
            } else if (format && format.length === 3){
                defaultValueArray = [format[0], format[1], format[2], format[1]];
            } else if (format && format.length === 4){
                defaultValueArray = format;
            } else {
                defaultValueArray = [defaultValue, defaultValue, defaultValue, defaultValue];
            }

            top = data.bordertop ? newValue ? newValue : defaultValueArray[0] : defaultValueArray[0];
            right = data.borderright ? newValue ? newValue : defaultValueArray[1] : defaultValueArray[1];
            bottom = data.borderbottom ? newValue ? newValue : defaultValueArray[2] : defaultValueArray[2];
            left = data.borderleft ? newValue ? newValue : defaultValueArray[3] : defaultValueArray[3];

            return (top + ' ' + right + ' ' + bottom + ' ' + left);
        }

        if (data.bordertop || data.borderbottom || data.borderleft || data.borderright) {
            //case : some false : top right bottom left
            if (!data.bordertop || !data.borderright || !data.borderbottom || !data.borderleft) {
                modifiedValue.borderwidth = makeBorderStyle(oldData.borderwidth, addPxSuffix(data.borderwidth), '1px', data);
                modifiedValue.borderstyle = makeBorderStyle(oldData.borderstyle, data.borderstyle, 'solid', data);
                modifiedValue.bordercolor = makeBorderStyle(oldData.bordercolor, data.bordercolor, '#000000', data);
            }
        } else {
            modifiedValue.borderwidth = oldData.borderwidth;
            modifiedValue.borderstyle = oldData.borderstyle;
            modifiedValue.bordercolor = oldData.bordercolor;
        }
    ...
    var applyStyleData$1 = function (editor, cells, data) {
      ...
      each$2(cells, function (item) {
        ...
        if (hasAdvancedCellTab(editor)) {
          ...
          updateAdvancedProps$1(modifier, processBorderSelection(editor, cellElm, data));
          // [covieditor] END
    ...
    var open$2 = function (editor, selections) {
      ...
      editor.windowManager.open({
        ...
        onTabChange: function(api, details) {
          if (details && details.newTabName === 'advanced') {
              var data = api.getData();
              if (data && data.bordertop === false && data.borderbottom === false && data.borderleft === false && data.borderright === false) {
                  api.setData({ bordertop: true, borderbottom: true, borderleft: true, borderright: true});
                  //border format is top right bottom left => default value change
                  var format;
                  if (data.borderwidth && data.borderwidth.length > 0) {
                      format = data.borderwidth.split(' ');
                      if (format && format.length > 1) {
                          api.setData({borderwidth: ''});
                      }
                  }
                  if (data.borderstyle && data.borderstyle.length > 0) {
                      format = data.borderstyle.split(' ');
                      if (format && format.length > 1) {
                          api.setData({borderstyle: ''});
                      }
                  }
                  if (data.bordercolor && data.bordercolor.length > 0) {
                      format = data.bordercolor.split(' ');
                      if (format && format.length > 1) {
                          api.setData({bordercolor: '#000000'});
                      }
                  }
              }
          }
    ...
    var getItems = function (editor, classes, insertNewTable) {
      ...
      return rowColCountItems.concat(alwaysItems).concat(appearanceItems).concat(selectionItem).concat(alignmentItem).concat(classListItem);
      // [covieditor] END
    };

    // [covieditor] START
    var processBorderOutlineBySelection = function(dom, elm, selection, data, name) {
        var isOutline = function(elm, direction) {
            var rowIndex = elm.parentElement.rowIndex;
            var cellIndex = elm.cellIndex;
            var lastRowIndex = elm.parentElement.parentElement.rows.length - 1;
            var lastCellIndex = elm.parentElement.cells.length - 1;
            var parentEl = elm.parentElement;
            var result = false;
			var rowSpan = parseInt(dom.getAttrib(elm, 'rowspan'), 10);
            if (rowSpan > 1 && direction === 'bottom') {
                rowIndex = rowIndex + rowSpan - 1;
            }

            if (parentEl) {
                var cellRect = elm.getBoundingClientRect();
                var parentRect = parentEl.getBoundingClientRect();

                switch(direction) {
                    case 'top':
                        result = rowIndex === 0 && (Math.abs(parentRect.top - cellRect.top) < 0.1);
                        break;
                    case 'right':
                        result = cellIndex === lastCellIndex && (Math.abs(parentRect.right - cellRect.right) < 0.1);
                        break;
                    case 'bottom':
                        if (rowSpan > 1) {
                            result = rowIndex === lastRowIndex;
                        } else {
                            result = rowIndex === lastRowIndex && (Math.abs(parentRect.bottom - cellRect.bottom) < 0.1);
                        }
                        break;
                    case 'left':
                        result = cellIndex === 0 && (Math.abs(parentRect.left - cellRect.left) < 0.1);
                        break;
                }
            }
		...
		var extractBorderStyle = function (elm, name) {
            var defaultValue;
            var defaultValueArray = [];
            var borderStyle;
            var format;
            ...
            if (name === 'border-style') {
                defaultValue = 'solid';
                borderStyle = dom.getStyle(elm, 'borderStyle');
            } else if (name === 'border-width') {
                defaultValue = '1px';
                borderStyle = dom.getStyle(elm, 'borderWidth');
            } else if (name === 'border-color') {
                defaultValue = '#000000';
                borderStyle = getRaw$2(SugarElement.fromDom(elm), 'border-color').map(rgbToHex(dom)).getOr('#000000');
            }

            format = borderStyle.split(' ');

            if (borderStyle === '') {
                defaultValueArray = [defaultValue, defaultValue, defaultValue, defaultValue];
            } else if (format && format.length === 1) {
                defaultValueArray = [format[0], format[0], format[0], format[0]];
            } else if (format && format.length === 2) {
                defaultValueArray = [format[0], format[1], format[0], format[1]];
            } else if (format && format.length === 3) {
                defaultValueArray = [format[0], format[1], format[2], format[1]];
            } else if (format && format.length === 4) {
                defaultValueArray = [format[0], format[1], format[2], format[3]];
            } else {
                defaultValueArray = [defaultValue, defaultValue, defaultValue, defaultValue];
            }
        ...
        var makeBorderStyle = function(elm, name, value, selection) {
            var newValue = name === 'border-width' ? addPxSuffix(value) : value;
            var top, right, bottom, left;
            var defaultValue;
            ...
            defaultValue = extractBorderStyle(elm, name);

            if (selection === 'inside') {
                top = isOutline(elm, 'top') ? defaultValue[0] : newValue;
                right = isOutline(elm, 'right') ? defaultValue[1] : newValue;
                bottom = isOutline(elm, 'bottom') ? defaultValue[2] : newValue;
                left = isOutline(elm, 'left') ? defaultValue[3] : newValue;
            } else {
                top = !isOutline(elm, 'top') ? defaultValue[0] : newValue;
                right = !isOutline(elm, 'right') ? defaultValue[1] : newValue;
                bottom = !isOutline(elm, 'bottom') ? defaultValue[2] : newValue;
        ...
        if (name['border-style']) {
            name['border-style'] = makeBorderStyle(elm, 'border-style', data.borderstyle, selection);
        }

        if (name['border-width']) {
            name['border-width'] = makeBorderStyle(elm, 'border-width', data.border, selection);
        }

        if (name['border-color']) {
            name['border-color'] = makeBorderStyle(elm, 'border-color', data.bordercolor, selection);
        }
    };

    var styleTDTH = function (dom, elm, selection, data, name, value) {
        if (elm.tagName === 'TD' || elm.tagName === 'TH') {
            if (isString(name)) {
                dom.setStyle(elm, name, value);
            } else {
                processBorderOutlineBySelection(dom, elm, selection, data, name);
                dom.setStyle(elm, name);
            }
        } else {
            if (elm.children) {
                for (var i = 0; i < elm.children.length; i++) {
                    styleTDTH(dom, elm.children[i], selection, data, name, value);
                }
            }
        }
    };
    var applyDataToElement = function (editor, tableElm, data, modifiedData) {
        var dom = editor.dom;
        var attrs = {};
        var styles = {};
        ...
        if (shouldStyleWithCss(editor) && tableElm.children) {
            for (var i = 0; i < tableElm.children.length; i++) {
                if (modifiedData.borderstyle || modifiedData.border || modifiedData.cellpadding) {
                    styleTDTH(dom, tableElm.children[i], data.selection, data, {
                ...
                if (hasAdvancedTableTab(editor) && modifiedData.bordercolor) {
                    styleTDTH(dom, tableElm.children[i], data.selection, data, { 'border-color': data.bordercolor });
                }
                if (hasAdvancedTableTab(editor) && modifiedData.backgroundcolor) {
                    styleTDTH(dom, tableElm.children[i], data.selection, data, { 'background-color': '' });
                }
            }
        }
    ...
    var onSubmitTableForm = function (editor, tableElm, oldData, api) {
      ...
      var modifiedData = filter$1(data, function (value, key) {
		// [covieditor] START
        if (key === 'border' || key === 'borderstyle' || key === 'bordercolor') {
            return true;
        }
        // [covieditor] END
        return oldData[key] !== value;
      ...
      editor.undoManager.transact(function () {
        ...
        if (size(modifiedData) > 0) {
          ...
          applyDataToElement(editor, tableElm, data, modifiedData);
          // [covieditor] END
