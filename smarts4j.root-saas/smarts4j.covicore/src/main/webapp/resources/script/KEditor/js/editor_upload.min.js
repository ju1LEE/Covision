...
function upload_Drop_zone_click(a){var b="Filedata",c="",d=!1;null!=document.getElementById("drop_zone_uploadtype")&&(c=document.getElementById("drop_zone_uploadtype").value);a.currentTarget&&a.currentTarget.getAttribute("drop_zone_uploadtype")&&(c=a.currentTarget.getAttribute("drop_zone_uploadtype"),b+="_"+c,d=null!=a.currentTarget.getAttribute("useOnlyHtml5")?!0:!1);null==document.getElementById(b)&&(b=KEDITORTOP.G_CURRKEDITOR._config.fileFieldID);if("1"==KEDITORTOP.G_CURRKEDITOR._config.useKManager&&
0==d){a=null;switch(c){case "image":case "input_image":a=1;"1"==KEDITORTOP.G_CURRKEDITOR._config.insertMultiImage&&"image"==c&&null==elemModify&&(a=0);a={agentCmd:"KC01",maxTotalCount:a,maxOneFileSize:0,limitOrAllow:0,limitOrAllowExt:"",getSize:1,callBack:function(a){"image"==c&&(g_curr_width=[],g_curr_height=[]);for(var b={files:[]},d=a.length,g=0;g<d;g++)b.files.push({name:a[g].path.substring(a[g].path.lastIndexOf("\\")+1),size:a[g].size,path:a[g].path,width:a[g].width,height:a[g].height}),"image"==
c&&(g_curr_width.push(a[g].width),g_curr_height.push(a[g].height));fileOnchange(b,c)}};break;case "video":case "video_media":case "poster_media":a={agentCmd:"KC01",maxTotalCount:1,maxOneFileSize:0,limitOrAllow:0,limitOrAllowExt:"",getSize:0,callBack:function(a){var b={files:[]};b.files.push({name:a[0].path.substring(a[0].path.lastIndexOf("\\")+1),size:a[0].size,path:a[0].path});fileOnchange(b,c,"drop_zone_"+c)}};break;case "insert_file":case "hyperlink_image":case "flash":case "media":case "m_image":case "doc_bg_image":case "cell_bg_image":case "p_table_bg_image":case "table_bg_image":a=
...
(d+=5));var f=null,f=null==b||void 0==b?document.getElementById("drop_zone"):b,f=KEditorLayerWin.getClientRect(f),p=f.right-f.left,g=f.bottom-f.top;b&&(0>b.className.indexOf("raonkdropzone_over")&&(b.className=b.className.replace("raonkdropzone","raonkdropzone_over")),d+=f.left,h+=f.top);var e=[["kcmd","KC02"]];e.push(["k16",c._config.managerLang]);e.push(["k17",0]);e.push(["k40",0]);e.push(["s01",encodeURIComponent("*.*")]);var m=1;"1"==KEDITORTOP.G_CURRKEDITOR._config.insertMultiImage&&(m=0);e.push(["s02",
m+""]);e.push(["s03","0"]);e.push(["s04","0"]);e.push(["s05",0]);e.push(["s06",0]);e.push(["s07",encodeURIComponent("")]);d=d+f.left+KEDITORTOP.RAONKSolution.Agent.vertical;d+=h+f.top+KEDITORTOP.RAONKSolution.Agent.vertical;d+=p+KEDITORTOP.RAONKSolution.Agent.vertical;d+=g+KEDITORTOP.RAONKSolution.Agent.vertical;d+=c._config.dropZoneTransparency;e.push(["kp1",d]);e.push(["kp2",1]);var l=KEditorLayerWin.setManagerParam("{}",e),l=KEDITORTOP.RAONKSolution.Agent.makeEncryptManagerParam(l),n=function(a){KEDITORTOP.RAONKEDITOR.IsCreatedDropZone=
