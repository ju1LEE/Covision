<%@page import="com.raonwiz.keditor.RAONKHandler"%>
<%@page import="com.raonwiz.keditor.event.*"%>
<%@page import="com.raonwiz.keditor.util.EventVo"%>
<%@page import="com.raonwiz.keditor.common.ImageApi"%>
...
<%
	...
	editor.settingVo.setAllowFileExtension("gif, jpg, jpeg, png, bmp, wmv, asf, swf, flv");//avi, mpg, mpeg, mp4, txt, doc, docx, xls, xlsx, ppt, pptx, hwp, zip, pdf, 
	...
	*/
	...
	event.addAfterUploadEventListener(new AfterUploadEventListener() {
		public void afterUploadEvent(EventVo eventVo) {
	        ...
	        String frontPath = RedisDataUtil.getBaseConfig("FrontStorage").replace("/{0}", "");
	        String strImageFileURL = frontPath+strResponseFileServerPath.substring(0,strResponseFileServerPath.indexOf("_"))+strResponseFileServerPath;
	        
	        String[] aryParameterValue = eventVo.getEditor().getParameterValue("ParameterName"); //클라이언트에서 AddFormData를 이용하여 추가된 파라미터를 얻습니다.

	        eventVo.setResponseFileServerPath(strImageFileURL); //Change ResponseFileServerPath Value strResponseFileServerPath
	...
	if(osType.equals("UNIX")){ 	// global.properties에 있는 FrontStorage 까지의 물리 경로.
	...
	//임시파일 물리적 경로설정
  	editor.settingVo.setTempPath(strUploadPath);//"D:\temp"C:\\eGovFrame-3.5.1\\FrontStorage\\temp
	
	//실제 업로드 할 기본경로 설정 (가상경로와 물리적 경로로 설정 가능)
  	//폴더명 제일 뒤에 .과 공백이 있다면 제거하시고 설정해 주세요.(운영체제에서 지원되지 않는 문자열입니다.)
  
  	//editor.settingVo.setVirtualPath("/FrontStorage");//"/raonkeditordata"/FrontStorage
	...
	editor.settingVo.setPhysicalPath(strUploadPath);
	...
	//editor.settingVo.setContextPath("");
