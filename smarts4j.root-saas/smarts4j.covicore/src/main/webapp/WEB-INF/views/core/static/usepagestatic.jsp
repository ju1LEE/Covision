...
<script type="text/javascript">
	...
	var headerData = [{key:'Items',  label:'<spring:message code="Cache.lbl_item"/>', width:'100', align:'center'},
	                  {key:'UseCount',  label:'<spring:message code="Cache.lbl_UseCount"/>', width:'100', align:'center', formatter:function(){
	                	  return this.value.number();
	                  }},
	                  {key:'UserCount',  label:'<spring:message code="Cache.lbl_UserCount"/>', width:'100', align:'center', formatter:function(){
	                	  return this.value.number();
	                  }},
	...
	function setChart(list){
		...
		if(myBarChart != null && myBarChart != undefined){
			myBarChart.removeAllData();
		}
		
		myBarChart = new Chart(ctx_bar).HorizontalBar(data_bar);
	}
	...
	function searchChart(){
		...
		$.ajax({
 			url:"static/getstaticusepagelist.do",
 			method:"POST",
 			data:{
 				"startdate":strdate,
 				"enddate":enddate,
...
<h3 class="con_tit_box">
	<span class="con_tit"><spring:message code="Cache.tte_UsePageStatic"/></span>
...
<form id="form1">
	<div style="width:100%;min-height: 500px">
		<div id="topitembar_1" class="topbar_grid">
			<label>
				<input type="button" id="per_page" value="<spring:message code="Cache.btn_EachPage"/>" onclick="setMode('Page');"class="AXButton"/>
				<input type="button" id="per_system" value="<spring:message code="Cache.btn_EachSystem"/>" onclick="setMode('System');"class="AXButton"/>
				<input type="button" id="per_service" value="<spring:message code="Cache.btn_EachService"/>" onclick="setMode('Service');"class="AXButton"/>
			</label>
		</div>
		...
		<div style="text-align: center; widows: 100%; margin: 20px">
			<canvas id="canvas_bar" height="400" width="800"></canvas>
		</div>
		<div id="userpagegrid"></div>
	</div>
	<input type="hidden" id="hidden_mode_val" value="Page"/>
</form>