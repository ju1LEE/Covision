...
<h3 class="con_tit_box">
	<span class="con_tit"><spring:message code="Cache.tte_ConnectionLog"/></span>
...
<script type="text/javascript">
	...
	//검색기간 제한
		...
		$("#enddate").attr("minDate", XFN_getDateString(searchDate, 'yyyy-MM-dd')); */
		 headerData =[{key:'UserCode', label:'<spring:message code="Cache.lbl_User"/>', width:'70', align:'center'},
		                 ...
		                 /*  {key:'StayTime', label:'<spring:message code="Cache.lbl_StayEN"/>(sec)', width:'65', align:'center'}, */
		                  {key:'LogonDate', label:'<spring:message code="Cache.lbl_LogonDate"/>' + Common.getSession("UR_TimeZoneDisplay"),   width:'100', align:'center', sort:'desc', formatter: function(){
		  					return CFN_TransLocalTime(this.item.LogonDate);
		  				  }, dataType:'DateTime'},
		                  {key:'LogoutDate', label:'<spring:message code="Cache.lbl_LogoutDate"/>' + Common.getSession("UR_TimeZoneDisplay"), width:'130', align:'center', formatter: function(){
	...
	//LOGID,LOGONID,IPADDRESS,OS,BROWSER,LOGONDATE
	...
	function searchLog(){
		...
		myGrid.bindGrid({
 			ajaxUrl:"/covicore/log/getconnectionloglist.do",
 			ajaxPars: {
 				"startdate":strdate,
 				"enddate":enddate,
 				"searchtext":text
	...
	function ExcelSave(){
		if(confirm("<spring:message code='Cache.msg_ExcelDownMessage'/>")){
			...
			location.href = "/covicore/connectionlogexceldownload.do?startdate="+strdate+"&enddate="+enddate+"&searchtext="+text
