...
<h3 class="con_tit_box">
	<span class="con_tit"><spring:message code="Cache.CN_190"/></span>
...
<form>
	<div style="width:100%;min-height: 500px">
		<div id="topitembar_1" class="topbar_grid">
			...
			<input type="button" value="<spring:message code="Cache.btn_Add"/>" onclick="viewArbitraryGroupPop('L', '', 'add',''); return false;" class="AXButton BtnAdd" />
			...
			<input type="button" value="<spring:message code="Cache.btn_ExcelDownload"/>" onclick="excelDownload('level');"class="AXButton BtnExcel"/> <!-- 엑셀 다운로드 -->
		</div>
		<div class="topbar_grid">
			...
			<span> <spring:message code="Cache.lbl_SearchCondition"/><!-- 검색 조건 -->
				<select name="" class="AXSelect" id="searchType">
					...
					<option value="JobLevelCode"><spring:message code='Cache.lbl_JobLevel_Code'/></option> <!-- 직급코드 -->
				</select>
...
<script>
	...
	function setGrid(){
		orgGrid.setGridHeader([
							  ...
							  {key:'GroupCode', label:"<spring:message code='Cache.lbl_JobLevel_Code'/>" , width:'70', align:'center', formatter : function(){
								  return "<a href='#' onclick='viewArbitraryGroupPop(\"L\", \""+ this.item.GroupCode +"\", \"modify\", \""+ this.item.PrimaryMail +"\"); return false;'>"+ "<span name='code'>" + this.item.GroupCode + "</span>"+"</a>";
							  }}, //직급코드
	...
	function viewArbitraryGroupPop(pType, pCode, pMode, mail){
		if(pMode == 'add'){
			var title = "<spring:message code='Cache.lbl_JobLevelAdd'/>";
		} else{
			var title = "<spring:message code='Cache.lbl_JobLevelEdit'/>";
		}
	...
	function getGridHeader( pHeaderType ){
		var headerData = [
			                  {key:'GroupCode',  label:'<spring:message code="Cache.lbl_JobLevel_Code"/>', width:'100', align:'center'},
