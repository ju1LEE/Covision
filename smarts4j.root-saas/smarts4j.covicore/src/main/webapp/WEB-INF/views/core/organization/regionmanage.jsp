...
<h3 class="con_tit_box">
	<span class="con_tit"><spring:message code="Cache.CN_192"/></span>
...
<form>
	<div style="width:100%;min-height: 500px">
		<div id="topitembar_1" class="topbar_grid">
			...
			<input type="button" value="<spring:message code="Cache.btn_Add"/>" onclick="viewRegionPop('', 'add'); return false;" class="AXButton BtnAdd" />
			...
			<input type="button" value="<spring:message code="Cache.btn_ExcelDownload"/>" onclick="excelDownload('region');"class="AXButton BtnExcel"/> <!-- 엑셀 다운로드 -->
		</div>
		<div class="topbar_grid">
			...
			<span> <spring:message code="Cache.lbl_SearchCondition"/><!-- 검색 조건 -->
				<select name="" class="AXSelect" id="searchType">
					...
					<option value="RegionCode"><spring:message code='Cache.lbl_RegionCode'/></option> <!-- 지역코드 -->
				</select>
...
<script>
	...
	function setGrid(){
		orgGrid.setGridHeader([
							  ...
							  {key:'DomainCode',  display:false}, //회사코드
							  {key:'DomainName', label:"<spring:message code='Cache.lbl_Company'/>", width:'70', align:'center', formatter : function(){
								  return "<a href='#' onclick='viewRegionPop(\""+ this.item.GroupCode +"\", \"modify\"); return false;'>"+ "<span name='code'>" + this.item.DomainName + "</span>"+"</a>";
							  }}, //회사
							  {key:'GroupCode', label:"<spring:message code='Cache.lbl_RegionCode'/>", width:'70', align:'center', formatter : function(){
								  return "<a href='#' onclick='viewRegionPop(\""+ this.item.GroupCode +"\", \"modify\"); return false;'>"+ "<span name='code'>" + this.item.GroupCode + "</span>"+"</a>";
							  }}, //지역(사업장) 코드
			                  {key:'GroupName',  label:"<spring:message code='Cache.lbl_DisplayName'/>", width:'70', align:'center'}, //표시이름
			                  {key:'SortKey',  label:"<spring:message code='Cache.lbl_PriorityOrder'/>", width:'40', align:'center'}, //우선순위
	...
	function viewRegionPop(pCode, pMode){
		
		if(pMode == 'add'){
			var title = "<spring:message code='Cache.lbl_RegionAdd'/>";
		} else{
			var title = "<spring:message code='Cache.lbl_RegionEdit'/>";
		}
		Common.open("", "ViewRegion", title, "/covicore/regioninfopop.do?code=" + encodeURIComponent(pCode) + "&mode=" + encodeURIComponent(pMode) + "&company=" + encodeURIComponent($("#domainCodeSelectBox :selected").val()), "530px", "480px", "iframe", true, null, null, true);
		return;
	}
	...
	function getGridHeader( pHeaderType ){
		var headerData = [
			                  ...
			                  {key:'GroupCode',  label:'<spring:message code="Cache.lbl_RegionCode"/>', width:'100', align:'center'},
