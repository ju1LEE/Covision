...
<h3 class="con_tit_box">
	<span class="con_tit"><spring:message code="Cache.CN_189"/></span>
...
<form>
	<div style="width:100%;min-height: 500px">
		<div id="topitembar_1" class="topbar_grid">
			...
			<input type="button" value="<spring:message code="Cache.btn_Add"/>" onclick="viewArbitraryGroupPop('P', '', 'add',''); return false;" class="AXButton BtnAdd" />
			...
			<input type="button" value="<spring:message code="Cache.btn_ExcelDownload"/>" onclick="excelDownload('position');"class="AXButton BtnExcel"/> <!-- 엑셀 다운로드 -->
		</div>
		<div class="topbar_grid">
			...
			<span> <spring:message code="Cache.lbl_SearchCondition"/><!-- 검색 조건 -->
				<select name="" class="AXSelect" id="searchType">
					...
					<option value="JobPositionCode"><spring:message code='Cache.lbl_JobPosition_Code'/></option> <!-- 직책코드 -->
				</select>
...
<script>
	...
	function setGrid(){
		orgGrid.setGridHeader([
							  ...
							  {key:'GroupCode', label:"<spring:message code='Cache.lbl_JobPosition_Code'/>", width:'70', align:'center', formatter : function(){
								  return "<a href='#' onclick='viewArbitraryGroupPop(\"P\", \""+ this.item.GroupCode +"\", \"modify\", \""+ this.item.PrimaryMail +"\"); return false;'>"+ "<span name='code'>" + this.item.GroupCode + "</span>"+"</a>";
							  }}, //직위코드
	...
	function viewArbitraryGroupPop(pType, pCode, pMode, mail){
		if(pMode == 'add'){
			var title = "<spring:message code='Cache.lbl_JobPositionAdd'/>";
		} else{
			var title = "<spring:message code='Cache.lbl_JobPositionEdit'/>";
		}
	...
	function getGridHeader( pHeaderType ){
		var headerData = [
			                  {key:'GroupCode',  label:'<spring:message code="Cache.lbl_JobPosition_Code"/>', width:'100', align:'center'},
