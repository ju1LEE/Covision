		...
		for(String importDomainId : importDomainIDList) { // 내보낼 메뉴를 받을 도메인 리스트
			...
			if(list.size() > 0) { // 내보낼 메뉴들에 대해서 데이터를 가공
				...
				for (int i = 0; i < menuList.size(); i++) {
					...
					String updateSortPath = String.format("%015d", maxSortKey) + ";";
					
					if (cMap.containsKey("sortPath")) {
						String sortPath = cMap.getString("sortPath").replaceFirst(originSortPath, updateSortPath);
						int sortKey = Integer.parseInt(sortPath.substring(sortPath.lastIndexOf(";") - 2 ,sortPath.lastIndexOf(";")));
						
						cMap.put("sortPath", sortPath);
						cMap.put("sortKey", sortKey);
					} else {
				...
				exportParam.put("domainID", importDomainId);
