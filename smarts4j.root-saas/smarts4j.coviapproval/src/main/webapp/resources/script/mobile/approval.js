...
function mobile_approval_getActiveModule() {
	// 문서오픈 후 닫을때 초기화할 이벤트 없음(전자결재 미결함에서 경비결재 문서오픈 후 닫으면 g_ActiveModule가 account로 유지됨
	//if(!g_ActiveModule) {
	//	g_ActiveModule = $.mobile.activePage.attr("id").split("_")[0];
	//}
	
	try{
		var tmpModule = $.mobile.activePage.attr("id").split("_")[0];
		if(tmpModule == "approval" || tmpModule == "account") g_ActiveModule = tmpModule; // 결재선지정창에서 org로 나오므로 이전값 유지
...
function mobile_approval_ListInit(){	
	...
	if (mobile_comm_getQueryString('searchtext') != 'undefined') {
		...
		{
    		_mobile_approval_list.SearchText = window.sessionStorage.getItem("ApprovalSearchText");
		}
		else {
	...
	if (mobile_comm_getQueryString('mode') != 'undefined' && _mobile_approval_list.Mode == "") {
		_mobile_approval_list.Mode = mobile_comm_getQueryString('mode');
    } else {
    	_mobile_approval_list.Mode = '';
    }
	if (mobile_comm_getQueryString('jobBizCode') != 'undefined' && mobile_comm_getQueryString('jobBizCode') != "" && _mobile_approval_list.JobBizKey == "") {
		_mobile_approval_list.JobBizKey = mobile_comm_getQueryString('jobBizCode');
    } 
	...
	if(_mobile_approval_list.ListMode == "normal") {
	    ...
	    //}, 20);
			mobile_approval_checkReset(); //체크박스 초기화
			mobile_approval_setJobBizBox(_mobile_approval_list.Mode); // 담당업무/업무문서 함 셋팅
...
function mobile_approval_reload() {
	...
	//$("#approval_list_list,#approval_list_searchlist").find("input[type=checkbox][name=approval_list_chkbox]").prop("checked", false).checkboxradio('refresh');
	$("#approval_list_list,#approval_list_searchlist").html("");
	// 리스트조회
	mobile_approval_getList(_mobile_approval_list);
...
function mobile_approval_getTopMenuHtml(approvalmenu) {	
	...
	$(approvalmenu).each(function (i, data){
		...
		*/
		if(data.Reserved1 != "ApprovalUser" && data.Reserved1 != "ApprovalDept" && data.Reserved1 != "JobFunction" && data.Reserved1 != "BizDoc") {
			return;
		}
		
		var sMode = mobile_comm_getQueryStringForUrl(data.URL, 'mode');
		
		// 담당업무함/업무문서함인경우(임의로 하위메뉴생성)
		if(data.Reserved1 == "JobFunction" || data.Reserved1 == "BizDoc"){
			sMode = data.Reserved1;
			
			if(!mJobBizBoxCnt[data.Reserved1]) return; // 권한있는 문서함이 없으면 표시안함
			
			var tmpSubType = [];
			switch(data.Reserved1){
		...
		sHtml += "<li mode=\"" + sMode + "\" displayname=\"" + data.DisplayName + "\">";
		...
		if(nSubLength > 0) {
			...
			var subMode = "";
			var parentDisplayName = (data.DisplayName) ? (data.DisplayName + " - ") : "";
			$(data.Sub).each(function (j, subdata){
				subMode = mobile_comm_getQueryStringForUrl(subdata.URL, 'mode');				
				if(data.Reserved1 == "JobFunction" || data.Reserved1 == "BizDoc") subMode = subdata.subMode; // 담당업무함/업무문서함인경우(임의로 하위메뉴생성)
					
				if(subMode.toUpperCase() == "TEMPSAVE" && mobile_comm_getBaseConfig("useMobileApprovalWrite") != "Y") {
					return;
				}
				sALink = "javascript: mobile_approval_ChangeMenu('" + subMode + "', this);";
				
				sHtml += "    <li mode=\"" + subMode + "\" displayname=\"" + parentDisplayName + subdata.DisplayName + "\">";
				sHtml += "        <a onclick=\"" + sALink + "\" class=\"t_link\">";
				sHtml += "            <span class=\"t_ico_board\"></span>";
				sHtml += "            " + subdata.DisplayName; //mobile_comm_getDicInfo(subdata.MultiDisplayName, lang);
				if(subMode == "Approval" || subMode == "Receive" || subMode.indexOf("Process") > -1 || subMode.indexOf("TCInfo") > -1) {
					if(mTotalCnt[subMode] != undefined) sHtml += "        <span class=\"txt_comment\">(" + mTotalCnt[subMode] + ")</span>";
				}
				sHtml += "        </a>";
				sHtml += "    </li>";
			});
...
function mobile_approval_ChangeFolder(mode) {
	...
	mobile_approval_setJobBizBox(_mobile_approval_list.Mode, true); // 담당업무/업무문서 함 셋팅
	mobile_approval_getList(_mobile_approval_list);
	mobile_approval_getDropmenuList(_mobile_approval_list);
}
...
function mobile_approval_getList(params) {
	...
	if(params.Mode.indexOf("JobFunction") > -1 || params.Mode.indexOf("BizDoc") > -1) url = "/approval/mobile/approval/getMobileJobBizApprovalListData.do";
...
function mobile_approval_getListHtml(apvlist) {
	...
	if(apvlist.length > 0) {
		$(apvlist).each(function (i, apvitem){
			...
			var g_listType = _mobile_approval_list.ListType;
			var mode = "";
			var gloct = "";
			var subkind = "";
			var userID = "";
			...
			switch (g_mode){
				...
				// 담당업무함
				case "JobFunctionApproval" 		: mode = "APPROVAL"; gloct = "JOBFUNCTION"; subkind=apvitem.FormSubKind; userID = $("#approval_list_Jobfunction").val(); break; // 담당업무 미결함 REDRAFT Created
				case "JobFunctionProcess" 		: mode = "PROCESS"; gloct = "JOBFUNCTION"; subkind=apvitem.FormSubKind; userID = $("#approval_list_Jobfunction").val(); break; // 담당업무 진행함 RECAPPROVAL Created
				case "JobFunctionComplete" 		: mode = "COMPLETE"; gloct = "JOBFUNCTION"; subkind=apvitem.FormSubKind; archived="true"; userID = $("#approval_list_Jobfunction").val(); break; // 담당업무 완료함 COMPLETE Finished
				case "JobFunctionReject" 		: mode = "REJECT"; gloct = "JOBFUNCTION"; subkind=apvitem.FormSubKind; archived="true"; userID = $("#approval_list_Jobfunction").val(); break; // 담당업무 반려함 REJECT Finished
				// 업무문서함
				case "BizDocProcess" 			: mode = "PROCESS"; gloct = "BizDoc"; subkind="T006"; userID = $("#approval_list_BizDoc").val(); break; // 업무문서함 진행함 StartDate
				case "BizDocComplete" 			: mode = "COMPLETE"; gloct = "BizDoc"; subkind="T006"; archived="true"; userID = $("#approval_list_BizDoc").val(); break; // 업무문서함 완료함 EndDate
			}
			...
			if(apvitem.ReadDate == "" && (apvitem.ReadCheck == undefined || apvitem.ReadCheck == "N")) {
				sRead = "unread";
			}
			if(_mobile_approval_list.Mode != "TempSave") {
				...
				if(g_mode == "PreApproval" || g_mode == "Approval" || g_mode == "Receive" || g_mode == "DeptProcess" || g_mode == "JobFunctionApproval" || g_mode == "JobFunctionProcess") { //예고함, 미결함, 부서수신함, 진행함(부서) , 담당업무(미결,진행)
				...
				} else if(g_mode.indexOf("Complete") > -1 || _mobile_approval_list.Mode == "Reject") { //완료함/반려함, 부서완료함/발신함/수신처리함 , 업무문서함(완료 BizDocComplete)
					...
					sDate += mobile_comm_getDateTimeString2('list', apvitem.StartDate);
				} 
				sDate += "</span>";
			}			
			//임시함은 Date X
			...
			if(g_mode == "Approval" || g_mode == "Receive" || g_mode.toUpperCase().indexOf("TCINFO") > -1 || g_mode == "Reject" || g_mode == "TempSave" || g_mode == "JobFunctionApproval") {
				...
				if(g_mode == "TempSave" || (apvitem.ExtInfo != undefined && (apvitem.ExtInfo.UseBlocApprove == "Y" || apvitem.ExtInfo.UseBlocCheck == "Y"))) {
					sHtml += "	<input type=\"checkbox\" name=\"approval_list_chkbox\" value='" + JSON.stringify(apvitem) + "' id=\"chk_" + g_mode + "_" + _mobile_approval_list.Page + "_" + i + "_" + g_listType + "\" onchange=\"mobile_approval_showAllChk();\">";
				} else {
					sHtml += "	<input type=\"checkbox\" name=\"approval_list_chkbox\" value='" + JSON.stringify(apvitem) + "' id=\"chk_" + g_mode + "_" + _mobile_approval_list.Page + "_" + i + "_" + g_listType + "\" disabled=\"disabled\">";
				}
				sHtml += "		<label for=\"chk_" + g_mode + "_" + _mobile_approval_list.Page + "_" + i + "_" + g_listType + "\"></label>";
				sHtml += "	</div>";
			} else {
			...
			if(g_mode != "TempSave") {
				...
				sHtml += "			<span class=\"photo\" style=\"background-image:url('" + mobile_comm_getimg(apvitem.PhotoPath) + "'), url('" + _mobile_approval_profileImagePath + "');\"></span>";
...
function mobile_approval_getDocListHtml(apvlist) {
	...
	if(apvlist.length > 0) {
		$(apvlist).each(function (i, apvitem){
			...
			sUrl += "&processID=" + (apvitem.ProcessID == undefined ? (apvitem.ProcessArchiveID == undefined ? '' : apvitem.ProcessArchiveID ) : apvitem.ProcessID);
			sUrl += "&workitemID=" + (apvitem.WorkItemID == undefined ? (apvitem.WorkitemArchiveID == undefined ? '' : apvitem.WorkitemArchiveID ): apvitem.WorkItemID);
			sUrl += "&performerID=" + (apvitem.PerformerID == undefined ? '' : apvitem.PerformerID) ;
			sUrl += "&processdescriptionID=" + (apvitem.ProcessDescriptionID == undefined ? (apvitem.ProcessDescriptionArchiveID == undefined ? '' : apvitem.ProcessDescriptionArchiveID ) : apvitem.ProcessDescriptionID);		
			sUrl += "&userCode=" + userID;
			sUrl += "&gloct=" + gloct;
			...
			sHtml += "			<span class=\"photo\" style=\"background-image:url('" + mobile_comm_getimg(apvitem.PhotoPath) + "'), url('" + _mobile_approval_profileImagePath + "');\"></span>"; //apvitem.
...
// 담당업무/업무문서 함 셋팅
function mobile_approval_setJobBizBox(pMode, bReset){
	var l_ActiveModule = mobile_approval_getActiveModule(); // 전자결재 결재함, e-Accounting 비용결재함 구분
	var tmp_listConfig = l_ActiveModule == "account" ? _mobile_account_list : _mobile_approval_list;
	
	if(!pMode) pMode = tmp_listConfig.Mode;
	
	// 초기화
	if(bReset) tmp_listConfig.JobBizKey = "";
	$("#" + l_ActiveModule + "_list_div_Jobfunction").hide();
	$("#" + l_ActiveModule + "_list_div_BizDoc").hide();
			
	// 셋팅
	switch(pMode){
		case "JobFunctionApproval":
		case "JobFunctionProcess":
		case "JobFunctionComplete":
		case "JobFunctionReject":
			if(bReset || $("#" + l_ActiveModule + "_list_Jobfunction option").length == 0){
				getJobFunctionList(l_ActiveModule);
			}
			if(tmp_listConfig.JobBizKey) $("#" + l_ActiveModule + "_list_Jobfunction").val(tmp_listConfig.JobBizKey);
			else tmp_listConfig.JobBizKey = $("#" + l_ActiveModule + "_list_Jobfunction").val();
			$("#" + l_ActiveModule + "_list_div_Jobfunction").show();
			break;
		case "BizDocProcess":
		case "BizDocComplete":
			if(bReset || $("#" + l_ActiveModule + "_list_BizDoc option").length == 0){
				getBizDocList(l_ActiveModule);
			}
			if(tmp_listConfig.JobBizKey) $("#" + l_ActiveModule + "_list_BizDoc").val(tmp_listConfig.JobBizKey);
			else tmp_listConfig.JobBizKey = $("#" + l_ActiveModule + "_list_BizDoc").val();
			$("#" + l_ActiveModule + "_list_div_BizDoc").show();
			break;
	}
}

// 권한있는 담당업무함 가져오기
function getJobFunctionList(pActiveModule){
	$("#" + pActiveModule + "_list_Jobfunction").empty();
	
	$.ajax({
		url:"/approval/user/getJobFunctionListData.do",
		type:"post",
		data:{
			JobFunctionType:pActiveModule.toUpperCase()
		},			
		async:false,
		success:function (data) {
			$(data.list).each(function(index){
				var boxCode = this.JobFunctionCode;
				var boxName = mobile_comm_getDicInfo(this.JobFunctionName);
				
				$("#" + pActiveModule + "_list_Jobfunction").append($("<option></option>").attr("value",boxCode).text(boxName));
			});
		},
		error:function(response, status, error){
			mobile_comm_ajaxerror("/approval/user/getJobFunctionListData.do", response, status, error);
		}
	});
}

// 권한있는 업무문서함 가져오기
function getBizDocList(pActiveModule){
	$("#" + pActiveModule + "_list_BizDoc").empty();
	
	$.ajax({
		url:"/approval/user/getBizDocListData.do",
		type:"post",
		data:{
			BizDocType:pActiveModule.toUpperCase()
		},
		async:false,
		success:function (data) {
			$(data.list).each(function(index){
				var boxCode = this.BizDocCode;
				var boxName = mobile_comm_getDicInfo(this.BizDocName);
				
				$("#" + pActiveModule + "_list_BizDoc").append($("<option></option>").attr("value",boxCode).text(boxName));
			});
		},
		error:function(response, status, error){
			mobile_comm_ajaxerror("/approval/user/getBizDocListData.do", response, status, error);
		}
	});
}

// 담당업무함,업무문서함 변경
function mobile_approval_changeListJobBiz(pObj){
	var l_ActiveModule = mobile_approval_getActiveModule(); // 전자결재 결재함, e-Accounting 비용결재함 구분
	
	if(l_ActiveModule == "account"){
		_mobile_account_list.JobBizKey = $(pObj).val(); // 업무함 코드 변경
		mobile_account_reload(); // 새로고침
	}else{
		_mobile_approval_list.JobBizKey = $(pObj).val(); // 업무함 코드 변경
		mobile_approval_reload(); // 새로고침
	}
}

//전체선택 div show or hide
function mobile_approval_showAllChk() {
	var objTarget = $("#approval_list_searchlist").is(":visible") ? $("#approval_list_searchlist") : $("#approval_list_list");
	$("#approval_list_div_allchk").hide();
	var cnt = 0;
	...
	if(cnt != 0) {
		...
		if(objTarget.find("input[type=checkbox][name=approval_list_chkbox]:not([disabled=disabled])").length > cnt) {
			$("#approval_list_allchk").prop("checked", false).checkboxradio('refresh');
		} else {
...
function mobile_approval_checkAll(obj) {
	var objTarget = $("#approval_list_searchlist").is(":visible") ? $("#approval_list_searchlist") : $("#approval_list_list");
	if($(obj).is(":checked")) {
		objTarget.find("input[type=checkbox][name=approval_list_chkbox]:not([disabled=disabled])").prop("checked", true).checkboxradio('refresh');
	} else {
		objTarget.find("input[type=checkbox][name=approval_list_chkbox]").prop("checked", false).checkboxradio('refresh');
	}
	mobile_approval_showAllChk();
}

//checkbox 초기화
function mobile_approval_checkReset() {
	$("#approval_list_list,#approval_list_searchlist").find("input[type=checkbox][name=approval_list_chkbox]").prop("checked", false).checkboxradio('refresh');
	mobile_approval_showAllChk();
}
...
function mobile_approval_clickwrite(pFormPrefix) {
	...
	if (isTeams) {
		var url = "/approval/layout/approval_FormList.do?CLSYS=approval&CLMD=user&CLBIZ=Approval";
		var popupId = "TeamsApprovalWrite";
		XFN_TeamsOpenWindow(url, popupId, "POPUP", 1200, 800, "both");
	} else {
...
function mobile_approval_clickmodify(open_mode) {
	...
	if(open_mode == "MODIFY") {
		...
		if(mobile_comm_getBaseConfig("MobileEditorTypeApproval") != "2") {
...
function mobile_approval_getDropmenuList(params) {
	...
	if(params.Mode.toUpperCase() == "APPROVAL") {
		 if(mobile_comm_getBaseConfig("isUsebatchApv") == "Y") sHtml += "<li><a onclick=\"mobile_approval_batchApproval('APPROVAL');\">" + (mobile_comm_getDic("btn_apv_blocApprove") == "" ? "일괄결재" : mobile_comm_getDic("btn_apv_blocApprove")) + "</a></li>"; //일괄결재
		sHtml += "<li><a onclick=\"mobile_approval_doDocRead('" + params.Mode + "');\">" + (mobile_comm_getDic("lbl_apv_ReadCheck") == "" ? "읽음확인" : mobile_comm_getDic("lbl_apv_ReadCheck")) + "</a></li>"; //읽음확인
	}
	if(params.Mode.toUpperCase() == "RECEIVE") {
		 if(mobile_comm_getBaseConfig("isUsebatchApv") == "Y")  sHtml += "<li><a onclick=\"mobile_approval_batchApproval('DEPT');\">" + (mobile_comm_getDic("btn_apv_blocReceipt") == "" ? "일괄접수" : mobile_comm_getDic("btn_apv_blocReceipt")) + "</a></li>"; //일괄접수
	...
	if(params.Mode.toUpperCase() == "JOBFUNCTIONAPPROVAL") {
		sHtml += "<li><a onclick=\"mobile_approval_batchApproval('JOBFUNCTION');\">" + (mobile_comm_getDic("btn_apv_blocApprove") == "" ? "일괄결재" : mobile_comm_getDic("btn_apv_blocApprove")) + "</a></li>"; //일괄결재
	}
	if(sHtml != "")
...
var ApvGraphicView = {
	  ...
	  $$_elmList.concat().each(function (index_div, $$_odiv) {
	    ...
	    $$_osteps.each(function (index_step, $$_ostep) {
			if($$_ostep.find("person>taskinfo").attr("visible") != "n" && $$_ostep.find("person>taskinfo").attr("kind") != "bypass"){
			  var ChargeDuptyChk ="";
			  if( $$_ostep.parent().find('step').json().length > index_step+1 ){
				  if(typeof ($$_ostep.next().find("ou>person>taskinfo").attr("kind"))!="undefined"){
					  ChargeDuptyChk = $$_ostep.next().find("ou>person>taskinfo").attr("kind");
				  }
			  }
			  var step = {
				...
				name: $$_ostep.attr('name'),
				substeps: [],
				chargeduptychk : ChargeDuptyChk
			  };
	...
	getSubStep: function($$_operson, $$_otaskinfo){
	  ...
	  if ($$_operson.nodename() == "role") {
	    ...
	    oudisplayname = $$_operson.attr("ouname");
	    code = $$_operson.attr("code");
	    comment = $$_otaskinfo.attr('comment');
	  }
	...
	/*getPhotoPath: function (code, stepUnitType){
		...
		if(ApvGraphicView.conf.debug) {
			...
			if(stepUnitType === 'person' || stepUnitType === 'ou'){
				photoPath = _mobile_approval_profileImagePath + code + ".jpg";
	  		} else {
	  ...
	  getStepTitle: function (step){
	    ...
	    var namemap = {
	      ...
	      routetype: {
	    	  approve : {
	    		...
	    		conveyance : mobile_comm_getDic("lbl_apv_forward"),				// 전달
	    		charge : mobile_comm_getDic("lbl_apv_charge")			// 수신
	      	},
			...
			audit  : mobile_comm_getDic("lbl_apv_audit"), 							// 감사
			review  : mobile_comm_getDic("lbl_apv_confirmor"), 					// 공람
		  ...
		  name: {
			//감사,준법
			audit_dept : mobile_comm_getDic("lbl_apv_dept_audit3"), 		//부서감사
			audit_law_dept : mobile_comm_getDic("lbl_apv_dept_audit"), 		//부서준법
			audit : mobile_comm_getDic("lbl_apv_person_audit3"), 			//개인감사
			audit_law : mobile_comm_getDic("lbl_apv_person_audit")	 		//개인준법
		  }
	    }
		...
		if(step.routetype == "approve") {
	    	...
	    	if(Array.isArray(step.person_taskinfo_kind)) {
				// 전달자 제외 (대결은 맨위에있지만 전달은 마지막에있음)
				//person_taskinfo_kind = step.person_taskinfo_kind[0];
				$.each(step.person_taskinfo_kind ,function(idx,item){ 
					if(item != "conveyance") {
						person_taskinfo_kind = item;
						return false;
					}
				});
	    	} else {
	    	...
	    	if(step.person_taskinfo_kind == "charge") {
				...
				if(step.chargeduptychk=="bypass") display = namemap['routetype'][step.routetype]['substitute'];
	    	} else {
		...
		} else if(step.routetype == "audit" && step.name){
			display = namemap['name'][step.name];
		}
	    else{
	...
	getGraphicHtml: function (step){
		...
		for(var i = 0 ; i < step.length ; i++) {
			...
			for(var j = 0; j < step[i].steps.length; j++) {
				...
				sHtml += "		<span class=\"photo\" style=\"background-image:url('" + mobile_comm_getimg(steps.substeps[0].photo) + "'), url('" + _mobile_approval_profileImagePath + "');\"></span>";
...
function mobile_approval_ViewInit(){
	...
	if (mobile_comm_getQueryString('usisdocmanager', 'approval_view_page') != 'undefined') {
		_mobile_approval_view.usisdocmanager = mobile_comm_getQueryString('usisdocmanager', 'approval_view_page');
    } else {
    	_mobile_approval_view.usisdocmanager = '';
    }
...
//		}
	...
	if(processdes != undefined && processdes != null && processdes != "") {
		...
		$('#approval_view_initiatorphoto').attr("style", "background-image:url('" + mobile_comm_getimg(mobile_approval_getUserPotoPath(forminstance.InitiatorID, forminstance.PhotoPath)) + "'), url('" + _mobile_approval_profileImagePath + "');");
...
function mobile_approval_clickbtn(obj, type) {
    ...
    if (getApvAgrOpt.length > 0){
    	...
    	if(getInfo("Request.mode") == "PCONSULT" && getInfo("Request.subkind") == "T009" && arrApvOpt[0] == "Y" && type == 'approved'){
    		alert(mobile_comm_getDic("_msg_AgrApprovedDisabled")); //합의자 동의 옵션이 비활성화 되어있습니다.
    		return false;
    	}
    	if(getInfo("Request.mode") == "PCONSULT" && getInfo("Request.subkind") == "T009" && arrApvOpt[1] == "Y"  && type == 'reject'){
    		alert(mobile_comm_getDic("_msg_AgrRejectDisabled")); //합의자 거부 옵션이 비활성화 되어있습니다.
    		return false;
    	}
    }
	...
	if(type != 'toggle') {
		...
		case "approved": //승인
		case "cAgree": //(검토)동의
		case "cDisagree": //(검토)거부
			commentPlaceHolder = $(".btn_approval:visible").text();	//승인
			break;
		case "reject": //반려
		...
		case "hold":
			...
			break;
		case "ConsultReq": //검토요청
			commentPlaceHolder = mobile_comm_getDic("btn_apv_consultReq");
			$('.fixed_btm_wrap').removeClass('open');
			break;
		case "ConsultReqCancel": //검토요청취소
			commentPlaceHolder = mobile_comm_getDic("btn_apv_consultReqCancel");
			break;
		case "ReConsultReq": //재검토요청
			commentPlaceHolder = mobile_comm_getDic("btn_apv_ReconsultReq");
			$('.fixed_btm_wrap').removeClass('open');
			break;
		default:
			break;
		...
		if(type === "cAgree" || type === "cDisagree" || type === "ConsultReq" || type === "ConsultReqCancel" || type === "ReConsultReq"){
			_usePWDCheck = "N";
		}
		
		if (_usePWDCheck == "Y" && mobile_approval_chkUsePasswordYN() ){
			$(".secret").show();
		...
		if(type === "rec"){
			if(_usePWDCheck == "Y"){
				$("#" + l_ActiveModule + "_" + viewMode + "_inputcomment").hide();
				
				if(useFIDO == "Y") {
					$("#" + l_ActiveModule + "_" + viewMode + "_inputpassword").css('width', '60%');
					$("#fido_btn").css({"width":"75px", "float":"none"});
					$("#" + l_ActiveModule + "_" + viewMode + "_btn_OK").css({"right":"-4px","bottom":"7px","height":"37px","line-height":"38px","width":"38px"});
				} else {
					$("#" + l_ActiveModule + "_" + viewMode + "_btn_OK").css("bottom", "12px");
				}
				commentHeight -= 85;
				$(".fixed_btm_wrap .approval_comment .txt_area").css("border", "none");
				
			} else {
				mobile_approval_doOK();
				return;
			}
		} else {
			$("#" + l_ActiveModule + "_" + viewMode + "_inputcomment").show();
		}
...
function mobile_approval_showExtMenu() {
	if($("#approval_view_dropMenuBtn").hasClass("show")) {
		...
		if($("#approval_view_dropMenuBtn").find(".exmenu_layer").length > 0) {
			...
			btnArr.push("btConsultationCommentView;" + mobile_comm_getDic("lbl_apv_consult_comment_view")); //검토의견조회
			btnArr.push("btConsultReq;" + mobile_comm_getDic("btn_apv_consultReq")); //검토요청
			btnArr.push("btConsultReqCancel;" + mobile_comm_getDic("btn_apv_consultReqCancel")); //검토요청취소
			btnArr.push("btConsultReqModify;" + mobile_comm_getDic("btn_apv_consultReqModify")); //검토자변경
			btnArr.push("btReConsultReq;" + mobile_comm_getDic("btn_apv_ReconsultReq")); //재검토요청
			...
			for(var i = 0; i < btnlength; i++) {
				...
				//검토요청
				else if(id == "btConsultReq" || id == "btReConsultReq"){
					var type = id.replace("bt", "");
					sHtml += "<li><a class=\"btn\" onclick=\"mobile_approval_openOrg('" + type + "');\" value=\"" + value + "\">" + value + "</a></li>";
					cnt++;	
				}
				//검토요청취소
				else if(id == "btConsultReqCancel") {
					var type = id.replace("bt", "");
					sHtml += "<li><a class=\"btn\" onclick=\"mobile_approval_clickbtn('', '" + type + "');\" value=\"" + value + "\">" + value + "</a></li>";
					cnt++;
				}
				else if(id == "btConsultReqModify"){
					sHtml += "<li><a class=\"btn\" onclick=\"mobile_approval_openOrg('" + id + "');\" value=\"" + value + "\">" + value + "</a></li>";
					cnt++;	
				}
				else {
...
function mobile_approval_openOrg(sMode) {
	...
	var sCallBackFn = "mobile_approval_setOrg();";
	var	sMultiSelectYN = "N";
	
	 switch (sMode) {
     case "charge": 
     case "ChangeReview": 
    	 sCallType = "SelectUser"; 
    	 break;
     case "chargegroup": 
    	 sCallType = "Select"; 
    	 break;
     ...
     case "ReConsultReq":
    	 sCallType = "SelectUser";
    	 sMultiSelectYN = "Y";
    	 sCallBackFn = "mobile_approval_clickbtn('','"+sMode+"');";
    	 break;
     case "btConsultReqModify":
    	 sCallType = "SelectUser";
    	 sMultiSelectYN = "Y";
    	 
    	 var userParams = ConsultPendingUsers().map(function(item){ return $$(item).attr("code") }).join(";");
    	 window.sessionStorage["userParams"] = userParams;
    	 sCallBackFn = "doButtonAction('" + sMode + "');";
    	 break;
     default: break;
	 }
	...
	window.sessionStorage["multi"] = sMultiSelectYN;
	window.sessionStorage["callback"] = sCallBackFn;
	
	mobile_comm_go(sUrl, 'Y');
}

function mobile_approval_setOrg() {
...
function mobile_approval_WriteInit () {
	...
	if (mobile_comm_getQueryString('usisdocmanager', "approval_write_page") != 'undefined') {
		_mobile_approval_write.usisdocmanager = mobile_comm_getQueryString('usisdocmanager', "approval_write_page");
    } else {
    	if (_mobile_approval_write.usisdocmanager == '') _mobile_approval_write.usisdocmanager = '';
    }
	...
	if(_mobile_approval_write.OpenMode != "APVLIST") {
		...
		if($("#approval_write_formList").find("option").length > 0) {
			...
			alert(mobile_comm_getDic("msg_apv_NoneAuthForm"));
			mobile_comm_back();
		}
	} else {
		var oApproveStep;
...
function mobile_approval_getFormInfo() {
	if(_mobile_approval_write.FormID != "") {		
		var Params = {
			...
			usisdocmanager : _mobile_approval_write.usisdocmanager
		};
...
function mobile_approval_setLinkedDoc(param) {
	if(param != "" && param != undefined) {
		...
		for(var i = 0; i < docs.length; i++) {
			...
			param.processID = doc.split("@@@")[0];
			param.formPrefix = doc.split("@@@")[1];
			param.subject = doc.split("@@@")[2];
			param.forminstanceID = doc.split("@@@")[3];
			param.bstored = doc.split("@@@")[4];
			param.businessData1 = doc.split("@@@")[5];
			param.businessData2 = doc.split("@@@")[6];
...
function mobile_approval_getApprovalLine(apv_type) {
	...
	if (getInfo("Request.mode") == "SUBREDRAFT" || getInfo("Request.mode") == "SUBAPPROVAL") {
        var $$_m_oApvList = $$(apvlist);
        ...
        $$_m_oApvList.find("division").has(">taskinfo[status='pending']").append("step", $$_m_oSubReStep.json());
    }
			...
			for(var j = 0; j < s_length; j++) {
				...
				if (getInfo("Request.mode") == "SUBREDRAFT" || getInfo("Request.mode") == "SUBAPPROVAL") {
					for(var k = 0 ; k < o_length ; k++) {
						...
						var p_length = ou.person.length == undefined ? 1 : ou.person.length;
						
						for(var p = 0 ; p < p_length ; p++) {
							var displayname = "";
		                    var displayBypass = "";
		                    var person = (ou.person[p] == undefined ? ou.person : ou.person[p]);
							
							var code = person.code == undefined ? "" : person.code;
							var name = mobile_comm_getDicInfo(person.name, lang);
							var position = mobile_comm_getDicInfo(person.position.split(";")[1], lang);
							
							switch (person.taskinfo.kind) {
						        case "normal":
						        	displayname = mobile_comm_getDic("lbl_apv_normalapprove");								//일반결재
						        	break;
							    case "consent":
							    	displayname = mobile_comm_getDic("lbl_apv_investigation");								//검토
							    	break;
							    case "authorize":
							    	displayname = mobile_comm_getDic("lbl_apv_authorize");									//전결
							    	break;
							    case "substitute":
							    	displayname = mobile_comm_getDic("lbl_apv_substitue");									//대결
							    	break;
							    case "review":
							    	displayname = mobile_comm_getDic("lbl_apv_review");										//후결
							    	break;
							    case "bypass":
							    	displayname = mobile_comm_getDic("lbl_apv_bypass");										//후열
							    	break;
							    case "charge":
							    	displayname = mobile_comm_getDic("lbl_apv_charge");										//담당
							    	break;
							    case "confidential":
							    	displayname = mobile_comm_getDic("lbl_apv_Confidential");								//친전
							    	break;
							    case "conveyance":
							    	displayname = mobile_comm_getDic("lbl_apv_forward");									//전달
							    	break;
							    case "skip":
							    	displayname = mobile_comm_getDic("lbl_apv_NoApprvl");									//결재안함
							    	break;
							    case "confirm":
							    	displayname = mobile_comm_getDic("lbl_apv_Confirm");									//확인
							    	break;
							    case "reference":
							    	displayname = mobile_comm_getDic("lbl_apv_share4list");									//참조
							    	break;
							    default:
							    	displayname = " ";
							}
							...
							sHtml += "<li class=\"person\" id=\"approval_write_ApvList" + i + "_" + j + "_" + k + "_" + p + "\" " + displayBypass + ">";
							
							if(isApvMod == "Y" && step.unittype != "role" && (person.taskinfo && person.taskinfo.status != "pending" && person.taskinfo.kind != "charge" && person.taskinfo.datecompleted == undefined)) {
								var kind = person.taskinfo.kind;
							...
							if(isApvMod == "N" || person.taskinfo.status == "pending" || person.taskinfo.kind == "charge") {
								
							} else if(person.taskinfo.datecompleted == undefined) {
								sHtml += "		<span class=\"del\" onclick=\"javascript: mobile_approval_delApvUser(this);\"></span>";
							}
							sHtml += "		</div>";
							sHtml += "		<p class=\"name\">" + name + " " + position + "</p>";
							if(person.taskinfo.datecompleted != undefined)
								sHtml += "	<p class=\"name\">" + formatDate(person.taskinfo.datecompleted, "H") + "</p>";
							sHtml += "	</a>";
							if(isApvMod == "N" || person.taskinfo.status == "pending" || person.taskinfo.kind == "charge") {
				...
				// ou
					for(var k = 0 ; k < o_length ; k++) {
						...
						if(step.unittype == "person") {
							...
							if(step.routetype == "receive") {
								displayname = mobile_comm_getDic("lbl_apv_charge_person");										//담당자
							} else if(step.routetype == "consult") {
								if (step.allottype == "parallel") displayname = mobile_comm_getDic("lbl_apv_PersonConsent"); 	//개인합의
						        else displayname = mobile_comm_getDic("lbl_apv_PersonalCon");  									//개인합의
							} else if(step.routetype == "assist") {
								if (step.allottype == "serial") displayname = mobile_comm_getDic("lbl_apv_assist");				//개인협조
						        else if (step.allottype == "parallel") displayname = mobile_comm_getDic("lbl_apv_assist_2");	//개인협조(병렬)
						        else displayname = mobile_comm_getDic("lbl_apv_assist"); 										//개인협조
							} else if(step.routetype == "audit") {
								displayname = mobile_comm_getDic("btn_apv_person_audit"); 										//개인감사
							} else if(step.routetype == "review") {
								displayname = mobile_comm_getDic("lbl_apv_PublicInspect"); 										//공람
							} else if(step.routetype == "notify") {
								displayname = mobile_comm_getDic("lbl_apv_SendInfo"); 											//통보
							} else if(step.routetype == "approve") {
								switch (temp.taskinfo.kind) {
							        case "normal":
							        	displayname = mobile_comm_getDic("lbl_apv_normalapprove");								//일반결재
							        	break;
								    case "consent":
						...
						if(type == "person") {
							...
							if(tempImgPath == undefined || tempImgPath == null)
								...
								tempImgPath = tempImgPath.PhotoPath;
							sHtml += " 		<span class=\"photo\" style=\"background-image:url('" + mobile_comm_getimg(tempImgPath) + "'), url('" + _mobile_approval_profileImagePath + "')\">";
						}
						else {
		...
		if(apv_type == undefined || apv_type == "tcinfo") {
			if(apvlist.steps.ccinfo != undefined) {
				...
				for(var i = 0; i < c_length; i++) {
					...
					if(getInfo("SchemaContext.scBeforCcinfo.isUse") == "Y" && (_mobile_approval_write.OpenMode == "DRAFT" || _mobile_approval_write.OpenMode == "APVLIST")){
					...
					if(type == "person") {
						...
						if(tempImgPath == undefined || tempImgPath == null)
							...
							tempImgPath = tempImgPath.PhotoPath;
						scHtml += " 		<span class=\"photo\" style=\"background-image:url('" + mobile_comm_getimg(tempImgPath) + "'), url('" + _mobile_approval_profileImagePath + "')\">";
					}
					else {
	...
	if(getInfo("SchemaContext.scIPub.isUse") == "Y") {
		if(getInfo("Request.mode") !== "REDRAFT" && getInfo("Request.mode") !== "RECAPPROVAL" && getInfo("Request.mode") !== "SUBREDRAFT" && getInfo("Request.mode") !== "SUBAPPROVAL")
					...
					if(receive != "") {
						...
						if(type == "person") {
							...
							if(tempImgPath == undefined || tempImgPath == null)
								...
								tempImgPath = tempImgPath.PhotoPath;
							sdHtml += " 		<span class=\"photo\" style=\"background-image:url('" + mobile_comm_getimg(tempImgPath) + "'), url('" + _mobile_approval_profileImagePath + "')\">";
						}
						else {
...
function mobile_approval_getApvTypeSelectOption(division, unittype, routetype, allottype, kind, name) {
	...
	if(unittype == "person" && !(routetype == "approve" && kind == "charge" && name == "기안자")) {
		...
		if(getInfo("Request.mode") != "SUBREDRAFT" && getInfo("Request.mode") != "SUBAPPROVAL" && division == "0") {
...
function mobile_approval_moveApvUser(pMode, obj) {
	...
	if(getInfo("Request.mode") == "SUBREDRAFT" || getInfo("Request.mode") == "SUBAPPROVAL") {
		...
		var division1 = apvlist.steps.division[d_num] == undefined ? apvlist.steps.division : apvlist.steps.division[d_num];
		
		s_num = $$(division1).find("step").has("taskinfo[status='pending'][piid='" + getInfo("ProcessInfo.ProcessID").toUpperCase() + "']").index();
		
		if(s_num > 0) {
			var step = division1.step[s_num] == undefined ? division1.step : division1.step[s_num];
			var o_num = $$(division1).find("step > ou").has("taskinfo[status='pending'][piid='" + getInfo("ProcessInfo.ProcessID").toUpperCase() + "']").index(); //ou
			var p_num = $(obj).parents("li").attr("id").replace("approval_write_ApvList", "").split("_")[3]; //person
			
			var ou = step.ou[o_num] == undefined ? step.ou : step.ou[o_num];
			
			last1 = ou.person.length != undefined ? ou.person.length-1 : 0;
			
			if(d_num == "0" && p_num == "0") {
				alert(mobile_comm_getDic("msg_apv_ApvLineMoveCheck1")); //해당 사용자는 기안자이므로 이동 불가합니다.
				return false;
			} else if ((d_num == "0" && p_num == "1" && pMode == "prev") || (d_num == "1" && p_num == "0" && pMode == "prev")) {
				alert(mobile_comm_getDic("msg_apv_ApvLineMoveCheck2")); //해당 사용자는 최초 결재자이므로 이동 불가합니다.
				return false;
			} else if(p_num == last1 && pMode == "next") {
				alert(mobile_comm_getDic("msg_apv_ApvLineMoveCheck3")); //해당 사용자는 최종 결재자이므로 이동 불가합니다.
				return false;
			} else {
				var temp_num = Number(p_num);
				var curr_person = JSON.parse(JSON.stringify(ou.person[temp_num]));
				
				if(pMode == "prev") {
					target_person = JSON.parse(JSON.stringify(ou.person[temp_num-1]));
					ou.person[temp_num] = target_person;
					ou.person[temp_num-1] = curr_person;
				} else if(pMode == "next") { 
					target_person = JSON.parse(JSON.stringify(ou.person[temp_num+1]));
					ou.person[temp_num] = target_person;
					ou.person[temp_num+1] = curr_person;
			...
			// 이동 가능 여부 체크
			if(target_person.taskinfo.status == "pending" || target_person.taskinfo.kind == "charge" || target_person.taskinfo.datecompleted) {
				return false;
			}
		}
	} else {
	...
	if(l_ActivePageObj.ApvLineMode == "approval") {
		...
		$(userinfos).each(function(i, userinfo) {
			...
			if(userinfo.itemType == "user") {
				...
				//중복결재자 체크
				if(getInfo("SchemaContext.scChkDuplicateApv.isUse") == "Y") { //중복결재자
					bDelete = "false";
					$(division.step).each(function(index,item){
					    if(division.step.length==undefined){
					    	if(division.step.unittype=="person"&&division.step.ou.person.code == userinfo.AN){
					    		bDelete = "true";
					    		bDeleteCC = "true";
					    		return false;
					    	}
					    }else{
					    	if(division.step[index].unittype=="person"&&division.step[index].ou.person.code == userinfo.AN){
					    		bDelete = "true";
					    		bDeleteCC = "true";
					    		return false;
					    	}
				...
				if(new_step.ou.person.consultation != undefined) 
					delete new_step.ou.person.consultation;
				
				if(new_step.ou.person.taskinfo == undefined)
				...
				if(new_step.ou.person.taskinfo.consultcomment != undefined)
					delete new_step.ou.person.taskinfo.consultcomment;
			...
			if(division.step.length == undefined) {
				...
				var temp_ou = $$(division).has(">taskinfo[status='pending']").find("step[unittype='ou'] > ou").has("taskinfo[status='pending'][piid='" + getInfo("ProcessInfo.ProcessID").toUpperCase() + "']").json();
				var temp_array = [];
				$(temp_ou.person).each(function(){
					temp_array.push(this);
				});
				temp_array.push(new_step.ou.person);
				temp_ou.person = temp_array;	
			} else {
		...
		if (bDeleteCC=="true") alert(mobile_comm_getDic("msg_apv_186")); //"결재자가 이미 지정되어 있습니다."
		mobile_approval_getApprovalLine("approval");
...
function mobile_approval_delApvUser(obj) {
	...
	if(_mobile_approval_write.ApvLineMode == "approval") {
		...
		if(getInfo("Request.mode") == "SUBREDRAFT" || getInfo("Request.mode") == "SUBAPPROVAL") {
			s_num = $$(division).find("step").has("taskinfo[status='pending'][piid='" + getInfo("ProcessInfo.ProcessID").toUpperCase() + "']").index();
			
			if(s_num > 0) {
				var step = division.step[s_num] == undefined ? division.step : division.step[s_num];
				var o_num = $$(division).find("step > ou").has("taskinfo[status='pending'][piid='" + getInfo("ProcessInfo.ProcessID").toUpperCase() + "']").index(); //ou
				var p_num = $(remove_li).attr("id").replace("approval_write_ApvList", "").split("_")[3]; //person
				
				var ou = step.ou[o_num] == undefined ? step.ou : step.ou[o_num];
				
				ou.person.splice(p_num, 1);
				$("#APVLIST").val(JSON.stringify(apvlist));
			}
		} else {
			var step = division.step[s_num] == undefined ? division.step : division.step[s_num];
...
function mobile_approval_setPreviewIframe() {
	...
	document.IframeFrom.target = "Iframe";
  	...
  	var url = "/approval/mobile/approval/preview.do?" + $(document.IframeFrom).serialize();
...
function mobile_approval_write_clickTab(obj, target, isPageLoad) {
	...
	//if((value != "approval_write_div_formSelect" && _mobile_approval_write.FormID != "") || value == "approval_write_div_formSelect") {
		...
		if(value == "approval_write_div_detailItem" || value == "approval_write_div_approvalLine" || value == "approval_write_div_preview") {
			...
			if(value == "approval_write_div_detailItem") {
				XFORM.multirow.eventInit();
			}
...
function mobile_approval_chkUsePasswordYN(){
	var returnval = false;
	
	$.ajax({
		url:"/approval/mobile/approval/chkUsePasswordYN.do",
		type:"post",
		data: {
			"ur_code" : mobile_comm_getSession("USERID"),
		},
		...
		error:function(response, status, error){
			CFN_ErrorAjax("/approval/mobile/approval/chkUsePasswordYN.do", response, status, error);
		}
	});
	return returnval;
}
...
function mobile_approval_batchApproval(mode){
	...
	var msgWarning = '';
	var l_ActiveModule = mobile_approval_getActiveModule();
	var list_divID = l_ActiveModule + ((l_ActiveModule == "approval" && _mobile_approval_list.ListType == "search") ? "_list_searchlist" : "_list_list");
	...
	$.each($("#" + list_divID).find("input[type=checkbox]:checked"), function(idx,obj){
		var item = $.parseJSON(obj.value)
		if(typeof item.ExtInfo == 'undefined'  || item.ExtInfo.UseBlocApprove != 'Y' || (item.FormSubKind == "T009" && getApvAgrOpt == "Y")){
			excludeItem = true;
			$("#"+obj.id).prop("checked", false).checkboxradio('refresh');
		} else if(typeof item.ExtInfo !== 'undefined' && item.ExtInfo.UseBlocApprove === "Y" && (item.FormSubKind === "R" || item.FormSubKind === "T008")){
			// 수신부서 1인결재 옵션 체크
			if(typeof item.SchemaContext !== 'undefined' && item.SchemaContext.scChrRedraft.isUse !== "Y"){
				recItemChk = true;
				$("#"+obj.id).prop("checked", false).checkboxradio('refresh');
			}
		}
	...
	var checked = $("#" + list_divID).find("input[type=checkbox]:checked");
	...
	if(checkApprovalList.length == 0){
		...
		if(confirm(msgConfirm)){				// 처리 하시겠습니까?
			// 결재 비밀번호 사용여부
			if(mobile_approval_chkUsePasswordYN()){
					...
					if(res.length > 0){
						...
						$(res).each(function(i, obj){
							...
							if($$(oCurrentPersonNode).find(">taskinfo").attr("result") === "consultation"){
								alert(mobile_comm_getDic("lbl_apv_notBatchApvConsultingDoc")); // 검토 요청 중인 문서는 일괄 결재 대상에서 제외됩니다.
								...
								alert(mobile_comm_getDic("lbl_apv_notBatchApvConsultDoc")); //"검토 문서는 일괄 결재 대상에서 제외됩니다."
								approvalList = approvalList.filter(function(item) { if(item.ProcessID !== $$(obj).attr("ProcessID")) return this; })
							} else {
	...
	$(approvalList).each(function(i, obj){
		...
		//setTimeout(function(){
			$.ajax({
				...
				error:function(response, status, error){
					mobile_comm_ajaxerror("/approval/draft.do", response, status, error);
				}
			});
		//}, 500);
	});
...
//읽음 확인
function mobile_approval_doDocRead(pMode) {
	var l_ActiveModule = mobile_approval_getActiveModule();
	var checkApprovalList = []; 
	var list_divID = l_ActiveModule + ((l_ActiveModule == "approval" && _mobile_approval_list.ListType == "search") ? "_list_searchlist" : "_list_list");
	var checked = $("#" + list_divID).find("input[type=checkbox]:checked");
	...
	if (checkApprovalList.length == 0) {
		...
		if (confirmResult) {
			...
			if (paramArr.length > 0) {
				...
				var url = "/approval/mobile/approval/docRead.do";
				if(pMode.toUpperCase() == "DEPTTCINFO" || pMode.toUpperCase() == "RECEIVE") {
					url = "/approval/mobile/approval/deptDocRead.do";
				}
...
function mobile_approval_doOK_exec() {
	...
	var _usePWDCheck = getInfo("SchemaContext.scWFPwd.isUse");
	if(pageObj.ActionType === "cAgree" || pageObj.ActionType === "cDisagree" || pageObj.ActionType === "ConsultReq" || pageObj.ActionType === "ConsultReqCancel" || pageObj.ActionType === "ReConsultReq"){
		_usePWDCheck = "N";
	}
	if (_usePWDCheck === "Y" && mobile_approval_chkUsePasswordYN()){
	...
	//검토요청 및 취소에 대한 의견창 및 버튼 제어
	var returnVal = true;
	if(pageObj.ActionType == "ConsultReq" || pageObj.ActionType == "btReConsultReq"){
		var pStrItemInfo = '{"item":[' + window.sessionStorage["userinfo"] + ']}';
		var oJSON = $.parseJSON(pStrItemInfo);
		...
		if(oJSON.item.length > 0){
			...
			var confirmMsg = Common.getDic("msg_requestConsultToUsers").replace("{0}", chargers).replace("\\n", "\n"); //"다음 결재자들에게 검토 요청을 하시겠습니까?\n[{0}]".replace("{0}", chargers);
			if(!confirm(confirmMsg)) returnVal = false;
		}else{
			alert(Common.getDic("msg_apv_noSelectedConsultUsers"));//"검토를 요청할 사용자가 선택되지 않았습니다."
			returnVal = false;
		}
	}else if(pageObj.ActionType == "ConsultReqCancel"){
	...
	if ((pageObj.ActionType == "reject"
			...
			|| pageObj.ActionType == "rejectedto" || pageObj.ActionType == "rejectlast" || pageObj.ActionType === "cDisagree")
	...
	sessionObj = mobile_comm_getSession(); //전체호출
		...
		$.ajax({
			...
			success:function (data) {``
				...
				var html = "";
				var sBackStoragePath =  mobile_comm_getBaseConfig("BackStorage").replace("{0}", mobile_comm_getSession("DN_Code"));
				var src;
				 $(data.list).each(function(index){
				    ...
				    if(this.FileID && this.FileID > 0){
				    	src = mobile_comm_getImgById("ApprovalSign", this.FileID);
				    } else if(this.FilePath.indexOf(sBackStoragePath) > -1){
				    	src = mobile_comm_getImgById(this.FilePath);
				    } else {
				    	src = mobile_comm_getImgById(sBackStoragePath + this.FilePath);
				    }
					 ...
					 if (this.IsUse == "Y") {
						 html = "<div class='signature_viewbox use'>"
							 ...
							 + "<img src='"+src+"' alt=\"\" ></span><span class='inuse'>"+ mobile_comm_getDic("approval_inUse") /*사용 중*/ +"</span></a>"
