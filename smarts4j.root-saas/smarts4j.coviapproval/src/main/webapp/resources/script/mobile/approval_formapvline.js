...
function __displayApvList(oApvList) {
	...
	if(boolDisplayAudit){
        ...
        if ($$_elmList_audit.length > 0) {
            try { __tdo.tables.AuditLine.push(__getRequestApvList($$_elmList_audit, $$_elmVisible_audit, "", false, mobile_comm_getDic("lbl_apv_person_audit3"), false, "AuditLine")); } catch (e) { }
        }
        //개인준법
        if ($$_elmList_audit_law.length > 0) {
            try { __tdo.tables.AuditLine.push(__getRequestApvList($$_elmList_audit_law, $$_elmVisible_audit_law, "", false, mobile_comm_getDic("lbl_apv_person_audit"), false, "AuditLine")); } catch (e) { }
        }
        //부서감사
        if ($$_elmList_audit_dept.length > 0) {
            try { __tdo.tables.AuditLine.push(__getRequestApvList($$_elmList_audit_dept, $$_elmVisible_audit_dept, "", false, mobile_comm_getDic("lbl_apv_dept_audit3"), false, "AuditLine")); } catch (e) { }
        }
        else {
            if ($$_elmRoot.find("division").concat().eq(0).find("> step[routetype='audit'][name='audit_dept'] > ou").length != 0) {
				try { __tdo.tables.AuditLine.push(__initApvListCols(mobile_comm_getDic("lbl_apv_dept_audit3"), CFN_GetDicInfo($$_elmRoot.find("division").concat().eq(0).find("> step[routetype='audit'][name='audit_dept'] > ou").attr("name")), "AuditLine")); } catch (e) { }
            }
        }
        //부서준법
        if ($$_elmList_audit_law_dept.length > 0) {
            try { __tdo.tables.AuditLine.push(__getRequestApvList($$_elmList_audit_law_dept, $$_elmVisible_audit_law_dept, "", false, mobile_comm_getDic("lbl_apv_dept_audit"), false, "AuditLine")); } catch (e) { }
        }
        else {
			if ($$_elmRoot.find("division").concat().eq(0).find("> step[routetype='audit'][name='audit_law_dept'] > ou").length != 0) {
				try { __tdo.tables.AuditLine.push(__initApvListCols(mobile_comm_getDic("lbl_apv_dept_audit"), CFN_GetDicInfo($$_elmRoot.find("division").concat().eq(0).find("> step[routetype='audit'][name='audit_law_dept'] > ou").attr("name")), "AuditLine")); } catch (e) { }
            }
        }
...
function __displayApvListCols(oApvList) {
    ...
    var l_Dic = mobile_comm_getDicAll(["lbl_type","lbl_apv_username","lbl_dept","lbl_Approved","lbl_apv_date","lbl_apv_comment"]);
                                ...
                                if (document.getElementById("RApvLine" + i) != null) {
                                    ...
                                    if (document.getElementById("RApvLine") != null) {
                                        ...
                                        if ($$_elmList.valLength() > 0) {
                                            ...
                                            document.getElementById("RApvLine").innerHTML = szPreRApvLine + ((szPreRApvLine == "") ? "" : "<p style='margin:0px; padding:1px;' />") + initApvListCols(mobile_comm_getDic("btn_apv_managedept"), mobile_comm_getDicInfo(sOUName));		 //m_oFormMenu.gLabel_managedept, mobile_comm_getDicInfo(sOUName));
                                        }
    ...
    if ($$_elmListCount != 0) {
        ...
        $$_elmOPList.concat().each(function (index, $$_element) {
            ...
            if ($$_element.nodeName == "role") {
                ...
                sTitle = mobile_comm_getDicInfo($$_element.attr("name")) + (g_DisplayJobType && (" " + getLngLabel($$_element.attr(g_DisplayJobType), true)));
            }
