...
XFORM.form = (function(){
        ...
        $el.unbind('dblclick').bind('dblclick', function (event) {
            if (g_select_mode == 'element' || g_select_mode == 'container') {
                ...
                if ($(this).find("input").prop('type') != null && ($(this).find("input").prop('type').toLowerCase() == 'radio' || $(this).find("input[type=radio]").length > 0) && xformRdo.length > 0) {
                	...
                	var chkRadioWrapper = $(this).closest('.xform-checkbox, .xform-radio');
                    if ($(this).is(':checkbox') && chkRadioWrapper.attr('class').indexOf("xform-checkbox") > -1) {
                        event.stopPropagation();
                        g_selected_object = chkRadioWrapper; // input 객체 클릭하여 속성 저장 시, 라벨에 html이 추가되어 수정
                        elementModal(xformChk);
                    } else if ($(this).is(':radio') && chkRadioWrapper.attr('class').indexOf("xform-radio") > -1) {
                        event.stopPropagation();
                        g_selected_object = chkRadioWrapper;
                        elementModal(xformRdo);
                    } else {
                        event.stopPropagation();
                        elementModal();
    ...
    elementModal = function (el) {
        ...
        if ($el.is(':checkbox, :radio') && $el.closest('.xform-checkbox, .xform-radio').length == 1) { // [16-04-26] kimhs, 체크박스, 라디오의 경우 input 객체를 감싸고 있는 Label의 속성창 띄워주기
            $el = $el.closest('.xform-checkbox, .xform-radio').eq(0);
        }
    ...
    makeRadioHtml = function (el, n){
        ...
        if (g_selected_object.attr("data-type") == 'mField') {
            ...
            for (var i = 0; i < $el.find('.selectOption').length; i++) {
                ...
                html_opt += '<label data-type="mField" data-element-type="rdo_d" data-node-name="' + $n.attr('data-node-name') + '" style="padding-left:2px;padding-right:2px;">';
            ...
            for (var i = 0; i < $el.find('.selectOption').length; i++) {
                ...
                html_opt += '<label data-type="rField" data-element-type="rdo_d" data-node-name="' + $n.attr('data-node-name') + '" style="padding-left:2px;padding-right:2px;">';
                html_opt += '\n<input type="radio"';
    ...
    makeCheckHtml = function (el, n) {
        ...
        if (g_selected_object.attr("data-type") == 'mField') {
	        ...
	        for (var i = 0; i < $el.find('.selectOption').length; i++) {
	            ...
	            html_opt += '<label data-type="mField" data-element-type="chk_d" data-node-name="' + $n.attr('data-node-name') + '" style="padding-left:2px;padding-right:2px;">';
	            html_opt += '<input type="checkbox"';
	            var seq = makeId();
	            html_opt += ' id="XFORM_ELID_' + seq + '"';
	            html_opt += ' name="' + $n.attr('data-node-name') + '" value="' + $val + '" data-text="' + $key + '">' + $key;
	            html_opt += '</label>';
	        }
        } else if (g_selected_object.attr("data-type") == 'rField') {
            for (var i = 0; i < $el.find('.selectOption').length; i++) {
                ...
                html_opt += '<label data-type="rField" data-element-type="chk_d" data-node-name="' + $n.attr('data-node-name') + '" style="padding-left:2px;padding-right:2px;">';
                html_opt += '\n<input type="checkbox"';
                var seq = makeId();
                html_opt += ' name="' + ($n.find('input').eq(i).attr('name') == '' ? 'XFORM_ELID_' + seq : $n.find('input').eq(i).attr('name')) + '" value="' + $val.text() + '" data-text="' + $key.text() + '" data-face-for="' + $n.attr('data-node-name') + '" >' + $key.text();
                html_opt += '</label>';
    ...
    element_click = function (el) {
        ...
        } else if (g_selected_object.is(':radio, :checkbox, label') && g_selected_object.closest('.xform-radio, .xform-checkbox').length == 1) {
        	// [16-05-18] kimhs, checkbox or radio 선택할 경우 input tag의 parent를 가지고 있게 함, 이후 copy&paste를 위함
        	g_selected_object = g_selected_object.closest('.xform-radio, .xform-checkbox');
        	g_selected_object.addClass('group-focus');   //element
        } else {
    ...
    showAttr = function(el) {
        ...
        if(typeof el === 'undefined'){
            ...
            if (el.is(':checkbox, :radio') && el.closest('.xform-checkbox, .xform-radio').length == 1) { // [16-04-26] kimhs, 체크박스, 라디오의 경우 input 객체를 감싸고 있는 Label의 속성 보여주기
                el = el.closest('.xform-checkbox, .xform-radio').eq(0);
            }
    ...
    objectDropped = function (o) {
        ...
        if(o.prop("tagName").toLowerCase() == "table") {
        	XFORM.form.tableResizable(o, "");
        }
        return;
    };
    ...
    getElementHtml = function (el_type) {        
        ...
        if( s_type == 'text'){
            ...
            //radio
            s = '<label class="xform-radio" data-type="mField" data-node-name="' + new_name + '">';
            s = s + '<label data-type="' + default_data_type + '" data-element-type="rdo_d" data-node-name="' + new_name + '" style="padding-left:2px;padding-right:2px;">';
            s = s + '	<input type="radio" name="' + new_name + '" value="y" data-text="Yes">Yes';
            s = s + '</label>';
            s = s + '<label data-type="' + default_data_type + '" data-element-type="rdo_d" data-node-name="' + new_name + '" style="padding-left:2px;padding-right:2px;">';
            s = s + '	<input type="radio" name="' + new_name + '" value="n" data-text="No">No';
            s = s + '</label>';
            s = s + '</label>';
        } else if (s_type == 'checkbox') {
            //radio
            s = '<span>Colors: </span>';
            s = s + '<label class="xform-checkbox" data-type="mField" data-node-name="' + new_name + '">';
            s = s + '<label data-type="mField" data-element-type="chk_d" data-node-name="' + new_name + '" style="padding-left:2px;padding-right:2px;">';
            s = s + '	<input type="checkbox" name="' + new_name + '" value="Red" data-text="Red">Red';
            s = s + '</label>';
            s = s + '<label data-type="mField" data-element-type="chk_d" data-node-name="' + new_name + '" style="padding-left:2px;padding-right:2px;">';
            s = s + '	<input type="checkbox" name="' + new_name + '" value="White" data-text="White">White';
            s = s + '</label>';
            s = s + '<label data-type="mField" data-element-type="chk_d" data-node-name="' + new_name + '" style="padding-left:2px;padding-right:2px;">';
            s = s + '	<input type="checkbox" name="' + new_name + '" value="Black" data-text="Black">Black';
            s = s + '</label>';
            s = s + '</label>';
    ...
    function renderElement(elems) {
        ...
        function setDefault(elem) {
            ...
            if (elem.is(':radio, :checkbox')) {
            	var chkRadioWrapper = $(elem).closest('.xform-checkbox, .xform-radio');
                if (chkRadioWrapper.length == 0 || chkRadioWrapper.attr('data-node-name') == undefined || chkRadioWrapper.attr('data-node-name') == '') {
                	chkRadioWrapper.attr('data-node-name', nm);

                    if (elem.attr('data-type') != 'rField') // rField가 아닐때만 id 생성
                        $(elem).attr("id", 'XFORM_ELID_' + makeId());
                }
                else {
                    $(elem).attr('name', chkRadioWrapper.attr('data-node-name'));
                }
            }
    ...
    tableResizable = function ($el, mode) {
    	var __onResize = function(e) {
			var tbl = e.currentTarget; // table
			setTimeout(function(){
				for(var i = 0; tbl.rows && i < tbl.rows[0].cells.length; i++){
					$(tbl.rows[0].cells[i]).width("");
				}
			}, 100);
    	};
    	if(mode == "ALL") {
    		var $__tables = $('#canvas table');
    		for(var i = 0; $__tables && $__tables.length > 0 && i < $__tables.length; i++) {
    			$($__tables[i]).colResizable({liveDrag:false, onResize:__onResize, partialRefresh:true, destroy:true});
    		}
    	}else{
    		if($el.length > 0 && $el[0].tagName.toLowerCase() != "table") return;
    		
    		$el.colResizable({liveDrag:false, onResize:__onResize, partialRefresh:true, destroy:true});
    	}
    ...
    loadStyleEditor = function (el) {
        ...
        $width_minus.unbind('mousedown').bind('mousedown', function () {
            ...
            w = w.replace('px', '').replace('em', '').replace('pt', '').replace('%', '');
            w = parseInt(w);
            w = (w > 0) ? w-1 : w;
            $slide_width.val(w);
            setWidth();
    ...
    var magic = {
        ...
        init : function(){
            ...
            this.bindEvent();
            this.stat = true;
            XFORM.form.multirow.stat = false; // multirow 기능과 팝업 함께 사용하도록 수정
        },
        bindEvent: function () {
            var that = this;
    ...
    var multirow = {
        ...
        init : function(){
			var html = '<div class="magic" style="overflow: auto; height: 570px"><div class="sh_kwrite snippet-wrap"><pre style="padding: 1em; line-height: 1.8em;">';
			html += '<li><span style="font-weight: bold; color: red">multi rows 기능을 사용하려면 반드시 아래 내용을 참고하시어 소스를 추가 하셔야 합니다.</span></li><br />';
			html += '<li>1. 좌측 화단의 "응용 객체 도구" 목록에서 <span style="font-weight: bold;">[multi rows]</span> 기능을 캔버스에 드래그&드랍 합니다.</li>';
			html += '<li>2. 우측 상단의 <span style="font-weight: bold;">[소스편집]</span> 체크박스를 체크 합니다.</li>';
			html += '<li>3. <span style="font-weight: bold;">[소스편집]</span> 을 체크 하면 우측에 소스를 직접 수정 할 수 있는 영역이 보여집니다. 상단 영역이 html 파일이고 하단 영영이 javascript 파일입니다.</li>';
			html += '<li>4. <span style="font-weight: bold;">상단 영역 html</span> 파일을 확인 해 보면 아래와 같은 소스가 자동으로 추가되어 있습니다. (자동으로 추가된 ID가 아래 예시와 같지 않을 수 있습니다.)</li>';
			html += '<ol class="snippet-num"><li>&lt;table class="multi-row-table table_10 tableStyle linePlus mt10 sortable" <span style="font-weight: bold; color: red">id="xformid_2"</span>&gt;</li></ol>';
			html += '<li>5. <span style="font-weight: bold;">"xformid_2"</span> 이란 ID값을 <span style="font-weight: bold;">"MULTI_TABLE_ID"</span> 으로 변경합니다. (중복되지 않은 임의의 값으로 변경해야 합니다. 예시로 "MULTI_TABLE_ID" 로 변경)</li>';
			html += '<ol class="snippet-num"><li>&lt;table class="multi-row-table table_10 tableStyle linePlus mt10 sortable" <span style="font-weight: bold; color: red">id="MULTI_TABLE_ID"</span>&gt;</li></ol>';
			//html += '<li>&lt;!-- html 파일에서 multi row table id와 js 파일의 multi row table id가 같아야 합니다. (아래 샘플 소스상에서의 MULTI_TABLE_ID 변경 필요)  --&gt;</li>';
			html += '<li>6. <span style="font-weight: bold;">하단 영역 javascript</span> 파일에서 <span style="font-weight: bold;">postRenderingForTemplate(), makeBodyContext()</span> 함수를 찾아서 붉은색 표시된 소스를 추가 합니다.</li>';
            html += '<ol class="snippet-num">';
			html += '<li>function <span style="font-weight: bold;">postRenderingForTemplate()</span> {</li>';
			html += '<li>    // 체크박스, radio 등 공통 후처리</li>';
			html += '<li>    postJobForDynamicCtrl();</li>';
			html += '<li></li>';
			html += '<li>    //읽기 모드 일 경우</li>';
			html += '<li>    if (getInfo("Request.templatemode") == "Read") {</li>';
			html += '<li>        $(\'*[data-mode="writeOnly"]\').each(function () {</li>';
			html += '<li>            $(this).hide();</li>';
			html += '<li>        });</li>';
			html += '<li></li>';
			html += '<li>        // 멀티로우처리</li>';
			html += '<li>        //&lt;!--loadMultiRow_Read--&gt;</li>';
			html += '<li>        <span style="font-weight: bold; color: red">if (JSON.stringify(formJson.BodyContext) != "{}" && formJson.BodyContext != undefined) {</span></li>';
			html += '<li>            <span style="font-weight: bold; color: red">XFORM.multirow.load(JSON.stringify(formJson.BodyContext.MULTI_TABLE_ID), \'json\', \'#MULTI_TABLE_ID\', \'R\');</span></li>';
			html += '<li>        <span style="font-weight: bold; color: red">}</span></li>';
			html += '<li>    }</li>';
			html += '<li>    else {</li>';
			html += '<li>        $(\'*[data-mode="readOnly"]\').each(function () {</li>';
			html += '<li>            $(this).hide();</li>';
			html += '<li>        });</li>';
			html += '<li></li>';
			html += '<li>        // 멀티로우처리</li>';
			html += '<li>        //&lt;!--loadMultiRow_Write--&gt;</li>';
			html += '<li>        <span style="font-weight: bold; color: red">if (JSON.stringify(formJson.BodyContext) != "{}" && JSON.stringify(formJson.BodyContext) != undefined) {</span></li>';
			html += '<li>            <span style="font-weight: bold; color: red">XFORM.multirow.load(JSON.stringify(formJson.BodyContext.MULTI_TABLE_ID), \'json\', \'#MULTI_TABLE_ID\', \'W\');</span></li>';
			html += '<li>        <span style="font-weight: bold; color: red">} else {</span></li>';
			html += '<li>            <span style="font-weight: bold; color: red">XFORM.multirow.load(\'\', \'json\', \'#MULTI_TABLE_ID\', \'W\', { minLength: 1 });</span></li>';
			html += '<li>        <span style="font-weight: bold; color: red">}</span></li>';
			html += '<li>    }</li>';
			html += '<li>}</li>';
			html += '<li>...</li>';
			html += '<li>...</li>';
			html += '<li>//본문 XML로 구성</li>';
			html += '<li>function <span style="font-weight: bold;">makeBodyContext()</span> {</li>';
			html += '<li>    var bodyContextObj = {};</li>';
			html += '<li>    bodyContextObj["BodyContext"] = getFields("mField");</li>';
			html += '<li>    <span style="font-weight: bold; color: red">$$(bodyContextObj["BodyContext"]).append(getMultiRowFields("MULTI_TABLE_ID", "rField"));</span></li>';
			html += '<li>    return bodyContextObj;</li>';
			html += '<li>}</li></ol>';
			html += '<li>7. 소스 추가 할 때 html 파일에서 작성한 table ID와 javascript 소스상에서의 ID가 반드시 같아야 합니다. (<span style="font-weight: bold; color: red">"MULTI_TABLE_ID"</span> 으로 작성된 소스 확인.)</li>';
			html += '</pre></div></div>';
			
			this.obj.container.css('width', '1000px');
			this.obj.content().empty()
                .append(html)
                .append('<div class="btn-fld" style="width: 450px"><button class="cancel">닫기 »</button></div>');
            this.bindEvent();
            this.stat = true;
            XFORM.form.magic.stat = false;
        },
        bindEvent: function () {
            var that = this;
            var cancel_btn = that.obj.container.find('.cancel');
