...
XFORM.menu = (function () {
    ...
    bindMenuEvent = function () {
        ...
        //multi row button
        $('.element-multirow').bind('click', function () { XFORM.form.multirow.show(); });
        //create new
    ...
    saveTemplateFile = function (filename, jsfilename, form_content, js_form_content, callback) { //g_template_version
    	$.ajax({
            ...
            data: {
            	...
            	"htmlFileContent" : encodeURIComponent(form_content), // 평문으로 보내면 request.getParameter 에서 script 제거해버려서 인코딩 해서 던짐 (a tag onclick, img tag src..)
...
XFORM.template = (function () {
    ...
    loadTemplateList = function () {
        ...
        if ($('#hidFormList').val() != '') {
            ...
            var form_name, file_name, form_id, form_version, js_form_name;
            var tbl_row = '';
        ...
        $.ajax({
            ...
            success: function (data) {
                if(data.result=="ok" ){
	                ...
	                var l_formname = lang=="ko"?formname.split(";")[0]:lang=="en"?formname.split(";")[1]:lang=="ja"?formname.split(";")[2]:lang=="zh"?formname.split(";")[3]:formname.split(";")[0];
	                $('.xform-form-name').text(l_formname);
                }
            },
            error: function (error) {
            	//에러 처리 필요
    ...
    loadToCanvas = function (formname, fcontent, js_fcontent) {
        ...
        var $div = $('#canvas').find('div').not(".JCLRgrips,.JCLRgrip"); // [16-05-18] kimhs, 기존 children()을 사용하여 직계 자식만 가져옴, find()로 바꿔주어 하위 div도 sortable-container 클래스가 추가되도록 함
        ...
        $('#canvas').find('input:radio').each(function () {
            var $parent = $(this).parent().parent("label");
...
XFORM.loadOption = (function () {
    ...
    loadMode = function (mode) {
        ...
        if(mode == 'implicit' || mode == 'explicit' || mode == 'origin') {
        	XFORM.form.tableResizable(null, "ALL");
        }
    }
