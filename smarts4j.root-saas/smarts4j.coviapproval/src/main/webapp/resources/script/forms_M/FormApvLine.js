    ...
    if ($$_elmRoot.exist()) {
        ...
        $$_elmRoot.find("division").concat().each(function (i, $$_element) {
            ...
            if ( _sc('scPRec') == 'N' && _sc('scDRec') == 'N' && _sc('scIPub') == 'N')//일반결재
            {
				...
				__tabledata.tableHeader.apvKind = Common.getDic("lbl_apv_Approved"); //결재유형 - 담당, 결재, 합의, 감사...   
				__tabledata.tableHeader.name = Common.getDic("lbl_apv_username"); //이름
				__tabledata.tableHeader.dept = Common.getDic("lbl_dept"); //부서
				__tabledata.tableHeader.signImage = Common.getDic("lbl_sign"); //서명이미지 - 서명 이미지경로
				__tabledata.tableHeader.state = Common.getDic("lbl_ApprovalState"); //결재상태 - 승인, 반려...          
                                ...
                                if (document.getElementById("RApvLine" + i) != null) {
                                    ...
                                    if (document.getElementById("RApvLine") != null) {
                                        ...
                                        if ($$_elmList.valLength() > 0) {
                                            ...
                                            document.getElementById("RApvLine").innerHTML = szPreRApvLine + ((szPreRApvLine == "") ? "" : "<p style='margin:0px; padding:1px;' />") + initApvListCols(Common.getDic("initApvListCols"), CFN_GetDicInfo(sOUName));		 //m_oFormMenu.gLabel_managedept, CFN_GetDicInfo(sOUName));
                                        }
    ...
    $$_elmList.concat().each(function (i, $$_elm) {
        ...
        if ($$_elmTaskInfo.attr("kind") != 'conveyance' && $$_elmTaskInfo.attr("kind") != 'bypass') {
            ...
            if ((bDisplayCharge && i == 0) || ($$_elmTaskInfo.attr("visible") != "n")) //결재선 숨기기한 사람 숨기기
            {
                ...
                switch ($$_elmTaskInfo.attr("kind")) {
                    case "authorize":
                        ApvSignLines += (strDate == "") ? CFN_GetDicInfo($$_elm.attr("name")) + "<br /><font size='2'>(" + Common.getDic("lbl_apv_authorize") + ")</font>" : ((bRejected) ? interpretResult($$_elmTaskInfo.attr("result"), "", $$_elmTaskInfo.attr("kind")) : getSignUrl(sCode, $$_elmTaskInfo.attr("customattribute1"), $$_elm.attr("name"), strDate, true, $$_elmTaskInfo.attr("result"), true));
                    ...
                    case "skip":
                        ...
                        break;
                    case "bypass":
                        ApvSignLines += (strDate == "") ? CFN_GetDicInfo($$_elm.attr("name")) + "<br /><font size='2'>(" + Common.getDic("lbl_apv_bypass") + ")</font>" : getSignUrl(sCode, $$_elmTaskInfo.attr("customattribute1"), $$_elm.attr("name"), strDate, true, $$_elmTaskInfo.attr("result"), true);
                        ...
                        break; //"후열"
                    case "review":
                        ApvSignLines += (strDate == "") ? CFN_GetDicInfo($$_elm.attr("name")) + "<br /><font size='2'>(" + Common.getDic("lbl_apv_review") + ")</font>" : getSignUrl(sCode, $$_elmTaskInfo.attr("customattribute1"), $$_elm.attr("name"), strDate, true, $$_elmTaskInfo.attr("result"), true);
...
function displayAssist($$_elmRoot) {
    ...
    if ($$_elmListCount != 0) {
        ...
        $$_elmOUList.each(function (index, $$_element) {
            ...
            /* ======================== 부서합의 결재자 모두보여주기 s ======================== */
            let $$_elmTaskInfo = $$_element.find("taskinfo");
