...
function initOnloadformmenu() {
    ...
    if (getInfo("Request.reuse") == "Y" || getInfo("Request.reuse") == "P" || getInfo("Request.reuse") == "YH") {
        ...
        document.getElementById("btReUse").style.display = "none"; document.getElementById("btPrint").style.display = "none"; document.getElementById("btCommentView").style.display = "none"; /*document.getElementById("li_btCommentView").style.display = "none";*/ /*추가기능 버튼 비사용 document.getElementById("btExt").style.display = "none";*/
        document.getElementById("btCirculate").style.display = "none"; document.getElementById("btCirculate_View").style.display = "none"; /*document.getElementById("li_btCirculate_View").style.display = "none";*/ document.getElementById("btHistory").style.display = "none"; /*document.getElementById("li_btHistory").style.display = "none";*/ document.getElementById("btReceiptView").style.display = "none"; /*비사용 document.getElementById("bt_receive_cc").style.display = "none";*/
        document.getElementById("btPcSave").style.display = "none"; document.getElementById("btPrintView").style.display = "none"; document.getElementById("btMailSend").style.display = "none";
        ...
        if (g_szEditable) {
            document.getElementById("btDocLinked").style.display = ""; /*document.getElementById("btDocLink").style.display = "";*/ document.getElementById("btPreView").style.display = ""; 
        }
...
function initBtn() {
    ...
    switch (getInfo("Request.loct")) {
        ...
        case "COMPLETE":
            if (getInfo("Request.loct") == "COMPLETE" && getInfo("Request.subkind") != "T010") { /*document.getElementById("li_btCirculate").style.display = "";*/ document.getElementById("btCirculate").style.display = ""; document.getElementById("btCirculate_View").style.display = ""; /*document.getElementById("li_btCirculate_View").style.display = "";*/ }
            ...
            /*else if (getInfo("FormInstanceInfo.InitiatorID") == getInfo("AppInfo.usid")) {
                if ('COMPLETE' == getInfo("Request.loct") && getInfo("Request.gloct") != "") {        //완료함에서만 재사용 버튼 보여주기
        ...
        case "REJECT":
            ...
            if ((getInfo("Request.loct") == "COMPLETE") && getInfo("Request.mode") == "COMPLETE") { if (getInfo("SchemaContext.scIPub.isUse") == "Y" || getInfo("SchemaContext.scOPub.isUse") == 'Y') { document.getElementById("btReceiptView").style.display = ""; /*document.getElementById("li_btReceiptView").style.display = "";*/ } }

            //의견버튼 추가 : 완료된 문서에 기안자+결재자에 한해서 의견 추가
            if ((getInfo("Request.loct") == "COMPLETE" && 
            	 getInfo("Request.mode") == "COMPLETE") && 
            	 getInfo("SchemaContext.scCmtAdd.isUse") == "Y" && 
            	 (getInfo("Request.subkind") == "T000" || getInfo("Request.subkind") == "T004" || getInfo("Request.subkind") == "T006" || getInfo("Request.subkind") == "T009" || getInfo("Request.subkind") == "T016")) {document.getElementById("btComment").style.display = ""; document.getElementById("btCommentView").style.display = ""; /*document.getElementById("li_btCommentView").style.display = "";*/ }
        ...
        case "APPROVAL":
            switch (getInfo("Request.mode")) {
                ...
                case "APPROVAL": //일반결재
                    ...
                    // 스키마 체크, 배포 부서에서 편집 X
                    // 동시결재 시 스크립트 오류 발생하여 주석처리함(수신부서는 REDRAFT, RECAPPROVAL일 때 탐)
                    //if (getInfo("SchemaContext.scCHBis.isUse") == "Y" && !(getInfo("ProcessInfo.ProcessName").indexOf("Basic") > -1 && $$(formJson.ApprovalLine).find("division").has("taskinfo[status=pending]").has("step>ou[taskid="+getInfo("ProcessInfo.TaskID")+"]").index() > 0)) {
    ...
    if (admintype == "ADMIN") {
        ...
        if ((getInfo("Request.loct") == "COMPLETE" && getInfo("Request.mode") == "COMPLETE") || (getInfo("Request.loct") == "REDRAFT")) { if (getInfo("SchemaContext.scIPub.isUse") == "Y" || getInfo("SchemaContext.scOPub.isUse") == 'Y') { document.getElementById("btReceiptView").style.display = ""; /*document.getElementById("li_btReceiptView").style.display = "";*/ } }
        ...
        /*문서삭제 버튼 비사용 document.getElementById("btDeleteDoc").style.display = "none"; */ //재사용
    }
    ...
    switch (getInfo("FormInfo.FormPrefix")) {
        ...
        default:
            ...
            if (m_sReqMode != "TEMPSAVE" && m_sReqMode != "APPROVE" && m_sReqMode != "DEPTDRAFT") {
                ...
                /*예약 비사용
                 * if (getInfo("Request.mode") == "DRAFT" && (getInfo("scReserved") || getInfo("scDistribute"))) {
                ...
                if (getInfo("SchemaContext.scDRec.isUse") == "Y" && (getInfo("Request.mode") == "DRAFT" || getInfo("Request.mode") == "TEMPSAVE" || getInfo("Request.mode") == "APPROVAL")) {
                    ...
                    if (elmOu.length == 0) {
                        if (confirm(Common.getDic("msg_apv_199"))) return checkForm((m_sReqMode == "TEMPSAVE" ? true : false)); //"경유부서 없이 진행하시겠습니까?"
                    } else {
                        //
                    }
                }
...
function setApvList() {//대결일 경우 처리
    try {
        ...
        if (getInfo("Request.mode") == "APPROVAL" || getInfo("Request.mode") == "PCONSULT" || getInfo("Request.mode") == "RECAPPROVAL") { //기안부서결재선 및 수신부서 결재선
...
function callProcessEndScriptFunction(res, sReqMode) {
	...
	if (res.status == "SUCCESS" || (res.status == "FAIL" && res.message.indexOf("NOTASK")>-1)) {
		...
		disableBtns(false);
		if (CFN_GetQueryString("isMobile") == "Y" || _mobile) {
			alert(Common.getDic("msg_apv_030") + " : " + res.message);
		}else{
			Common.Error(Common.getDic("msg_apv_030") + " : " + res.message); //오류가 발생했습니다.
		}
	}
}
...
function requestProcessReserve(){
    ...
    $$(apvListTemp).find("step>ou").concat().find("taskinfo[result=pending]").concat().each(function(i, elm){
    	if(getInfo("Request.userCode") == $$(elm).parent().attr("code")){
    		...
    		$$(elm).attr("datecompleted", getInfo("AppInfo.svdt"));
    ...
    try {
        ...
        switch (sReqMode) {
            ...
            case "DOCLISTSAVE": //문서대장 등록-미구현
                XFN_FileSave();
                sAddage = getFormJSON() + getDocListJSON();
...
doButtonCase['btLine'] = function (oBtn, oVar) {//결재선관리
    ...
    if ((getInfo("Request.loct") == "MONITOR") || (getInfo("Request.loct") == "PREAPPROVAL") || (getInfo("Request.loct") == "PROCESS") || (getInfo("Request.loct") == "REVIEW") || (getInfo("Request.loct") == "COMPLETE") || (getInfo("Request.mode") == "REJECT") || (getInfo("Request.mode") == "JOBDUTY") || (getInfo("Request.mode") == "PCONSULT") || getInfo("Request.subkind") == "T019" || getInfo("Request.subkind") == "T005") { //20110318확인결재추가-확인결재자 결재선변경권한없음
...
doButtonCase['btForward'] = function (oBtn, oVar) {//문서전달
    ...
    if (openMode == "L" || openMode == "P") {
        Common.Confirm(Common.getDic("msg_apv_197"), "Confirmation Dialog", function (result) {		//gMessage197
            if (result) {
...
doButtonCase['btGovRecDept'] = function (oBtn, oVar) {//대외수신처관리
    /*oVar.iHeight = 643; oVar.iWidth = 1019;
    oVar.sUrl = "/WebSite/Approval/GovDocs/GovAddress/ApvDeployMgr.aspx?itemnum=999";*/
	alert("추후 개발 예정입니다.");
}
    	...
    	$.ajax({
    		...
    		success:function (res) {
    			if(res.status == "SUCCESS")
    				Common.Inform(Common.getDic("msg_apv_DeonModify"), "Information", function(){
    					Common.Close();
    					opener.CoviMenu_GetContent(opener.location.href.replace(opener.location.origin, ""),false);	//opener.location.reload();
    				});
    		},
        ...
        if (extdatatype == "writelink_d_text") {
            if (document.getElementById("ACTIONINDEX").value == 'approve') {
                ...
                $('*[ext-data-type1="writelink_d_text"]').each(function (i, item) {
                    ...
                    let strValue = $(this).text() == "" ? $(this).val() : $(this).text();
