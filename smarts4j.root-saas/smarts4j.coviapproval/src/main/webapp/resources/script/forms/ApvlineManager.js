        ...
        switch (sRT) {
            ...
            case "consult":
                switch (sUT) {
                    case "ou":
                        switch (sParentUT) {
                            case "ou": sSignType = (stepallottype=="parallel")? Common.getDic("lbl_apv_DeptConsent_2"):Common.getDic("lbl_apv_DeptConsent");  break;
                            case "role":
                            case "person": sSignType = convertKindToSignType(sKind, scustomattribute2); break;
                        } break;
                    case "role":
            ...
            case "assist":
                switch (sUT) {
                    case "ou":
                        switch (sParentUT) {
                            case "ou": sSignType = (stepallottype=="parallel")? Common.getDic("lbl_apv_DeptAssist2") :Common.getDic("lbl_apv_DeptAssist");  break;
                            case "role":
                            case "person": sSignType = convertKindToSignType(sKind, scustomattribute2); break;
                        } break;
                    case "role":
...
function initButtons() {
	if(m_sApvMode != "CFADMIN") {
	    ...
	    if(standalone_mode) {
        	...
        	var m_rectype = m_oFormEditor.$('[name=MULTI_RECEIVER_TYPE]').eq(idx).val();
	        	if(m_rectype == Sendtype){
	        		if(window.location.href.toUpperCase().indexOf("MULTIRECEIVELINE") > 0) { // 수신처 팝업에서는 자기것만 
        			m_RecDept = m_oFormEditor.$('[name=MULTI_RECEIVENAMES]').eq(idx).val();
	        		}
        ...
        //대외수신처 추가
        if (getInfo("SchemaContext.scDistribution.isUse")=="Y" && m_oFormEditor.document.getElementById("RECEIVEGOV_NAMES") != null){
        	m_DistDept = m_oFormEditor.document.getElementById("RECEIVEGOV_NAMES").value;
        	chkActionDist(0);
        }
		...
		$("#btnOpen").hide();
    	$("#btnClose").hide();
    	$("#divApvLineMgrSub").hide();
    	$(".sendRight").hide();
	}
	
    //디자인조정
    if ((getInfo("SchemaContext.scCC.isUse") != "Y"  && getInfo("SchemaContext.scIPub.isUse") != "Y") || m_sApvMode == "CFADMIN") {
        ...
        if (getInfo("SchemaContext.scDistribution.isUse")=="Y"){
        	$("#tabrecinfo").attr("style", "display:none;");//배포목록 버튼 숨김
            //$("#tabdistribution").attr("style", "display:;");
            //$("#dvDist").attr("style", "display:;");
        }
        ...
        if (getInfo("Request.mode") == "REDRAFT" || getInfo("Request.mode") == "RECAPPROVAL") {
          ...
          //대외수신처 추가
            if (getInfo("SchemaContext.scDistribution.isUse")=="Y"){
            	$("#tabrecinfo").attr("style", "display:none;");//배포목록 버튼 숨김
	            //$("#tabdistribution").attr("style", "display:;");
	            //$("#divdistribution").attr("style", "display:;");
	            //$("#dvDist").attr("style", "display:;");
            }
    ...
    if (admintype == 'ADMIN') {//관리자로 결재선관리 open 한 경우
        ...
        switch (m_sApvMode) {
            ...
            case "DEPTLIST":
                break;
            case "CFADMIN":
            	if (getInfo("SchemaContext.scPAgrSEQ.isUse") == "Y") $("#btPConsult").show();
                if (getInfo("SchemaContext.scPAgr.isUse") == "Y") $("#btPConsult2").show();
                if (getInfo("SchemaContext.scPCoo.isUse") == "Y") $("#btPAssist").show();
                if (getInfo("SchemaContext.scPCooPL.isUse") == "Y") $("#btPAssistPL").show();
            	break;
            case "TEMPSAVE" :
            case "DRAFT":
...
function getNextElm(elmCur) {
    ...
    return getSibling(elmCur, sLevel, sKeyAttribute, sKeyValue, true, false);
}
...
function deletePerson(obj) {
    ...
    if ($(oSelTR.childNodes[division_divisiontype]).text() == "receive" && $(oSelTR.childNodes[step_routetype]).text() == "receive" 
    	&& ($(oSelTR.childNodes[step_unittype]).text() == "ou" || $(oSelTR.childNodes[step_unittype]).text() == "person")) {
...
function setCC(allottype,befortype) {
    ...
    if (getInfo("SchemaContext.scCC.isUse") === "Y" && (getInfo("SchemaContext.scCC.value") === "NN" || getInfo("SchemaContext.scCC.value").substring(0, 1) === "Y")) {
    	...
    	if($$_m_JsonExt.find("Items > item[itemType=user]").concat().length > 0)
    		Common.Warning(Common.getDic("msg_apv_notAddCCPerson")); //개인참조를 추가할 수 없는 양식입니다.
    }
    ...
    if (getInfo("SchemaContext.scCC.isUse") === "Y" && (getInfo("SchemaContext.scCC.value") == "NN" || getInfo("SchemaContext.scCC.value").substring(1) === "Y")) {
    	...
    	if($$_m_JsonExt.find("Items > item[itemType=group]").concat().length > 0)
    		Common.Warning(Common.getDic("msg_apv_notAddCCDept")); //부서참조를 추가할 수 없는 양식입니다.
    }
}
...
function refreshList() {
    ...
    var $$_elmList = $$_m_oApvList.find("steps > division").has("taskinfo[visible!='n']");
...
function refreshRule(isChanged, ruleItem) {
			...
			$("<div>",{ "class" : "nea_appline_wrap", "style" : "margin:auto;" })
				.append(
					$("<div>",{ "class" : "nea_appline_top" })
						.append($("<p>", { "class" : "nea_appline_tit" }).text(Common.getDic("apv_btn_rule")))
						.append($("<ul>", { "class" : "nea_appline_list" }))
						.append($("<a>", { "href" : "#", "class" : "nea_appline_btn", "id" : "btnChangeRuleApv" }).append($("<span>").text(Common.getDic("lbl_apv_change"))))
						.append($("<input>", { "type" : "hidden", "id" : "hidRuleItemInfo" }))
				)
				.append(
					$("<div>",{ "class" : "nea_appline_bot" })
						.append($("<p>", { "class" : "nea_appline_tit" }).text(Common.getDic("lbl_ApprovalLine")))
						.append($("<ul>", { "class" : "nea_appline_list" }))
				)
		);
	...
	*/
	...
	if ($("#searchdiv").find("SPAN[id='spanRuleApv']").length <= 0) {
		$("#searchdiv").append("<span id=\"spanRuleApv\"><input type=\"button\" class=\"smButton\" id=\"btnChangeRuleApv\" name=\"btnChangeRuleApv\" value=\"" + Common.getDic("apv_btn_rule") + "\" style=\"padding: 0px; width: 80px; height: 30px; margin-left: 5px;\" /></span>");
...
function setRuleApvLine(isChanged, ruleItem) {
		...
		pathArr.map( function(item, idx) { 
			...
			if(idx == pathArr.length-1) {
				if(ruleItem.MaxAmount && ruleItem.MaxAmount.split(".")[0] != "0") {
					$li.append($("<span>", { "class" : "nea_appline_span", "style" : "margin: 5px;" }).html("( ~" + CFN_AddComma(ruleItem.MaxAmount.split(".")[0]) + " " + Common.getDic("lbl_below") + ")"));
	...
	if($ccinfo.length > 0) {		
		...
		$(".nea_appline_bot").find("ul").append(
			$("<li>").append(
				$("<span>", { "class" : "nea_appline_span" }).html(Common.getDic("lbl_apv_cc") + "(" + ccinfoStr.substr(0, ccinfoStr.length -1) + ")")
			)
		);
	}
}
...
function moveUpDown(str) {
    ...
    var sel_table = $(oSelTR).closest('table').attr('id');
    if (oSelTR == null || sel_table == 'tblccinfo') {
        Common.Warning(Common.getDic("msg_apv_Line_UnSelect")); // 대상을 선택하세요.
        return false;
    }
...
function setApvList() {
	if (evaluateApvList()) {
        if (chkAuthorize) {
            if (!window.confirm(Common.getDic("msg_apv_chkAuthorize"))) {//전결자가 지정되어있는 경우 이후 결재자는 모두 결재안함으로 변경 됩니다.
                return;
            }
        }
        //감사/준법 1개 이상 체크
        ...
        if (chkAudit != "") {
            Common.Warning(Common.getDic("msg_apv_chkAudit").replace("~", chkAudit));
            return;
        }
        ...
        //대외수신처 관련 시작
        if (getInfo("SchemaContext.scDistribution.isUse") == "Y" && m_oFormEditor.document.getElementById("RECEIVEGOV_NAMES") != null){
        	m_oFormEditor.document.getElementById("RECEIVEGOV_NAMES").value = m_DistDept;
        	
        	var sReceiveGovInfo = "";
        	var receveCnt = m_DistDept.split(":")[0] == "0" ? 13 : 8;

            for (var i = 0; i < m_DistDept.split(";").length; i++) {
                if (m_DistDept.split(";")[i] != '') {
                    if (sReceiveGovInfo == "") {
                        sReceiveGovInfo = m_DistDept.split(";")[i].split(":")[receveCnt];
                    } else {
                        sReceiveGovInfo += ", " + m_DistDept.split(";")[i].split(":")[receveCnt];
                    }
                }
         ...
         if(sReceiveGovInfo != ""){
            m_oFormMenu.$("#DistReceiveLine").show();
            m_oFormMenu.$("#tblDistDocProperties").show();
            m_oFormMenu.$("#RECEIVEGOV_INFO").val(sReceiveGovInfo);
            m_oFormMenu.$("#RECEIVEGOV_INFO").text(sReceiveGovInfo);
            
            // m_oFormEditor.document.getElementById("RECEIVE_INFO").value = sReceiveGovInfo;
         }
      ...
      //대외수신처 관련 종료
        ...
        if (admintype == "ADMIN") {//관리자 일 경우 결재선 변경 함수 호출
            ...
            if (bChange) {
                ...
                if(m_oFormEditor.drawFormApvLinesAll){
            ...
            if ((sMode == "DRAFT") || (sMode == "TEMPSAVE") || (sMode == "REDRAFT") || (sMode == "SUBREDRAFT") || (sMode == "APPROVAL") || (sMode == "PROCESS") || (sMode == "RECAPPROVAL") || (sMode == "SUBAPPROVAL")) {  //2007.10 박동현 (sMode == "RECAPPROVAL") 추가 [2016-04-21] 김지현(sMode == "SUBAPPROVAL") --> 합의부서내결재 추가
                ...
                var sIsLastApprover = "false";

                if ((oPendingSteps.length == 1) && (oinActiveSteps.length == 0)) {
					sIsLastApprover = "true";
                } else {
					sIsLastApprover = "false";
                }
                ...
                if ((sMode == "REDRAFT") || (sMode == "SUBREDRAFT")) {
                	if (sMode == "SUBREDRAFT"){ 
                		...
                		if($$_m_oSubReOu.find("person").concat().length == 1) sIsLastApprover = "true";
                	} 
                	if(sIsLastApprover == "true") {
               	...
               	if(oLASTAPPROVER) oLASTAPPROVER.value = sIsLastApprover;
                //마지막 결재자인지 확인
            }
...
function makeApvLineXml() {
    ...
    $$(prev_apvlist).find('division').concat().each(function(i, $$division){
        ...
        if(processDescID && processDescID !=='') {
            var $$_new_division = $$_m_oApvList.find('division').concat().eq($$_m_oApvList.find('division').concat().length > 1 ? i : 0); //문서유통의 접수한 문서는 결재라인이 발신부서가 없어 division이 수신부서만 존재하는 경우 발생.
...
function makeApvLineRolePersonXml(oTr, $$_oOu) {
    ...
    // 검토
    if ($(oTr.childNodes[roleperson_consultation]).text() != "") {
    	$$_oRolePerson.append("consultation", JSON.parse($(oTr.childNodes[roleperson_consultation]).text()));
    }
...
function evaluateApvList() {
    ...
    //결재선 필수체크
    //부서수신함 일괄결재선 제외
    if(m_oInfoSrc.CFN_GetQueryString("mode")!="Receive"){
	    m_oInfoSrc.chkAutopopup = this;
	    if(m_oInfoSrc.chkAutoApprovalLine()) {
			m_oInfoSrc.chkAutopopup = "";
			return false;
		}
		m_oInfoSrc.chkAutopopup = "";
    }
    ...
    if( oLastAppSteps.length >0 && parseInt(oLastAppSteps.index()) == $$_m_oApvList.find("division>step").concat().length - 1 && getInfo("SchemaContext.scLastAssistDraft.isUse") != "Y" ){
    	Common.Warning(strMsg_RuleError);
    	return false;
    }
    
    // 개인합의 및 부서합의 체크 마지막 결재선 체크
    var oLastAppSteps = $$_m_oApvList.find("division>step").concat().has("[routetype=consult']").last();
    if( oLastAppSteps.length >0 && parseInt(oLastAppSteps.index()) == $$_m_oApvList.find("division>step").concat().length - 1  ){
    	Common.Warning(strMsg_RuleError);
    	return false;
    }
    
    var m_oApvList_SubRe;
    var $$_m_oApvList_SubRe;
    if (m_sApvMode == "SUBREDRAFT" || m_sApvMode == "SUBAPPROVAL") {
        m_oApvList_SubRe = jQuery.parseJSON(m_oFormMenu.document.getElementById("APVLIST").value);
        $$_m_oApvList_SubRe = $$(m_oApvList_SubRe);
    ...
    if ((getInfo("SchemaContext.scPAgr.isUse") == "Y" || getInfo("SchemaContext.scPAgrSEQ.isUse") == "Y" )&& (m_sApvMode == "DRAFT" || m_sApvMode == "TEMPSAVE" || m_sApvMode == "APPROVAL" || m_sApvMode == "PCONSULT")) {
        //$$_elmConsult = $$_elmRoot.find("division>step[routetype='consult'][unittype='person']");
		$$_elmConsult = $$_elmRoot.find("division[divisiontype='send']>step[routetype='consult'][unittype='person']");
    ...
    //부서합의(순차),부서합의(병렬)
    if (( getInfo("SchemaContext.scDAgr.isUse") == "Y" || getInfo("SchemaContext.scDAgrSEQ.isUse") == "Y" )&& (m_sApvMode == "DRAFT" || m_sApvMode == "TEMPSAVE" || m_sApvMode == "APPROVAL" || m_sApvMode == "PCONSULT")) {
    	if (!chkConsultAppLine($$_elmRoot)) return false;
    }
    
    //부서협조(순차),부서협조(병렬)
    if (( getInfo("SchemaContext.scDCoo.isUse") == "Y" || getInfo("SchemaContext.scDCooPL.isUse") == "Y" )&& (m_sApvMode == "DRAFT" || m_sApvMode == "TEMPSAVE" || m_sApvMode == "APPROVAL" || m_sApvMode == "PCONSULT")) {
    	if (!chkConsultAppLine($$_elmRoot)) return false;
    }
    
    if (getInfo("SchemaContext.scDAgr.isUse") == "Y" && (m_sApvMode == "DRAFT" || m_sApvMode == "TEMPSAVE" || m_sApvMode == "APPROVAL" || m_sApvMode == "PCONSULT")) {
        $$_elmConsult = $$_elmRoot.find("division[divisiontype='send']>step[routetype='consult'][unittype='ou']");
        if (!$$_elmConsult.exist()) {
...
function chkConsultAppLine($$_elmRoot) {
    ...
    $$_emlSteps.concat().each(function (i, $$_emlStep) {
        ...
        if (i == $$_emlSteps.valLength() - 1 && $$_emlStep.attr("routetype") == "assist" && getInfo("SchemaContext.scLastAssistDraft.isUse") != "Y" ) CurAssist = true;
    });
        ...
        case "person":
            if (m_sSelectedStepType == "ccinfo") {
                ...
                $("#tblApvLine").find("tr").each(function (i, itemTR) {
                	if($(itemTR.childNodes[roleperson_taskinfo_kind]).text() != "conveyance") {
...
//itemtaskinfo: $$jsoner
    ...
    var $$_nDivision, $$_nDivisionTaskinfo, $$_nStep, $$_nStepTaskinfo, $$_nOu, $$_nOuTaskinfo,
    	$$_nRolePerson, $$_nRolePersonTaskinfo, $$_nRolePersonTaskinfoComment, $$_nRolePersonTaskinfoCommentLast, $$_nRolePersonTaskinfoCommentFileInfo, $$_nRolePerson_Person, $$_nRolePerson, 
    	$$_nRolePersonConsultation;
    ...
    if ($$_nTempTaskinfo.parent().exist() && ($$_nTempTaskinfo.parent().nodename() == "person" || $$_nTempTaskinfo.parent().nodename() == "role")) {
        ...
        if ($$_nRolePerson.parent()!= null && $$_nRolePerson.parent().nodename() == "ou") {
            ...
            if ($$_nOu.parent() != null && $$_nOu.parent().nodename() == "step") {
                ...
                if($$_nRolePerson.love("consultation").exist()){
                	$$_nRolePersonConsultation = $$_nRolePerson.love("consultation");
                }
            }
        }
    ...
    var arrRolePersonConsultation = [];
    if ($$_nRolePersonConsultation != null) {
    	arrRolePersonConsultation.push(($$_nRolePersonConsultation.json() == null) ? "" : JSON.stringify($$_nRolePersonConsultation.json()));
    }

	// nRolePerson_Person 조건 추가 (role - 부서장 결재단계 - taskinfo 이후에 person이 다시 있어서 이부분 유지)
    inserthtmlrow("tblApvLine", index, viewtype, displayonly, taskType, arrDivision, arrDivisionTaskinfo, arrStep, arrStepTaskinfo, arrOu, arrOuTaskinfo, arrRolePerson, arrRolePersonTaskinfo, arrRolePersonTaskinfoComment, arrRolePersonTaskinfoCommentLast, arrRolePersonTaskinfoCommentFileInfo, description, arrRolePerson_person, displayBypass, arrRolePersonConsultation);
}

// 결재자 목록 그릴 때 사용자 정보 그리는 부분
function getDisplayUserInfoHtml(name, ouName, level, title, position){
	var displayUserInfo = CFN_GetDicInfo(name, langCode);
    var displayJob = "";
    if(g_DisplayJobType){
	...
	displayJob = splitName(displayJob);
	displayUserInfo += "<br>"+ "(" + (displayJob ? displayJob + "," : "") + CFN_GetDicInfo(ouName,langCode) + ")&#160;";
...
function inserthtmlrow(objTB, index, viewtype, displayonly, taskType, arrDivision, arrDivisionTaskinfo, arrStep, arrStepTaskinfo, arrOu, arrOuTaskinfo, arrRolePerson, arrRolePersonTaskinfo, arrRolePersonTaskinfoComment, arrRolePersonTaskinfoCommentLast, arrRolePersonTaskinfoCommentFileInfo, description, arrRolePerson_person, displayBypass, arrRolePersonConsultation) {
    ...
    if (viewtype == "create" || viewtype == "change") {
        ...
        if (arrStep[2] == "parallel"	
            ...
            && ($(nTBodyRow[0].childNodes[step_unittype]).text() == "person" && arrStep[0] == "person"
            	|| $(nTBodyRow[0].childNodes[step_unittype]).text() == "ou" && arrStep[0] == "ou")
    ...
    var oCell_roleperson_consultation = oRow.insertCell(roleperson_consultation);
    ...
    oCell_roleperson_consultation.style.display = "none";
    ...
    if (arrRolePerson != null && arrRolePerson[0] != null) {
        ...
        oCell_dp_apv_username.innerHTML = getDisplayUserInfoHtml(arrRolePerson[2], arrRolePerson[7], arrRolePerson[5], arrRolePerson[4], arrRolePerson[3]);
    } else {
    ...
    if (arrRolePersonTaskinfo != null && arrRolePersonTaskinfo[0] != null) {
        ...
        if (taskType == "RolePerson") {
            if ($(oCell_roleperson_taskinfo_kind).text() == "charge" || $(oCell_roleperson_taskinfo_kind).text() == "skip") oCell_key_readonly.innerHTML = "True";
    ...
    if (arrRolePersonConsultation != null && arrRolePersonConsultation[0] != null) {
    	oCell_roleperson_consultation.innerHTML = arrRolePersonConsultation[0];
    }
	...
	//if ((oCell_key_readonly.innerHTML == "True") && 
		...
		if (oCell_step_ruleapvclass.innerHTML == "DRAFTER") {
	    	oCell_dp_apv_checkbox.innerHTML="<input type='checkbox' isDrafter='true' style='display: none;' />";
...
function inserthtmlrowCC(objTB,taskType, arrCCinfo, arrOu, arrPerson) {
    ...
    if (arrOu != null && arrOu[0] != null) {
        ...
        if (taskType == "Ou") {
            ...
            if (arrCCinfo[0] == "receiver") oCell_dp_cc_username.innerHTML = oCell_dp_cc_username.innerHTML + "- " + Common.getDic("lbl_apv_receive");
        }
    } else {
    ...
    if (arrPerson != null && arrPerson[0] != null) {
        ...
        if (taskType == "RolePerson" && arrCCinfo[0] == "receiver") {
            oCell_dp_cc_username.innerHTML = getDisplayUserInfoHtml(arrPerson[2], arrPerson[7], arrPerson[5], arrPerson[4], arrPerson[3]) + "- " + Common.getDic("lbl_apv_receive");
        } else if (taskType == "RolePerson") {
            oCell_dp_cc_username.innerHTML = getDisplayUserInfoHtml(arrPerson[2], arrPerson[7], arrPerson[5], arrPerson[4], arrPerson[3]);
        }
        ...
        if (oTempTR.tagName == "TR") {
            if (oTempTR.childNodes[roleperson_taskinfo_kind].innerHTML == "skip") {
        		...
        		if (oTempTR.childNodes[step_ruleapvclass].innerHTML == "DRAFTER") {
        			oTempTR.childNodes[dp_apv_checkbox].innerHTML="<input type='checkbox' isDrafter='true' style='display: none;'/>";
        	    	...
        	    	oTempTR.childNodes[dp_apv_checkbox].innerHTML="<input type='checkbox' isDrafter='false' style='display: none;'/>";
        	    }

        	    // 삭제버튼
        		oTempTR.childNodes[dp_apv_delete_button].innerHTML = "<a href='#' class='icnDel' onclick='deletePerson(this)'>"+Common.getDic("btn_apv_delete")+"</a>";
...
function splitName(sValue) {
    var rtnValue = "";
    if(typeof rtnValue == "string" && rtnValue){
    	rtnValue = sValue.substring(sValue.indexOf(";") + 1);
    	rtnValue = CFN_GetDicInfo(rtnValue,langCode);
    }
    return rtnValue;
}
...
function addDistribution() {
    ...
    var sSelectedUserJson = aDistContentAdd_OnClick();
    var $$_m_JsonExt = $$(sSelectedUserJson);
    //const BOOL_KEYNAME_INCLUDED = true;  //[IE 10 이하  const 사용 오류]
    var BOOL_KEYNAME_INCLUDED = true;				// 선언 이외의 곳에서 값 변경 X
    ...
    setDistributionDept($$_m_Json.find("selected"));
}
...
function delDistList(obj) {
    ...
    if (oSelTR != null) {
        if (oSelTR.id != null) {
            ...
            m_DistDept = sRecDept;
            if (m_DistDept.indexOf(";") == 0) m_DistDept = m_DistDept.substring(1);
        } else {
            if (oSelTR.type == "0") {
                Common.Warning(strMsg_187); //"배포리스트 구분을 조직도를 선택하세요."
            } else {
...
//대외수신처 추가 
function setDistributionDept($$_oList) {
    ...
    if (sRecDept != ""){
	    if(m_DistDept.substring(0,1) != m_DistDept.substring(1,2) || $("#govReceiveGubun").val() == "gov24") m_DistDept = sRecDept;
	    else m_DistDept += sRecDept;
...
//대외수신처 추가
function chkActionDist(actType) {
    if (m_sApvMode.toUpperCase() != "DEPTLIST" && m_sApvMode.toUpperCase() != "CFADMIN") {
        mType = actType;
...
//대외수신처 추가
function make_selRecDist() {
    var otbl = document.getElementById("tbldistinfo");
    var tbllength = otbl.rows.length;
    //Table 지우기
    for (var i = 0; i < tbllength - 2; i++) {
        otbl.deleteRow(tbllength - i - 1);
    ...
    var sRec = m_DistDept.split(";");
    if (m_DistDept == "") return;
    for (var i = 0; i < sRec.length; i++) {
        if (sRec[i] != "" && sRec[i] != null) {
            aRec = sRec[i].split(":");
            eTR = otbl.insertRow(otbl.rows.length);
            ...
            //삭제 버튼 추가
            eTD = eTR.insertCell(eTR.cells.length);
            eTD.innerHTML = "<a href='#' class='icnDel' onclick='delDistList(this)'>"+Common.getDic("btn_apv_delete")+"</a>";
        }
    }
    return;
}
...
function chkDuplicate(code) {
    ...
    if (arr != '') {
        ...
        if (cmpIndex < 0) { check = true; } else { check = false; }
    }
    return check;
}
...
function btnSearch_OnClick() {
    ...
    if (sSearchType == "") {
        Common.Warning(Common.getDic("msg_apv_OrgMap01"));  // 검색조건을 선택하여 주십시오.
        return;
    }
    $("#hidSearchType").val(sSearchType);
    ...
    if (sSearchText.length < 2) {
        Common.Warning(Common.getDic("msg_Common_07"));  // 검색어는 2글자 이상 입력하여 주십시오.
        return;
    }
...
//대외수신처 추가
function aDistContentAdd_OnClick() {
    function makeItem(oThis, arrItem, bTree){
        if (oThis.is(":checked")) {
            var item = oThis.val().replace(/&/g, "&amp;");
            arrItem.push(JSON.parse(item));
...
var strlable_Seq1 = Common.getDic("lbl_apv_assist");
var strlable_Par1 = Common.getDic("lbl_apv_assist_2");
...
var strlable_DeptConsent2 = Common.getDic("lbl_apv_DeptConsent_2");
var strlable_DAsist = Common.getDic("lbl_apv_DeptAssist");
var strlable_DAsist2 = Common.getDic("lbl_apv_DeptAssist2");
...
var roleperson_consultation = 77 + CON_PRE_TD_COUNT; //검토요청 정보
//결재선 정보 변수 선언 종료
