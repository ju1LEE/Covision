...
XFORM.multirow = (function () {
                ...
                if ($row_control.length > 0) {
                    $(this).unbind('click').bind('click', function () {
                        ...
                        $chk.each(function () {
                            ...
                            if (template_rows_num == 1) {
                            	if($checked_row.find('td.multi-row-autospan').length > 0){
                                    var tdComCnt = 0;
                                    $checked_row.find('td.multi-row-autospan').each(function(){
                                        var tdAutoCnt = 0;
	                                     if($(this).hasClass('multi-row-autospan')) tdAutoCnt = 1;
	                                     var tdindex = ($(this).index()-tdAutoCnt)+tdComCnt;
	                                     if (tdindex < 0) tdindex = 0;
                                        var vslice = $checked_row.nextAll().andSelf().slice(template_rows_num, template_rows_num+1).children().eq(tdindex);
                                        if (tdindex == 0 && $(this).index() == 0) vslice.before($(this));
                                        else vslice.after($(this));
                                        tdComCnt++;
                                    });
                                }
                            	$checked_row.remove();
    ...
    var eventInit = function (){
    	$(".multi-row-table").each(function(){
    		var $table = $(this);
        	rowControlButton.init($table, 'table');
    	});
    };
