...
var g_DisplayJobType = Common.getBaseConfig("ApprovalLine_DisplayJobType"); // 직책, 직급, 직위 중 결재선에 표시될 타입 설정 (직책 : title, 직급 : level, 직위 : position)
...
var ApvGraphicView = {
	          ...
	          if (steproutetype == "notify") {
	            ...
	            $$_ostep.find("ou").concat().each(function (index_ou, $$_oou) {
	              ...
	              if (assureouvisible) {
		            ...
		            if($$_oou.parent().attr('unittype')=="ou" && !$$_oou.find("person").exist() || $$_oou.parent().attr("allottype")!="parallel"  || $$_oou.parent().attr("allottype")!="serial"){
	                    substep['name'] = CFN_GetDicInfo(me.getDisplayName(displayname));
	                    substep['dept'] = CFN_GetDicInfo(me.getDisplayName(displayname));
	                    substep['position'] = '';
	                    substep['level'] = '';
	                }
  ...
  getSubStep: function($$_operson, $$_otaskinfo){
    ...
    substep['title'] = splitName(title);
    substep['position'] = splitName(position);
    substep['level'] = splitName(level);
  ...
  render: function($target, viewData, targetType){
    ...
    if(targetType != "account") {
	    ...
	    tmpHtml += '<span class="{{waitCircle}}"><img src="{{photo}}" alt="" style="max-width: 100%; height: auto;" onerror="coviCmn.imgError(this, true)"></span>'; //사진
	    tmpHtml += '<a class="state-icon {{stateIcon}}" href="#">{{state}}</a>'; //결재상태 아이콘
	    tmpHtml += '</dt>';
    }
    // 2022-11-11 플라워 네임 적용
    if (typeof (setUserFlowerName) == 'function') {
		tmpHtml += '<dd style="height:23px;">' + setUserFlowerName('{{code}}','{{name}}',"flowerPos") + '&nbsp;{{'+ g_DisplayJobType + '}}({{dept}}){{commentlink}}</dd>'; //이름
	} else {
		tmpHtml += '<dd style="height:23px;">{{name}}&nbsp;{{'+ g_DisplayJobType + '}}({{dept}}){{commentlink}}</dd>'; //이름
	}    
    ...
    function getStateIcon(step_kind, state){
      ...
      if(step_kind == Common.getDic("lbl_apv_DeptConsent") || step_kind == Common.getDic("lbl_apv_DeptConsent_2")
    		  || step_kind == Common.getDic("lbl_apv_DeptAssist") || step_kind == Common.getDic("lbl_apv_DeptAssist2")
    		  || step_kind == Common.getDic("btn_apv_consultors") || step_kind == Common.getDic("btn_apv_consultors_2")
    		  || step_kind == Common.getDic("lbl_apv_assist") || step_kind == Common.getDic("lbl_apv_assist_2") ){
        if(state==='yes'){
    ...
    function getRenderedHtml(divisions, tmpHtml){
      ...
      for(var i =0; i< divisions.length; i++){
        ...
        for(var j =0; j < steps.length; j++){
    	  ...
    	  if(step.kind == Common.getDic("lbl_apv_DeptConsent_2") || step.kind == Common.getDic("lbl_apv_DeptAssist2")
        		  || step.kind == Common.getDic("btn_apv_consultors_2") || step.kind == Common.getDic("lbl_apv_assist_2") 
    	  ...
    	  }else if(step.kind == Common.getDic("lbl_apv_DeptConsent") || step.kind == Common.getDic("lbl_apv_DeptAssist")
        		  || step.kind == Common.getDic("btn_apv_consultors") || step.kind == Common.getDic("lbl_apv_assist") 
				  || step.name == "audit_law_dept" || step.name == "audit_dept"){
  ...
  /*getPhotoPath: function (code, stepUnitType){
	  ...
	  if(ApvGraphicView.conf.debug) {
		  ...
		  if(stepUnitType === 'person' || stepUnitType === 'ou'){
			  photoPath = Common.getBaseConfig("ProfileImagePath").replace("{0}", Common.getSession("DN_Code"))+code+".jpg";
		  } else {
  ...
  getStepTitle: function (step){
    ...
    var namemap = {
      allottype: {
        serial : Common.getDic("lbl_apv_serial"), 						// 순차
        parallel : Common.getDic("lbl_apv_parallel") 						// 병렬
      },
      unittype: {
        ou : Common.getDic("lbl_apv_dept"), 								// 부서
        person: Common.getDic("lbl_apv_person") 							// 개인
      },
      routetype: {
    	approve : {
    		normal : Common.getDic("lbl_apv_normalapprove"),				// 일반결재
    		consent : Common.getDic("lbl_apv_investigation"),				// 검토
    		authorize : Common.getDic("lbl_apv_authorize"),				// 전결
    		substitute : Common.getDic("lbl_apv_substitue"),				// 대결
    		review : Common.getDic("lbl_apv_review"),						// 후결
    		bypass : Common.getDic("lbl_apv_bypass"),						// 후열
    		charge_send : Common.getDic("lbl_apv_Draft"),					// 기안
    		charge_receive : Common.getDic("lbl_apv_receive"),			// 수신
    		skip : Common.getDic("lbl_apv_NoApprvl"),						// 결재안함
    		confirm : Common.getDic("lbl_apv_Confirm"),					// 확인
    		reference : Common.getDic("lbl_apv_share4list"),				// 참조
    		conveyance : Common.getDic("lbl_apv_forward"),				// 전달
    		charge : Common.getDic("lbl_apv_charge")			// 수신
    	},
        consult : Common.getDic("lbl_apv_tit_consent"), 					// 합의
        receive : Common.getDic("lbl_apv_Acceptdept"), 					// 수신부서
        assist : Common.getDic("lbl_apv_reject_consent"), 				// 협조
        audit  : Common.getDic("lbl_apv_audit"), 							// 감사
        review  : Common.getDic("lbl_apv_confirmor"), 					// 공람
        
        // 부서
        serial_consult_ou : Common.getDic("lbl_apv_DeptConsent"), 		// 순차합의
        parallel_consult_ou : Common.getDic("lbl_apv_DeptConsent_2"), 	// 병렬합의
        serial_assist_ou : Common.getDic("lbl_apv_DeptAssist"), 			// 순차협조
        parallel_assist_ou : Common.getDic("lbl_apv_DeptAssist2"), 		// 병렬협조

        // 개인
        serial_consult_person : Common.getDic("btn_apv_consultors"), 		// 순차합의
        parallel_consult_person : Common.getDic("btn_apv_consultors_2"), 	// 병렬합의
        serial_assist_person : Common.getDic("lbl_apv_assist"), 			// 순차협조
        parallel_assist_person : Common.getDic("lbl_apv_assist_2") 		// 병렬협조
      },
	  name: {
		//감사,준법
        audit_dept : Common.getDic("lbl_apv_dept_audit3"), 		//부서감사
        audit_law_dept : Common.getDic("lbl_apv_dept_audit"), 	//부서준법
        audit : Common.getDic("lbl_apv_person_audit3"), 		//개인감사
        audit_law : Common.getDic("lbl_apv_person_audit"), 		//개인준법
	  }
    }
