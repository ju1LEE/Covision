...
<script>
	...
	function decDrmDocument() {
		...
		var list;
		$.ajax({
			url: '/approval/manage/decRequestApproval.do',
			processData: false,
			contentType: false,
			data: formData,
			type: 'POST',
			...
			if (confirmResult) {
				...
				$.ajax({
					...
					data:{
						...
						"entcode" : opener.$("#hidden_domain_val").val()
					},   
					url:"/approval/manage/excelUploadRuleManage.do",
					success:function (result) {		
						if(result.status=="SUCCESS"){
							list = result.data;
	...
	function excelDownLoad(){
		location.href = "excelRulManageDownload.do?entcode="+opener.$("#hidden_domain_val").val();
	}
</script>
