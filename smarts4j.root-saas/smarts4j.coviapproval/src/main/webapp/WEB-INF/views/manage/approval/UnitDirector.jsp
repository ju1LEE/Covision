...
<div class="cRConTop titType AtnTop">
	<h2 class="title">
		<span><spring:message code="Cache.lbl_targetDept"/></span> 
	</h2>
</div>
...
<script type="text/javascript">
		...
		var configObj = {
			...
			body:{
				 onclick: function(){
					     ...
					     updateAuthority(false,this.item.UnitCode);					    
					 }			
			}
		};
	...
	function searchConfig(){
		...
		myGrid.bindGrid({
 			ajaxUrl:"/approval/manage/getUnitDirectorList.do",
 			ajaxPars: {
	...
	function addAuthority(pModal){
		...
		objPopup = parent.Common.open("","addAuthority","<spring:message code='Cache.lbl_addAuthority'/>|||<spring:message code='Cache.lbl_apv_Specific_Unit_Manage'/>","/approval/manage/goUnitDirectorSetPopup.do?mode=add&paramEntCode="+paramEntCode,"700px","650px","iframe",pModal,null,null,true);
	}
	
	// 그리드 안의 설정키 클릭 했을 때 수정 레이어 팝업
	function updateAuthority(pModal, configkey){	
		var paramEntCode = $("#hidden_useAuthority_val").val();
		objPopup = parent.Common.open("","updateAuthority","<spring:message code='Cache.lbl_updateAuthority'/>|||<spring:message code='Cache.lbl_apv_Specific_Unit_Manage'/>","/approval/manage/goUnitDirectorSetPopup.do?mode=modify&paramEntCode="+paramEntCode+"&key="+configkey,"700px","650px","iframe",pModal,null,null,true);
	}
