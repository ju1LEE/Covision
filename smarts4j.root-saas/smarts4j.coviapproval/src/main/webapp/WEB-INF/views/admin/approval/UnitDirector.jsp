...
<h3 class="con_tit_box">
	<span class="con_tit"><spring:message code="Cache.lbl_targetDept"/></span>	
</h3>
<form id="form1">
...
<script type="text/javascript">
	...
	function setGrid(){
		...
		var headerData =[					
		                  ...
		                  {key:'UnitName', label:'<spring:message code="Cache.lbl_Specific_Dept"/>', width:'100', align:'center',
		                	  formatter:function () {
		      					return "<a onclick='updateAuthority(false, \""+ this.item.UnitCode +"\"); return false;'>"+this.item.UnitName+"</a>";
		      				}},
		                  {key:'TargetUnitName',  label:'<spring:message code="Cache.lbl_apv_Assignmented_Dept"/>', width:'400', align:'left'},
	...
	function searchConfig(){
		...
		myGrid.bindGrid({
 			ajaxUrl:"/approval/admin/getUnitDirectorList.do",
 			ajaxPars: {
	...
	function addAuthority(pModal){
		...
		objPopup = parent.Common.open("","addAuthority","<spring:message code='Cache.lbl_addAuthority'/>|||<spring:message code='Cache.lbl_apv_Specific_Unit_Manage'/>","/approval/admin/goUnitDirectorSetPopup.do?mode=add&paramEntCode="+paramEntCode,"660px","520px","iframe",pModal,null,null,true);
	}
	
	// 그리드 안의 설정키 클릭 했을 때 수정 레이어 팝업
	function updateAuthority(pModal, configkey){	
		var paramEntCode = $("#selectUseAuthority").val();
		objPopup = parent.Common.open("","updateAuthority","<spring:message code='Cache.lbl_updateAuthority'/>|||<spring:message code='Cache.lbl_apv_Specific_Unit_Manage'/>","/approval/admin/goUnitDirectorSetPopup.do?mode=modify&paramEntCode="+paramEntCode+"&key="+configkey,"660px","520px","iframe",pModal,null,null,true);
	}
