...
<script  type="text/javascript">
	...
	function addDataRow(pCode, pName, pViewStartDate, pViewEndDate) {
		if(pCode && pName) {
			...
			sHTML += "	<td style='text-align: center; word-break: break-all;'><input type='hidden' name='hidden_PERSON_NAME' id='hidden_PERSON_NAME_" + XFN_ReplaceAllSpecialChars(pCode) + "' value='" + pName + "'><label for='user_" + XFN_ReplaceAllSpecialChars(pCode) + "'>" + CFN_GetDicInfo(pName) + "</label></td>";
			sHTML += "	<td style='word-break: break-all;'><span>" + pCode + "</span><input type='hidden' name='hidden_PERSON_CODE' id='hidden_PERSON_CODE_" + XFN_ReplaceAllSpecialChars(pCode) + "' value='" + pCode + "'></td>";
		...
		$.ajax({
			...
			success:function (data) {
				if(data.cnt>0){
					...
					$("#AuthStartDate").val(getStringDateToString("yyyy-MM-dd",data.list[0].AuthStartDate));
					$("#AuthEndDate").val(getStringDateToString("yyyy-MM-dd",data.list[0].AuthEndDate));
					
					$("#MGRDEPTLIST").empty();

					for(var i=0; i<data.list.length;i++){
						addDataRow(data.list[i].TargetCode, data.list[i].TargetName, data.list[i].ViewStartDate, data.list[i].ViewEndDate);
					}
			...
			error:function(response, status, error){
				CFN_ErrorAjax("getPersonDirectorData.do", response, status, error);
			}
		});
	}
	...
	function saveSubmit(){
		...
		parent.Common.Confirm("<spring:message code='Cache.msg_apv_155' />", "Confirmation Dialog", function(result){
			if(!result){
				...
				var persondirectormemberArray = new Array();
				
				jQuery.ajaxSettings.traditional = true;
				
				$("#MGRDEPTLIST > tr").each(function(i, oRow){
					var persondirectormemberObj = new Object();
					
					persondirectormemberObj.UserCode = UserCode;
					persondirectormemberObj.TargetCode = $(oRow).find("input[name='hidden_PERSON_CODE']").val();
					persondirectormemberObj.TargetName = $(oRow).find("input[name='hidden_PERSON_NAME']").val();
					persondirectormemberObj.ViewStartDate = $(oRow).find("input[id^='SEARCH_START']").val();
					persondirectormemberObj.ViewEndDate = $(oRow).find("input[id^='SEARCH_END']").val();
					
					persondirectormemberArray.push(persondirectormemberObj);			
				});	
				
				//jsavaScript 객체를 JSON 객체로 변환
				persondirectormemberArray = JSON.stringify(persondirectormemberArray);		
				...
				$.ajax({
					...
					data:{
						...
						"persondirectormember" 	: persondirectormemberArray
					},
					url:"insertPersonDirector.do",
					success:function (data) {
						if(data.result == "ok") {
					...
					error:function(response, status, error){
						CFN_ErrorAjax("insertPersonDirector.do", response, status, error);
					}
				});
			}
		});
		...
		parent.Common.Confirm("<spring:message code='Cache.msg_AreYouDelete' />", "Confirmation Dialog", function(result){
			if(!result){
				...
				$.ajax({
					...
					url:"deletePersonDirector.do",
					success:function (data) {
						if(data.result == "ok")
							parent.Common.Inform("<spring:message code='Cache.msg_50' />");
						closeLayer();
						parent.setUseAuthority();
					},
					error:function(response, status, error){
						CFN_ErrorAjax("deletePersonDirector.do", response, status, error);
					}
				});
			}
		});
	...
	function setMGRDEPTLIST(peopleValue){		
		...
		if(dataObj.item.length > 0) {
			if(flag == 0){
				...
				paramUserCode = dataObj.item[0].AN;
...
<form id="PersonDirectorOfUnitSetPopup" name="PersonDirectorOfUnitSetPopup">
	<div>
		<p class="txt_nt_01"><span style="font-size: 12px;color: #ee9700;font-weight: normal;"><spring:message code="Cache.msg_apv_UserDocAuthorityScope"/></span></p>
		...
		<table class="AXFormTable">
			...
			<tr>
				...
				<td>
					<input type="text" id="AuthStartDate" style="width: 85px" class="AXInput" />
					~
					<input type="text" kind="twindate" date_startTargetID="AuthStartDate" id="AuthEndDate" style="width: 105px" class="AXInput" />
				</td>
			</tr>
			<tr>
				<th rowspan="2"><spring:message code='Cache.lbl_apv_Assignmented_Person'/></th>
				<td class="t_back01_line">
					...
					<div id="hDivManualInput" style="display: none; background-color: #eee; border: solid 1px #cccccc; padding: 4px; margin-top: 5px;">
						<span>
							<div style="display: inline-block; width: 55px; margin-left: 10px; padding-top: 5px;"><spring:message code="Cache.lbl_USER_NAME_01"/></div>
							<input type="text" id="DEPTNAME" style="width: 100px;" maxlength="30" class="AXInput av-required"/>
						</span>
						<span>
							<div style="display: inline-block; width: 70px; margin-left: 10px; padding-top: 5px;"><spring:message code="Cache.lbl_UserCode"/></div>
						...
						<table class="AXFormTable">
							...
							<thead>								
								<tr style="height: 38px;">
									...
									<th style="text-align: center; padding: 5px; border-left: none !important; border-right: none !important;"><spring:message code="Cache.lbl_USER_NAME_01"/></th>
									<th style="text-align: center; padding: 5px; border-left: none !important; border-right: none !important;"><spring:message code="Cache.lbl_UserCode"/></th>
