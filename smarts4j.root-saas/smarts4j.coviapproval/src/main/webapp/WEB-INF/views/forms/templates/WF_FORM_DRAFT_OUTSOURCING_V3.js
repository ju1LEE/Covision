...
function Cal(obj) {
    ...
    XFORM.multirow.event('afterRowAdded', function ($rows) {
                    ...
                    if (rowSDateMonth >= 13) {
                        ...
                        if (parseInt(rowSDateMonth) < 10 && parseInt(rowSDateMonth) != 1) { //시작월 10 미만 && 시작월 1 아닐때
                            ...
                            if (parseInt(rowSDateMonth >= 10)) {
                                ...
                                lastDayOfMon = (parseInt(rowSDateYear) + currentYear) + "-"  + "0" +(parseInt(rowSDateMonth) - (12 * parseInt(currentYear))) + "-" + lastSDate; //마지막일 (2017-11-31)
                            }
        ...
        //var monthDay = 30; 	//관리본부 요청으로 매달 30일로 고정 2019-01-17
        var monthDay = 32 - new Date(tempSUse.val().split("-")[0], tempSUse.val().split("-")[1]-1, 32).getDate(); // 매월 일수에 대한 일할 계산으로 변경
        
        if(rowSDateDay2 == simFirstDateDay && rowEDateDay2 == sumlastDateDay){ //한달 꽉 채운 경우 (1~마지막날)
        	monthDay = parseInt(sumlastDateDay);
        }
        ...
        if (index == 1) {
            //if (parseInt(rowSDateDay2) == 1) {
                ...
                tempCal = ((parseInt(rowEDateDay2) - parseInt(rowSDateDay2) + 1) / monthDay); //첫달 1일부터 시작 X
                hiddentempCal = ((parseInt(rowEDateDay2) - parseInt(rowSDateDay2) + 1) / monthDay);
            //}
            //else {
            //    tempCal = ((parseInt(rowEDateDay2) - parseInt(rowSDateDay2) + 1) / monthDay); //첫달 1일부터 시작 X
            //    hiddentempCal = ((parseInt(rowEDateDay2) - parseInt(rowSDateDay2) + 1) / monthDay);
            ...
            tempCal = ((parseInt(rowEDateDay2) - parseInt(rowSDateDay2) + 1) / monthDay);
            hiddentempCal = ((parseInt(rowEDateDay2) - parseInt(rowSDateDay2) + 1) / monthDay);
...
function rowNumTest(obj) {
    ...
    if (parseInt(mmSelected) == 1) {
    	...
    	var calTotal = parseFloat(priceInput * hiddenMMselected).toFixed();
    }
