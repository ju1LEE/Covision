...
function postRenderingForTemplate() {
	...
	var codeCol = ['EduType','EduTypeTwo','AfterEdu','EduPlanYn','EduPayway'];
...
function makeBodyContext() {
	...
	$$(bodyContextObj["BodyContext"]).append("eduSeq", CFN_GetQueryString('eduSeq')!='undefined'? CFN_GetQueryString('eduSeq') : "");
...
function getRequestData(){
	...
	var eduSeq = CFN_GetQueryString('eduSeq')!='undefined'? CFN_GetQueryString('eduSeq') : ""
	...
	if(eduSeq== null){
		...
		$.ajax({
			type:"POST",
			data: params,
			url:"/hrmanage/hrEdu/getHrEduInfoDetail.do",
			success:function (data) {
				if(data.result == "ok") {
					if(data.list != null && data.list[0] != null){
						
						for(var i=0;i<data.list.length;i++){
							$("#eduYear").val(data.list[i].EDU_YEAR);
							
							$("input:radio[name=EduType][value='"+data.list[i].EDU_TYPE+"']").prop("checked", true);
							$("input:radio[name=EduTypeTwo][value='"+data.list[i].EDU_TYPE2+"']").prop("checked", true);
							
							$("#eduName").val(data.list[i].EDU_NM);
							
							$("input:radio[name=AfterEdu][value='"+data.list[i].EDU_AF_ACT+"']").prop("checked", true);
							
							$("#SDATE").val(data.list[i].EDU_START_DATE);
							$("#EDATE").val(data.list[i].EDU_END_DATE);
							$("#eduTime").val(data.list[i].EDU_TIME);
							$("#eduAgency").val(data.list[i].EDU_CENTER);
		
							$("#eduIns").val(data.list[i].EDU_INS);

							$("input:radio[name=refund][value='"+data.list[i].REFUND_TYPE+"']").prop("checked", true);

							$("#eduCost").val(data.list[i].EDU_EXP);
							$("#refEduExp").val(data.list[i].REF_EDU_EXP);
							
							$("#eduProcess").val(data.list[i].EDU_PROCESS);
							$("#eduContent").val(data.list[i].EDU_CONTENT);
							...
							$("#eduCost").attr("disabled","disabled");
							$("#refEduExp").attr("disabled","disabled");
							$("#eduProcess").attr("disabled","disabled");
							...
							$("input:radio[name=AfterEdu]").attr("disabled","disabled");
							$("input:radio[name=refund]").attr("disabled","disabled");
							
							$("input:radio[name=EduType]:checked").css("background","black");
							$("input:radio[name=EduTypeTwo]:checked").css("background","black");
							$("input:radio[name=AfterEdu]:checked").css("background","black");
							$("input:radio[name=refund]:checked").css("background","black");
						}
