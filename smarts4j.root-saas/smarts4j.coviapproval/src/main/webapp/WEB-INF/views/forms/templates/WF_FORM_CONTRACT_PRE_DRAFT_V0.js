        ...
        if (formJson.Request.mode == "DRAFT" || formJson.Request.mode == "TEMPSAVE") {
            ...
            if (formJson.Request.mode == "DRAFT" && formJson.Request.gloct == "") {
                ...
                XFORM.multirow.load(json_multi_data, 'json', '#MultiRowTable1', 'W');
                 document.getElementById("Subject").value = "[계약품의] ";
            }
            else {
...
function checkForm(bTempSave) {
    ...
    if (bTempSave) { // 임시저장
        ...
        //else if ($("#AttFileInfo").children().length == 0) { // 기안 시 [첨부파일] 란에 첨부파일이 없다면
        else if ($("#fileInfo").children().find("input[type=checkbox]").length == 0) { // 기안 시 [첨부파일] 란에 첨부파일이 없다면
        	Common.Warning('계약서 및 최종견적서를 첨부하세요.');
            return false;
        }
        else {
    ...
    if (m_xmlHTTP.readyState == 4) {
        ...
        if (m_xmlHTTP.responseXML == null) {
            ...
            for (var i = 0; i < elmlist.length; i++) {
                ...
                document.getElementById("SEL_ENTPART").options.add(oOption);
...
// 프로젝트 명이 바뀔 때마다 제목 변경 [2016-09-05]
function changeTitle(contractName, MainClient) {
	if (formJson.Request.mode == "DRAFT"){
		$("#Subject").val("[계약품의] " + $(contractName).val() + "_" + $(MainClient).val());
	}
}