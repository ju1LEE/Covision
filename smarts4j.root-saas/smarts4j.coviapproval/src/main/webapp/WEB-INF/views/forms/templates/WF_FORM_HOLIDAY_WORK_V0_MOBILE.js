...
function postRenderingForTemplate() {
    ...
    if (getInfo("Request.templatemode") == "Read") {
        ...
        if (formJson.Request.mode == "DRAFT" || formJson.Request.mode == "TEMPSAVE") {
            ...
            //제목설정
            $("#approval_write_Subject").val("휴일근무 신청서_" + Common.getSession('UR_Name'));
		...
		if (sObj <= eObj) {
			...
			if (Common.getBaseConfig("ExtenMaxTime") != "" && Common.getBaseConfig("ExtenMaxTime") != "0"){
				if (gap > parseInt(Common.getBaseConfig("ExtenMaxTime"),10) *3600){
					alert("하루 최대 근무시간이 초과되어 신청할 수 없습니다.("+Common.getBaseConfig("ExtenMaxTime")+"H)");
					return;
				}
			}
...
function checkHolidayOne(obj) {
	var m_index = $("input[name='JobDate']").index(obj);
	var userCode = $("input[name=UserCode]").eq(m_index).val();
	var userName = $("input[name=UserName]").eq(m_index).val();
	var companyCode = $("input[name=CompanyCode]").eq(m_index).val();
	var targetDate = $("input[name=JobDate]").eq(m_index).val();
	...
	CFN_CallAjax("/approval/legacy/attendanceHolidayCheckOne.do", {"UserCode":userCode, "CompanyCode":companyCode, "TargetDate":targetDate}, function (data){
		if(data.status == "SUCCESS"){
			...
			if(isHoliday != "H") {
				$(obj).val("");
				alert(userName + "님은 '" + targetDate + "'에 휴무가 아닙니다.");
			}
...
function ChangeSubject(obj){
	...
	$("#approval_write_Subject").val("휴일근무 신청서_" + Common.getSession('UR_Name') + "(" + sMonth + "/" + sDay + ")");
	mobile_approval_changeSelectBox($("#approval_write_Subject"));
}
