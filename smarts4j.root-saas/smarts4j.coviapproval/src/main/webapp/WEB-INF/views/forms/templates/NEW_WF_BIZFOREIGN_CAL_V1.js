...
function Requester_CallBack(pStrItemInfo) {
   ...
   //     objTxtSelect.value = CFN_GetDicInfo(I_User.DN);
		...
		var name = CFN_GetDicInfo(I_User.DN);
        var deptName = CFN_GetDicInfo(I_User.RGNM);
        var position = CFN_GetDicInfo(I_User.PO.split('&')[1]);

        $(currentObj).closest("tr").find("input[name=BiztripName]").val(name);
        $(currentObj).closest("tr").find("input[name=BiztripDept]").val(deptName);
        $(currentObj).closest("tr").find("input[name=BiztripPosition]").val(position);
   // }
   //  else {
