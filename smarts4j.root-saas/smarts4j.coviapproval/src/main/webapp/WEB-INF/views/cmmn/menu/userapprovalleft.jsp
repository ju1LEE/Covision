...
<!-- </nav> -->
 <div class="lmb_wrap">
   <h1 class="brand"><a href="#" onclick="leftmenu_goToPageMain(); window.sessionStorage.setItem('UserLeftID', '');"><img src="/approval/resources/images/Approval/logo.gif" alt="CoviFlow"></a></h1>
   <ul class="lmb_menu">
   </ul>
 </div>
<script type="text/javascript">
	...
	function getApprovalListCount(){
		...
		$.ajax({
			...
			error:function(response, status, error){
				CFN_ErrorAjax("user/getApprovalNotDocReadCnt.do", response, status, error);
			}
		});
		//개인-진행함
// 		$.ajax({
...
// 		});
		...
		$.ajax({
			...
			error:function(response, status, error){
				CFN_ErrorAjax("user/getDeptProcessNotDocReadCnt.do", response, status, error);
			}
		});
		// 부서-참조/회람함
		$.ajax({
			...
			error:function(response, status, error){
				CFN_ErrorAjax("user/getDeptTCInfoNotDocReadCnt.do", response, status, error);
			}
		});
	}
</script>
