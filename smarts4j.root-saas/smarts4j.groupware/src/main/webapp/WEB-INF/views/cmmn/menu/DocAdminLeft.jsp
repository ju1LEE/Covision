...
<script type="text/javascript">
	...
	function changeCompany(){
		var url = '/groupware/layout/board_boardManage.do?CLSYS=doc&CLMD=admin&CLBIZ=Doc&domainID='+$("#domainIDSelectBox").val();
	...
	function ctxCreateBoardPopup(pMenuID, pFolderID){
		var title = Common.getDic('lbl_DocCate') + '/' + Common.getDic('FolderType_Doc') + '|||' + Common.getDic('lbl_Add');
	...
	function ctxEditBoardPopup(pMenuID, pFolderID){
		var title = Common.getDic('lbl_DocCate') + '/' + Common.getDic('FolderType_Doc') + '|||' + Common.getDic('lbl_Modify');
		parent.Common.open("","setBoard", title,
			"/groupware/admin/goBoardConfigPopup.do?folderID="+pFolderID+"&mode=edit&bizSection="+bizSection+"&domainID="+$("#domainIDSelectBox").val()+"&menuID="+pMenuID,
			"750px","600px","iframe",true,null,null,true
		);
	}
	...
	function leftInit(){
		...
		myFolderTree.setConfig({contextMenu:{
            ...
            filter:function(id){
            	if(this.sendObj.FolderType == 'Root' || this.sendObj.FolderType == 'DocRoot'){
		...
		if(loadContent == 'true'){
    		CoviMenu_GetContent('/groupware/layout/board_boardManage.do?CLSYS=doc&CLMD=admin&CLBIZ=Doc&domainID='+$("#domainIDSelectBox").val());	
    	}
	}
