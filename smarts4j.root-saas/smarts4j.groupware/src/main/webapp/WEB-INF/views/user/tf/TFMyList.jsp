...
<div class='cRConTop titType'>
	<h2 class="title"><spring:message code='Cache.MN_976'/></h2><!-- 나의 T/F 팀룸 -->
...
<div class='cRContBottom mScrollVH surveyProgress'>
	...
	<div class="selectBox" id="selectBoxDiv">
		<div class="pagingType02 buttonStyleBoxLeft">
			<a href="#" id="btnDelete" class="btnTypeDefault left" onclick="javascript:deleteTFTeamRoom();"><spring:message code='Cache.lbl_delete'/></a>
		</div>
...
var MyTFGrid = new coviGrid();
var msgHeaderData = "";
MyTFGrid.config.fitToWidthRightMargin=0;
...
//폴더 그리드 세팅
function setMyTFGrid(){
	MyTFGrid.setGridHeader(GridHeader());
	selectMyTFList();
}

function selectMyTFList(){
	var sortColumn = "";
	var sortDicrection  = "";
	...
	MyTFGrid.bindGrid({
		ajaxUrl:"/groupware/layout/selectUserMyTFGridList.do",
		ajaxPars: {
...
function setMyTFGridConfig(){
	...
	MyTFGrid.setGridConfig(configObj);
}
...
function GridHeader(){
	...
	msgHeaderData = MyTFGridHeader;
	return MyTFGridHeader;
}
...
function goCommunitySite(community){
	...
	//window.open("/groupware/layout/TFMain.do?C="+community, "community", specs);
	window.open("/groupware/layout/userCommunity/communityMain.do?C="+community, "community", specs);
}

function OpenTFInfo(CU_ID) {
	var memberType = getMemberType(CU_ID);
	if (Number(memberType) >= 8) { //수정권한(PM, 간사)
		Common.open("", "TFInfo"+CU_ID, "<spring:message code='Cache.lbl_TFView'/>", "/groupware/layout/goTFInfoEditPopup.do?CU_ID=" + CU_ID, "750px", "700px", "iframe");
	} else { //조회권한
		Common.open("", "TFInfo"+CU_ID, "<spring:message code='Cache.lbl_TFView'/>", "/groupware/layout/goTFInfoViewPopup.do?CU_ID=" + CU_ID, "750px", "700px", "iframe");
	}
}

function getMemberType(CU_ID) {
	var memberType = "";
	$.ajax({
		type:"POST",
		data: {
		...
		url:"/groupware/layout/selectMemberGrade.do",
		async: false,
		success:function(data){
			memberType = data.data;
		},
		error:function(response, status, error){
			 CFN_ErrorAjax("/groupware/layout/selectMemberGrade.do", response, status, error);
		}
	});
...
function deleteTFTeamRoom() {
	var chkObj = MyTFGrid.getCheckedList(0);
	var CU_IDs = '';
			...
			$.ajax({
				...
				success:function (data) {
					if(data.status == "SUCCESS") {
						...
						Common.Warning("<spring:message code='Cache.msg_ErrorOccurred'/>", '', function (result) { selectMyTFList(); }); //오류가 발생하였습니다.
					} else {
						Common.Warning("<spring:message code='msg_MyListAuth'/>" + "\n" + data.message, '', function (result) { selectMyTFList(); }); //선택한 항목은 삭제권한이 없습니다.
					}
				},
				error:function(response, status, error){
