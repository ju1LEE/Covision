...
<div>
	<h3 class="con_tit_box">
		<span class="con_tit">외주직원 단가 관리</span>
	</h3>
...
<script>
	...
	function setGrid(){
		osGradeGrid.setGridHeader([
			                  ...
			                  {key:'MonthPrice',  label:'MM단가', width:'100', align:'center', formatter:function() {
			                	  return '<input type="text" mode="moneyint" class="AXInput priceBox" id="mmAmount' + this.item.GradeKind + '" data-gradekind="'+this.item.GradeKind+'" data-axbind="pattern" value="' + this.item.MonthPrice + '" readonly="true" style="background : none; border:none;">';
			                  }},
			                  {key:'MonthPriceEx', label:'MM단가(지방)', width:'100', align:'center', formatter:function() {
			                	  return '<input type="text" mode="moneyint" class="AXInput priceBox" id="mmExAmount' + this.item.GradeKind + '" data-gradekind="'+this.item.GradeKind+'" data-axbind="pattern" value="' + this.item.MonthPriceEx + '" readonly="true" style="background : none; border:none;">';
			                  }}
				      		]);
		setGridConfig();
		bindGridData();
	...
	function bindGridData() {
		...
		osGradeGrid.bindGrid({
			ajaxUrl:"getoutsourcinggrade.do",
			ajaxPars: {
				year : year
			},
			onLoad: function() {
				...
				if(osGradeGrid.list.length > 0) {
					...
					$("#btnModify").on("click", function() {
						...
						$("#btnSave").one("click", function() {
							// 각 등급별 MM단가, MM단가(지방) 값 읽어서  json parameter로 전송
							...
							$(osGradeGrid.list).each(function(idx, data){
								...
								obj.MonthPriceEx = new Number($("#mmExAmount" + data.GradeKind).val().replace(/,/g, ''));
							...
							$.ajax({
								...
								error : function(response, status, error){
								     //TODO 추가 오류 처리
								     CFN_ErrorAjax("ModOutSourcingGrade.do", response, status, error);
								}
							});
						});
					...
					$("#btnDelete").on("click", function() {
						Common.Confirm("삭제 하시겠습니까?", "알림", function(result) {
							if(result) {
								var year = $("#selYear").val();
								$.post("deleteoutsourcinggrade.do", {year : year}, function(d) {
									bindGridData();
								}).error(function(response, status, error){
								     //TODO 추가 오류 처리
								     CFN_ErrorAjax("deleteoutsourcinggrade.do", response, status, error);
								});
							}
						});
					});
	...
	function gradeAddOpen() {
		Common.open("btnAdd", "GradeAddPop", "등급등록", "addGrade.do?mode=outsourcing", "400px", "230px", "iframe", true, null, null, true);
	};
	...
	function reUseRecentYear() {
		...
		Common.Confirm((currentYear - 1) + "년도 단가로 해당년도 단가를 설정합니다.", "알림", function(result) {
			if(result) {
				$.post("reuseOutSourcingGrade.do", {ApplyYear : currentYear}, function(d) {
					if(d.cnt == 0)
				     ...
				     //TODO 추가 오류 처리
				     CFN_ErrorAjax("reuseOutSourcingGrade.do", response, status, error);
				});
			}
		});
	}
