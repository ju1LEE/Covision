<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"
		 import="egovframework.baseframework.util.PropertiesUtil,egovframework.baseframework.util.RedisDataUtil" %>
...
<body>
<input id=AttSeq value="${data.AttSeq != null ? data.AttSeq: AttSeq}" type=hidden>
<input id=SchSeq value="${data.SchSeq}" type=hidden>

<div class="layer_divpop ui-draggable docPopLayer" style="width:100%;" source="iframe" modallayer="false"
	 layertype="iframe" pproperty="">
	<div class="divpop_contents">
		<div class="ATMgt_popup_wrap">
			<ul class="tabMenu tabTemp clearFloat">
				<li class="active" data-tab="tab-1"><a href="#"><spring:message code="Cache.lbl_baseInfo"/></a></li> <!-- 기본정보 -->
				<li data-tab="tab-2"><a href="#"><spring:message code="Cache.lbl_Applicable"/></a></li>
				<li data-tab="tab-3"><a href="#"><spring:message code="Cache.lbl_apv_Admin"/></a></li>
			</ul>
			<div class="ATMgt_popup_cont active" id="tab-1">
				<table class="ATMgt_popup_table type02" style="width:100%">
					...
					<tr>
						...
						<td>
							<input type="radio" id="WorkingSystemType1" name="WorkingSystemType" value="0" ${data.WorkingSystemType==null || data.WorkingSystemType == '0'?"checked":""}/>
							<label for="WorkingSystemType1"><spring:message code="Cache.lbl_designatedTime"/></label> <!-- 지정시간 -->
							<input type="radio" id="WorkingSystemType2" name="WorkingSystemType" value="1" ${data.WorkingSystemType == '1'?"checked":""}/>
							<label for="WorkingSystemType2"><spring:message code="Cache.lbl_Autonomous"/></label> <!-- 자율 -->
							<input type="radio" id="WorkingSystemType3" name="WorkingSystemType" value="2" ${data.WorkingSystemType == '2'?"checked":""}/>
					...
					<tr>
						...
						<td><input type="text" placeholder="<spring:message code='Cache.lbl_EnterTemplateName'/>" id="SchName" name="SchName" class="WorkingStatusAdd_title_input w100" value="${data.SchName }"></td>
					</tr>
					<tr>
						<td class="ATMgt_T_th"><spring:message code='Cache.lbl_n_att_coreTime'/></td> <!-- 코어타임 -->
						<td>
							<div class="ATMgt_T_Time">
                                <span class="alarm type01"><input id="CoreTimeYn" value="Y" type="checkbox" ${data.CoreTimeYn!=null && data.CoreTimeYn == 'Y'?"checked":""} style="display:none">
                                <label class="onOffBtn ${data.CoreTimeYn!=null && data.CoreTimeYn == 'Y'?"on":""}" href="#" for="CoreTimeYn"><span></span></label></span>
								<select name="CoreStartTimeHour">
									<c:forEach begin="00" end="23" var="hour">
										<option value="${ hour < 10 ? '0' : '' }${hour}" <c:if test="${ hour == (data.CoreStartTimeHour!=null?data.CoreStartTimeHour:'00') }">selected</c:if>>${ hour < 10 ? '0' : '' }${hour}</option>
									</c:forEach>
								</select>
								<select name="CoreStartTimeMin">
									<c:forEach begin="00" end="59" var="min">
										<option value="${ min < 10 ? '0' : '' }${min}" <c:if test="${ min == (data.CoreStartTimeMin!=null?data.CoreStartTimeMin:'00') }">selected</c:if>>${ min < 10 ? '0' : '' }${min}</option>
									</c:forEach>
								</select>
								~
								<select name="CoreEndTimeHour">
									<c:forEach begin="00" end="23" var="hour">
										<option value="${ hour < 10 ? '0' : '' }${hour}" <c:if test="${ hour == (data.CoreEndTimeHour!=null?data.CoreEndTimeHour:'00') }">selected</c:if>>${ hour < 10 ? '0' : '' }${hour}</option>
									</c:forEach>
								</select>
								<select name="CoreEndTimeMin">
									<c:forEach begin="00" end="59" var="min">
										<option value="${ min < 10 ? '0' : '' }${min}" <c:if test="${ min == (data.CoreEndTimeMin!=null?data.CoreEndTimeMin:'00') }">selected</c:if>>${ min < 10 ? '0' : '' }${min}</option>
									</c:forEach>
								</select>
							</div>
						</td>
								...
								<div class="ATMgt_T_l">
									<div class="ATMgt_T_Time">
										<select id="WeekStartHour" name="WeekStartHour">
											<c:forEach begin="00" end="23" var="hour">
												<option value="<c:out value="${ hour < 10 ? '0' : '' }${hour}"/>" <c:if test="${ hour == (data.AttDayStartTime!=null?fn:substring(data.AttDayStartTime,0,2):9) }">selected</c:if> > <c:out value="${ hour < 10 ? '0' : '' }${hour}"/></option>
											</c:forEach>
										</select>
										<span>:</span>
										<input type="text" name="WeekStartMin" id="WeekStartMin" maxlength="2" value="${data.AttDayStartTime != null ? fn:substring(data.AttDayStartTime,2,4) : '00'}"/>
										<span>-</span>
										<select id="WeekEndHour" name="WeekEndHour">
											<c:forEach begin="00" end="23" var="hour">
												<option value="<c:out value="${ hour < 10 ? '0' : '' }${hour}"/>" <c:if test="${ hour == (data.AttDayEndTime!=null?fn:substring(data.AttDayEndTime,0,2):18) }">selected</c:if> > <c:out value="${ hour < 10 ? '0' : '' }${hour}"/></option>
											</c:forEach>
										</select>
										<span>:</span>
										<input type="text" name="WeekEndMin" id="WeekEndMin" maxlength="2" value="${data.AttDayEndTime != null ? fn:substring(data.AttDayEndTime,2,4) : '00'}"/>
										<input type="checkbox" id="WeekNextDayYn" name="WeekNextDayYn" value="Y" ${data.NextDayYn == 'Y'?"checked":""}/>
					...
					<tr id="GoWorkTimeTr" style="display: none;">
						...
						<td>
							<div class="ATMgt_T_Time">
                                <span class="alarm type01"><input id="GoWorkTimeYn" value="Y" type="checkbox" ${data.GoWorkTimeYn!=null && data.GoWorkTimeYn == 'Y'?"checked":""} style="display:none">
                                <label class="onOffBtn ${data.GoWorkTimeYn!=null && data.GoWorkTimeYn == 'Y'?"on":""}" href="#" for="GoWorkTimeYn"><span></span></label></span>
								<select name="GoWorkStartTimeHour">
									<c:forEach begin="00" end="23" var="hour">
										<option value="${ hour < 10 ? '0' : '' }${hour}" <c:if test="${ hour == (data.GoWorkStartTimeHour!=null?data.GoWorkStartTimeHour:'00') }">selected</c:if>>${ hour < 10 ? '0' : '' }${hour}</option>
									</c:forEach>
								</select>
								<select name="GoWorkStartTimeMin">
									<c:forEach begin="00" end="59" var="min">
										<option value="${ min < 10 ? '0' : '' }${min}" <c:if test="${ min == (data.GoWorkStartTimeMin!=null?data.GoWorkStartTimeMin:'00') }">selected</c:if>>${ min < 10 ? '0' : '' }${min}</option>
									</c:forEach>
								</select>
								~
								<select name="GoWorkEndTimeHour">
									<c:forEach begin="00" end="23" var="hour">
										<option value="${ hour < 10 ? '0' : '' }${hour}" <c:if test="${ hour == (data.GoWorkEndTimeHour!=null?data.GoWorkEndTimeHour:'00') }">selected</c:if>>${ hour < 10 ? '0' : '' }${hour}</option>
									</c:forEach>
								</select>
								<select name="GoWorkEndTimeMin">
									<c:forEach begin="00" end="59" var="min">
										<option value="${ min < 10 ? '0' : '' }${min}" <c:if test="${ min == (data.GoWorkEndTimeMin!=null?data.GoWorkEndTimeMin:'00') }">selected</c:if>>${ min < 10 ? '0' : '' }${min}</option>
									</c:forEach>
								</select>
							</div>
						</td>
					...
					<tr id="OffWorkTimeTr" style="display: none;">
						...
						<td>
							<div class="ATMgt_T_Time">
                                <span class="alarm type01"><input id="OffWorkTimeYn" value="Y" type="checkbox" ${data.OffWorkTimeYn != null && data.OffWorkTimeYn == 'Y'?"checked":""} style="display:none">
                                <label class="onOffBtn ${data.OffWorkTimeYn != null && data.OffWorkTimeYn == 'Y'?"on":""}" href="#" for="OffWorkTimeYn"><span></span></label></span>
								<select name="OffWorkStartTimeHour">
									<c:forEach begin="00" end="23" var="hour">
										<option value="${ hour < 10 ? '0' : '' }${hour}" <c:if test="${ hour == (data.OffWorkStartTimeHour!=null?data.OffWorkStartTimeHour:'00') }">selected</c:if>>${ hour < 10 ? '0' : '' }${hour}</option>
									</c:forEach>
								</select>
								<select name="OffWorkStartTimeMin">
									<c:forEach begin="00" end="59" var="min">
										<option value="${ min < 10 ? '0' : '' }${min}" <c:if test="${ min == (data.OffWorkStartTimeMin!=null?data.OffWorkStartTimeMin:'00') }">selected</c:if>>${ min < 10 ? '0' : '' }${min}</option>
									</c:forEach>
								</select>
								~
								<select name="OffWorkEndTimeHour">
									<c:forEach begin="00" end="23" var="hour">
										<option value="${ hour < 10 ? '0' : '' }${hour}" <c:if test="${ hour == (data.OffWorkEndTimeHour!=null?data.OffWorkEndTimeHour:'00') }">selected</c:if>>${ hour < 10 ? '0' : '' }${hour}</option>
									</c:forEach>
								</select>
								<select name="OffWorkEndTimeMin">
									<c:forEach begin="00" end="59" var="min">
										<option value="${ min < 10 ? '0' : '' }${min}" <c:if test="${ min == (data.OffWorkEndTimeMin!=null?data.OffWorkEndTimeMin:'00') }">selected</c:if>>${ min < 10 ? '0' : '' }${min}</option>
									</c:forEach>
								</select>
							</div>
						</td>
					...
					<tr id="AssTr" class="divAss" style="display: none;">
						...
						<td>
							<div class="ATMgt_T_Time">
                                <span class="alarm type01"><input id="AssYn" value="Y" type="checkbox" ${data.AssYn != null && data.AssYn == 'Y'?"checked":""} style="display:none">
                                    <label class="onOffBtn ${data.AssYn != null && data.AssYn == 'Y'?"on":""}" href="#" for="AssYn"><span></span></label>
                                </span>
								<select id="AssSeq" name="AssSeq" ${data.AssYn != null && data.AssYn == 'Y'?"":"disabled"}>
					...
					<tr id="SelfCommTr" style="display: none;">
						...
						<td>
							<div class="ATMgt_T ATMgt_T_wd">
								<div class="ATMgt_T_l">
									<div class="ATMgt_T_Time">
										<input id="SelfCommYn" type="hidden" value="${data.SelfCommYn != null && data.SelfCommYn != ''?data.SelfCommYn:'N'}" />
										<span><spring:message code='Cache.lbl_Mail_Maximum'/><spring:message code='Cache.lbl_att_ac_time'/> : </span><input type="text" name="AttDayAC" id="AttDayAC" style="width:65px" value="${data.AttDayAC}"><span><spring:message code='Cache.lbl_Minutes' /></span>	<!-- 최대인정시간 : 분 -->
									</div>
								</div>
								<div class="ATMgt_T_r">
					...
					<tr id="AutoExtenTr" style="display: none;">
						...
						<td>
							<div class="ATMgt_T_Time">
							<span class="alarm type01"><input id="AutoExtenYn" value="Y" type="checkbox" ${data.AutoExtenYn != null && data.AutoExtenYn == 'Y'?"checked":""} style="display:none">
							<label class="onOffBtn ${data.AutoExtenYn != null && data.AutoExtenYn == 'Y'?"on":""}" href="#" for="AutoExtenYn"><span></span></label></span>
								<span><spring:message code='Cache.lbl_Minimum'/> <spring:message code='Cache.lbl_att_ac_time'/> : </span> <!-- 최소 인정시간 -->
								<select name="AutoExtenMinTime">
									...
									<option value="60">60<spring:message code='Cache.lbl_Minutes'/></option> <!-- 분 -->
								</select>
				...
				<div class="ATMgt_memo_wrap">
					<table class="ATMgt_popup_table type02" cellpadding="0" cellspacing="0">
						...
						<tr>
							...
							<td><textarea maxlength="255" rows="5" id="Memo" name="Memo" class="ATMgt_Tarea av-required w100">${data.Memo}</textarea></td>
						</tr>
						</tbody>
					</table>
				</div>
						...
						<tr>
							...
							<td>
								<div class="searchBox03">
									<span>
										<input type="text" name="WorkZone" class="w100" id="WorkZone" value="${data.WorkZone}"/>
										<button class="btnSearchType01" id="btnMap" type="button"></button>
									</span>
								</div>
								<input type="text" name="WorkAddr" class="w100 mt5" id="WorkAddr" value="${data.WorkAddr}"/>
								<input type="hidden" name="WorkPointX" id="WorkPointX" value="${data.WorkPointX}"/>
								<input type="hidden" name="WorkPointY" id="WorkPointY" value="${data.WorkPointY}"/>
							</td>
						</tr>
						<tr>
							<td class="ATMgt_T_th"><spring:message code='Cache.lbl_CoordinateRadius'/>(m)</td> <!-- 좌표반경 -->
							<td><input type="text" class="w100" name="AllowRadius" id="AllowRadius" value="${data.AllowRadius}"></td>
						</tr>
						<tr>
							<td class="ATMgt_T_th"><spring:message code='Cache.lbl_AttendanceArea'/> <spring:message code='Cache.lbl_Add'/></td> <!-- 출근지 추가 -->
							<td>
								<select name="GoWorkPlace">
								</select>
								<a class="btn_Ok" onclick="addWorkPlace('GoWorkPlace');" style="height: 30px; line-height: 28px;"><spring:message code='Cache.lbl_Add'/></a> <!-- 추가 -->
								<div id="GoWorkPlaceSelected" style="width:100%; margin: 10px 0;">
									<c:forEach items="${goWorkPlaceList}" var="list" varStatus="status">
										<div class="ui-autocomplete-multiselect-item placeLabel" data-value="${list.LocationSeq}" id="GoWorkPlace${list.LocationSeq}"><span>${list.WorkZone}</span><a class="ui-icon ui-icon-close tag_del" onclick="removeWorkPlace('GoWorkPlace', '${list.LocationSeq}', '${list.WorkZone}')"></a></div>
									</c:forEach>
								</div>
							</td>
						</tr>
						<tr>
							<td class="ATMgt_T_th"><spring:message code='Cache.lbl_LeaveWorkArea'/> <spring:message code='Cache.lbl_Add'/></td> <!-- 퇴근지 추가 -->
							<td>
								<select name="OffWorkPlace">
								</select>
								<a class="btn_Ok" onclick="addWorkPlace('OffWorkPlace');" style="height: 30px; line-height: 28px;"><spring:message code='Cache.lbl_Add'/></a> <!-- 추가 -->
								<div id="OffWorkPlaceSelected" style="width:100%; margin: 10px 0;">
									<c:forEach items="${offWorkPlaceList}" var="list" varStatus="status">
										<div class="ui-autocomplete-multiselect-item placeLabel" data-value="${list.LocationSeq}" id="OffWorkPlace${list.LocationSeq}"><span>${list.WorkZone}</span><a class="ui-icon ui-icon-close tag_del" onclick="removeWorkPlace('OffWorkPlace', '${list.LocationSeq}', '${list.WorkZone}')"></a></div>
									</c:forEach>
								</div>
							</td>
						</tr>
				...
				<div class="bottom mtop20">
					<a id="btnSave" class="btnTypeDefault btnTypeChk"><spring:message code='Cache.ACC_btn_save'/></a><!-- 저장 -->
				...
				<div class="ATMgt_Work_Popup" id="divWeek" style="width:500px; right:40px; z-index:105;display:none; top:52px">
								...
								<td>
									<div class="ATMgt_T_Time">
										<select id="${StartHour}" name="${StartHour}">
											<c:forEach begin="00" end="23" var="hour">
												<option value="<c:out value="${ hour < 10 ? '0' : '' }${hour}"/>" <c:if test="${ hour == (data[StartHour]!=null?data[StartHour]:9) }">selected</c:if> ><c:out value="${ hour < 10 ? '0' : '' }${hour}"/></option>
											</c:forEach>
										</select>
										<span>:</span>
										<input type="text" name="${StartMin}" id="${StartMin}" maxlength="2" value="${data[StartMin] != null ? data[StartMin] : '00'}"/>
										<span>-</span>
										<select id="${EndHour}" name="${EndHour}">
											<c:forEach begin="00" end="23" var="hour">
												<option value="<c:out value="${ hour < 10 ? '0' : '' }${hour}"/>"<c:if test="${ hour == (data[EndHour]!=null?data[EndHour]:18) }">selected</c:if> > <c:out value="${ hour < 10 ? '0' : '' }${hour}"/></option>
											</c:forEach>
										</select>
										<span>:</span>
										<input type="text" name="${EndMin}" id="${EndMin}" maxlength="2" value="${data[EndMin] != null ? data[EndMin] : '00'}"/>
									</div>
								</td>
								<td>
					...
					<div class="bottom">
						<a href="#" class="btnTypeDefault btnTypeChk" id="btnAdd"><spring:message code='Cache.lbl_Add'/></a> <!-- 추가 -->
						<a href="#" class="btnTypeDefault" id="btnDivClose"><spring:message code='Cache.lbl_close'/></a> <!-- 닫기 -->
					</div>
				</div>
				...
				<div class="ATMgt_Work_Popup" id="divSelfCommWeek" style="width:500px; right:40px; z-index:105;display:none; top:52px">
					...
					<table class="ATMgt_DayTime_T" cellpadding="0" cellspacing="0">
						...
						<tr>
							<th width="80"><spring:message code='Cache.lbl_ADay'/></th> <!-- 요일 -->
							<th><spring:message code='Cache.lbl_att_ac_time'/></th> <!-- 인정시간 -->
						</tr>
						</thead>
						<tbody>
						<c:forEach items="${week}" var="list" varStatus="status">
							...
							<tr>
								...
								<td>
									<div class="ATMgt_T_Time">
										<span><spring:message code='Cache.lbl_att_ac_time'/> : </span><input type="text" name="Ac${list}" id="Ac${list}" style="width:65px" value="${data[Ac] != null ? data[Ac] : ''}"><span><spring:message code='Cache.lbl_Minutes'/></span> <!-- 인정시간 : 분 -->
									</div>
								</td>
							</tr>
						</c:forEach>
					...
					<div class="bottom">
						<a href="#" class="btnTypeDefault btnTypeChk" id="btnSelfCommAdd"><spring:message code='Cache.lbl_Add'/></a> <!-- 추가 -->
						<a href="#" class="btnTypeDefault" id="btnSelfCommDivClose"><spring:message code='Cache.lbl_close'/></a> <!-- 닫기 -->
					</div>
				</div>
			</div>
			<!-- 지정자 -->
			<div class="ATMgt_popup_cont" id="tab-2">
				<c:set var="url" value="src='/groupware/attendSchedule/goAttSchMemListPopup.do?SchSeq=${data.SchSeq}' load='Y'"/>
				<iframe id="SchMemPop_if" class="bgiframe" frameborder="0" ${mode == 'add'?'': url} style="display: block; position: relative; width: 100%; height: 470px;" scrolling="auto"></iframe>
			</div>
			<!-- 관리자 -->
			<div class="ATMgt_popup_cont" id="tab-3">
				<c:set var="url" value="src='/groupware/attendSchedule/goAttSchAllocListPopup.do?SchSeq=${data.SchSeq}' load='Y'"/>
				<iframe id="SchAllocPop_if" class="bgiframe" frameborder="0"  ${mode == 'add'?'': url} style="display: block; position: relative; width: 100%; height: 470px;" scrolling="auto"></iframe>
			</div>
		</div>
	</div>
</div>
...
<script>
	...
	$(document).ready(function () {
		// 근무제에 따른 화면 표시
		var workingSystemType = ${data.WorkingSystemType==null || data.WorkingSystemType==''?0:data.WorkingSystemType};
		var selfCommYn = "${data.SelfCommYn == null || data.SelfCommYn==''?'N':data.SelfCommYn}";

		beforeWorkingSystemType = workingSystemType;

		if ($("#SchSeq").val() != "") {
			if(workingSystemType == 0) {
				$("#WeekSet").prop("checked", true);
		...
		if(selfCommYn == 'Y') {
			if(workingSystemType == 0) { //근무제 타입 설정이 없었을 때 대비 코딩
				workingSystemType = 1;
				$("input[name=WorkingSystemType]:radio[value='1']").prop('checked', true);
			}
		}
		...
		$('.tabTemp li').click(function () {
			//alert($("#SchSeq").val() )
			if ($("#SchSeq").val() == "") {
				Common.Confirm("<spring:message code='Cache.mag_Attendance63' />", "Confirmation Dialog");	//근무제를 입력하세요.
				return;
			}

			var tab_id = $(this).attr('data-tab');

			$('.tabTemp li').removeClass('active');
			$('.ATMgt_popup_cont').removeClass('active');
			...
			switch ($(this).index()) {
				case 1://지정자
					if ($('#SchMemPop_if').attr('load') != "Y") {
						$('#SchMemPop_if').attr('load', 'Y');
						$('#SchMemPop_if').attr('src', "/groupware/attendSchedule/goAttSchMemListPopup.do?SchSeq=" + $("#SchSeq").val());
					}
					break;
				case 2://관리자
					if ($('#SchAllocPop_if').attr('load') != "Y") {
						$('#SchAllocPop_if').attr('load', 'Y');
						$('#SchAllocPop_if').attr('src', "/groupware/attendSchedule/goAttSchAllocListPopup.do?SchSeq=" + $("#SchSeq").val());
					}
					break;
			}
		});
		...
		$("input[name='WorkingSystemType']:radio").change(function() {
			...
			if(tmpValue == 0) {
				...
				Common.Confirm("<spring:message code='Cache.mag_Attendance64' />", "Confirmation Dialog", function (confirmResult) {	//근무시간이 초기화 됩니다. 자율출근제로 변경하시겠습니까?
					if (confirmResult) {
						//지정 근무 초기화
						$("#WeekStartHour").val("00");
						$("#WeekStartMin").val("00");
	...
	function saveSchedule() {
		...
		objParams["Idle${list}"] = 0
		</c:forEach>

		$.ajax({
			type: "POST",
			url: "/groupware/attendSchedule/saveAttendSchedule.do",
			data: objParams,
			success: function (data) {
				if (data.result == "ok") {

					parent.Common.Inform("<spring:message code='Cache.msg_Been_saved'/>");	//저장되었습니다.
					if ("${mode}" == "add") $("#SchSeq").val(data.SchSeq);
					//Common.Close();
					try {
						parent.AttendSchedulePopup_CallBack();
					} catch (e) {
						coviCmn.traceLog(e);
					}
					Common.Close();
...
//	parent.Common.open("","SchMemPop",titlemessage,url,"490px","470px","iframe",true,null,null,true);
	...
	function addWorkPlace(workPlaceType) {
		...
		var orgMapItem = $('<div class="ui-autocomplete-multiselect-item placeLabel" id="' + workPlaceType + code + '"/>')
				.attr({'data-value': code })
				.append($("<span></span>")
					.text(name))
				.append($("<a></a>")
						.addClass("ui-icon ui-icon-close tag_del")
