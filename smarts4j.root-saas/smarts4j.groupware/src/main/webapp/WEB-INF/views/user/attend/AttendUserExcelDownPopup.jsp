...
function popExcelDownload(){
	...
	var params = {
				...
				,"EndDate" : $("#EndDate").val()
				,"dataMode" : $("input[name=dataMode]:checked").length>0?$("input[name=dataMode]:checked").val():null
				,"dateTerm" : $("#excelDownType").val()
				,"RetireUser" : $("#retireUser").is(":checked")?"Y":"N"
				,"outputNumtype" : $("#outputNumtype").is(":checked")?"Y":"N"
				,"AttStatus" : $("input[name=AttStatus]:checked").length>0?$("input[name=AttStatus]:checked").val():""
			};

	//ajax_download('/groupware/attendUserSts/excelDownloadForAttMngStatusV2.do', params);	// 엑셀 다운로드 post 요청
	ajax_download('/groupware/attendUserSts/excelDownloadForAttMngStatus.do', params);	// 엑셀 다운로드 post 요청
}
			...
			<div class="ATMgt_work_wrap">
				<table class="ATMgt_popup_table type03" cellpadding="0" cellspacing="0">
					...
					<tr>
						<td class="ATMgt_T_th">
							<select class="selectType02" id="excelDownType" onchange="selectBoxChg(this.value);">
								...
								<option value=""><spring:message code='Cache.lbl_SpecifyingPeriod'/></option> <!-- 기간지정 -->
							</select>
						</td>
						<td>
							<div class="dateSel">
					...
					<tr>
						...
						<td style="text-align:center"><input id="dataMode1" name="dataMode" value="" type="checkbox" checked>
							...
							<label for=outputNumtype>RAW <spring:message code="Cache.btn_apv_print"/></label><!-- btn_apv_print-->
						</td>
					</tr>
				</table>	
			</div>		
