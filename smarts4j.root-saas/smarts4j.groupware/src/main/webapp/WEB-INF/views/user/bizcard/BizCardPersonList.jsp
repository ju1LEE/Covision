...
<div class="cRConTop titType">
	<h2 class="title"><spring:message code='Cache.lbl_AllList' /></h2> <!-- 전체 목록 -->
	...
	<div class="bizcardAllCont">
		...
		<div id="divTabTray" class="tblList tblCont tblBizcardList">
			<div class="tblFilterTop">
				<!-- p class="filterGroup">
					<a id="bizCardListGroup2" href="#" onclick="clickTab(this);" value="Group"><spring:message code='Cache.lbl_group' /></a> <!-- 그룹 -->
				</p-->
				<ul class="filterList">
			...
			bizCardGrid.setGridHeader([
			  ...
			  {key:'IsFavorite',hideFilter : 'Y',colHeadTool:false,  label:'<a class=\"tblBtnFavorite\" href=\"#\">Favorite</a>', width:'20', align:'center', sort:false, formatter : function() {
				  	if(this.item.BizCardType == 'BizCard'  ){
		               ...
		               {key:'Name',  label:"<spring:message code='Cache.lbl_name'/>", width:'200', align:'left', //이름
		             	  formatter:function () {
		  				  ...
		  				  {key:'PhoneNum',  label:"<spring:message code='Cache.lbl_MobilePhone'/>", width:'100', align:'center', formatter : function() { //핸드폰
		             		var PhoneNum = this.item.PhoneNum;
		             		if(PhoneNum.indexOf(";") > 0)
		             			PhoneNum = PhoneNum.substr(0,PhoneNum.indexOf(";"));
		             		return PhoneNum;
		               ...
		               {key:'ComName',  label:"<spring:message code='Cache.lbl_Company'/>", width:'70', align:'center'}, //회사
		               ...
		               {key:'JobTitle', label:"<spring:message code='Cache.lbl_JobTitle'/>", width:'70', align:'center'}, //직책
		               {key:'ShareType',  label:"<spring:message code='Cache.lbl_Division'/>", width:'70', align:'center', formatter : function() { //구분
		             	  var shareType = "";
		             	  switch(this.item.ShareType) {
		             	  case 'P' : shareType = "<spring:message code='Cache.lbl_ShareType_Personal'/>"; break; //개인
...
//		if($("#divTabTray .active").find("a").attr('value') == 'Group'){
	...
	var viewBizCardPop = function(id) { //명함 보기
		var BizCardID = id;
	//	Common.open("", "ViewBizCardPerson", Common.getDic("lbl_Mail_CardView"), "/groupware/bizcard/ViewBizCardPerson.do?BizCardID=" + encodeURIComponent(BizCardID) + "&ShareType=" + bizCardViewType, "500px", "530px", "iframe", true, null, null, true);		
	//	return;
	...
	function initContent(){
		...
		$("#listView").off('click').on('click', function(){
			...
			//setGrid(); 2019-01-08  iwyoon 그룹,연락처 통합조회를 위해 수정
			setGridAll();
		});
