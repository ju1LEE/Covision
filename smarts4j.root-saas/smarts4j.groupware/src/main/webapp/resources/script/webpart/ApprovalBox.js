...
var approvalBox ={
		...
		/*	if(this.webpartType>=0){ 
			...
			$.each( data[0], function(index, value) {
				...
				$("#approvalBox_approvalList > tbody").append('<tr><td height="25"><a href=\'#none\'  style="color:black;" onclick="approvalBox.clickPopup(\'APPROVAL\',\''+value.ProcessID+'\',\''+value.WorkItemID+'\',\''+value.PerformerID+'\',\''+value.ProcessDescriptionID+'\',\''+value.FormSubKind+'\',\'\',\'\',\'\',\'\',\''+value.UserCode+'\',\''+value.FormPrefix+'\'); return false;"   >'
						+value.FormSubject+
						'</td><td>'
						+value.FormName+
						'</td><td>'
			...
			$.each( data[1], function(index, value) {
				var finished = 	CFN_TransLocalTime(value.Finished, "yyyy.MM.dd");
			
				$("#approvalBox_processList > tbody").append('<tr><td height="25"><a href=\'#none\'  style="color:black;" onclick="approvalBox.clickPopup(\'PROCESS\',\''+value.ProcessID+'\',\''+value.WorkItemID+'\',\''+value.PerformerID+'\',\''+value.ProcessDescriptionID+'\',\''+value.FormSubKind+'\',\'\',\'\',\'\',\'\',\''+value.UserCode+'\',\''+value.FormPrefix+'\'); return false;"   >'
						+value.FormSubject+
						'</td><td>'
						+value.FormName+
						'</td><td>'
