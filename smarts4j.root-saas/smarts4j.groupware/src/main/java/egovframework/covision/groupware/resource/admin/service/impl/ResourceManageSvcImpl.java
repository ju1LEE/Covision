package egovframework.covision.groupware.resource.admin.service.impl;
...
public class ResourceManageSvcImpl extends EgovAbstractServiceImpl implements ResourceManageSvc {
	...
	public int insertMainResourceList(CoviMap params) throws Exception {
		...
		String[] resourceIDArr = (String[])params.get("resourceIDArr");
		if(!"".equals(resourceIDArr[0])) {
	...
	CoviMap returnObj = new CoviMap(); 
		...
		CoviList shareResourceList = CoviSelectSet.coviSelectJSON(list, "FolderID,DomainName,ResourceName,RegisterName,RegistDate,MultiDisplayName");
	...
	public int insertFolderData(CoviMap params) throws Exception {
		...
		folderDataObj.put("menuID", 0); 	// Resource일 경우, MenuID가 not null 설정이라서 의미없는 0값으로 입력.
...
String userCode = SessionHelper.getSession("UR_Code");		
		...
		folderDataObj.put("menuID", 0); 	// 자원입력의 경우, MenuID가 Not Null 설정이라서 0값으로 입력.
	...
	@Override
	public int selectDuplicateShareResource(CoviMap params) throws Exception {
		int cnt = (int) coviMapperOne.getNumber("admin.resource.selectDuplicateShareResource", params);
		return cnt;
	}
