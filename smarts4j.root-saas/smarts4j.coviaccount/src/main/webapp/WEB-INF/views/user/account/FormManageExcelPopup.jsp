...
<div class="popContent vmExcelUploadPopup" style="padding: 0px 24px 30px;">
	<div class="">
		...
		<div class="bottom" style="text-align: center;">
			<a class="btnTypeDefault btnTypeBg" onclick="FormManageExcelPopup.excelUpload()"><spring:message code="Cache.ACC_btn_upload" /></a>	<!-- 업로드 -->
			<a class="btnTypeDefault" onclick="FormManageExcelPopup.closeLayer()"><spring:message code='Cache.ACC_btn_close' /></a>				<!-- 닫기 -->
		</div>
	</div>
</div>

<script>
	if (!window.FormManageExcelPopup) {
		window.FormManageExcelPopup = {};
	}
	
	(function(window) {
		var FormManageExcelPopup = {
				...
				excelUpload : function() {
					...
					$.ajax({
						url: '/account/formManage/formManageExcelUpload.do',
						processData: false,
						contentType: false,
						data: formData,
						type: 'POST',
						success: function(result) {
							if(result.status == "SUCCESS"){ 
								if(result.err == 'formCode'){
									...
									FormManageExcelPopup.closeLayer();
		...
		window.FormManageExcelPopup = FormManageExcelPopup;
	})(window);

	FormManageExcelPopup.initContent();
