		...
		<div class="eaccountingCont">
			<div class="eaccountingTopCont">
				...
				<div class="pagingType02 buttonStyleBoxLeft">
					<!-- 새로고침 -->
					<a class="btnTypeDefault"			onclick="TaxInvoice.refresh()"><spring:message code="Cache.ACC_btn_refresh"/></a>
					<!-- 엑셀저장 -->
					<a class="btnTypeDefault btnExcel"	onclick="TaxInvoice.excelDownload();"><spring:message code="Cache.ACC_btn_excelDownload"/></a>
					<!-- 상계처리 -->
					<a class="btnTypeDefault"			onclick="TaxInvoice.updateIsOffset('Y')"	id="btnIsOffsetY"><spring:message code="Cache.ACC_lbl_offsetY"/></a>
					<!-- 상계취소처리 -->
					<a class="btnTypeDefault"			onclick="TaxInvoice.updateIsOffset('N')"	id="btnIsOffsetN"><spring:message code="Cache.ACC_lbl_offsetN"/></a>
					<!-- 전달 -->
					<a class="btnTypeDefault"			onclick="TaxInvoice.tossUserPopup()"		id="btnTossUser"><spring:message code="Cache.ACC_lbl_tossUser"/></a>
					<!-- 동기화 -->
					<a class="btnTypeDefault"			onclick="TaxInvoice.taxInvoiceSync();"		id="btnSyncTax"><spring:message code="Cache.ACC_btn_sync"/></a>
					<!-- 현금 영수증 동기화 -->
					<a class="btnTypeDefault"			onclick="TaxInvoice.cashBillSync();"		id="btnSyncCash"><spring:message code="Cache.ACC_lbl_cashBillSync"/></a>
					<!-- 미상신 알림전송 -->
					<a class="btnTypeDefault" 			onclick="TaxInvoice.sendDelay();"><spring:message code="Cache.lbl_AccDelaySend"/></a>
					<!-- 완료처리-->
					<%-- <a class="btnTypeDefault"			onclick="TaxInvoice.updateIsOffset('A');"		id="btnConfirm"><spring:message code="Cache.ACC_lbl_offsetA"/></a> --%>
				</div>
				<!-- 상단 버튼 끝 -->				
			</div>
				...
				<div class="inPerView type08">
					<div style="width:900px;">
						<div class="inPerTitbox">
							...
							<span id="companyCode" class="selectType02" onchange="TaxInvoice.changeCompanyCode()">
							</span>
						</div>
						<div class="inPerTitbox">
							<span class="bodysearch_tit">
							...
							<input onkeydown="TaxInvoice.onenter()" id="searchStr" type="text" placeholder="" style ="width:200px">
						</div>
					</div>
					<div style="width:900px;">
						<div class="inPerTitbox">
						...
						<div class="inPerTitbox">
							...
							<div id="writeDate1" class="dateSel type02">
							</div>
						</div>						
						<a class="btnTypeDefault  btnSearchBlue" onclick="TaxInvoice.searchList();"><spring:message code='Cache.ACC_btn_search'/></a><!-- 검색 -->
					</div>
				</div>
			</div>
				...
				<div class="inPerView type07">
					<div style="width:900px;">
						<div class="inPerTitbox">
							...
							<div id="writeDate2" class="dateSel type02">
							</div>
						</div>
						<a class="btnTypeDefault  btnSearchBlue" onclick="TaxInvoice.searchList();"><spring:message code='Cache.ACC_btn_search'/></a>
					</div>
				</div>
			</div>
			<div class="inPerTitbox">
				<div class="buttonStyleBoxRight">	
					<span id="listCount" class="selectType02 listCount" onchange="TaxInvoice.searchList();">
					</span>
					<button class="btnRefresh" type="button" onclick="TaxInvoice.searchList();"></button>
				</div>
			</div>
...
(function(window) {
	var TaxInvoice = {
			...
			setPageViewController :function(){
				...
				var syncPropertyTax		= accountCtrl.getInfo("syncPropertyTax").val();
				var syncPropertyCash 	= accountCtrl.getInfo("syncPropertyCash").val();
				
				if(searchProperty == "SAP"){
					//SAP가 추가될 경우 이곳에 정의
				}else if(searchProperty == "SOAP"){
					accountCtrl.getInfo("btnIsOffsetY").css("display",	"none");
					accountCtrl.getInfo("btnIsOffsetN").css("display",	"none");
					accountCtrl.getInfo("btnTossUser").css("display",	"none");
					accountCtrl.getInfo("btnSyncTax").css("display",	"none");
					accountCtrl.getInfo("btnSyncCash").css("display",	"none");
					accountCtrl.getInfo("searchbar2").show();
					accountCtrl.getInfo("searchbar1").hide();
					
				}else{
					accountCtrl.getInfo("btnIsOffsetY").css("display",	"");
					accountCtrl.getInfo("btnIsOffsetN").css("display",	"");
					accountCtrl.getInfo("btnTossUser").css("display",	"");
					accountCtrl.getInfo("btnSyncTax").css("display",	"");
					accountCtrl.getInfo("btnSyncCash").css("display",	"");
			...
			pageDatepicker : function(){
				if(accountCtrl.getInfo("searchbar1").css("display") != "none") {
					...
					makeDatepicker('writeDate2','writeDateE','writeDateE','','','');						
				}
			},
			...
			setSelectCombo : function(pCompanyCode){
				...
				accountCtrl.getInfo("listCount").addClass("selectType02").addClass("listCount").attr("onchange", "TaxInvoice.searchList();");
				...
				var AXSelectMultiArr	= [	
					...
					,	{'codeGroup':'TaxInvoiceActive',	'target':'taxInvoiceActive',	'lang':'ko',	'onchange':'',	'oncomplete':'',	'defaultVal':'',	'useDefault':"<spring:message code='Cache.ACC_lbl_comboAll' />"} //전체
			...
			setHeaderData : function() {
				...
				if(searchProperty == "SOAP"){
					me.params.headerData = [	{	key:'WriteDate',			label:"<spring:message code='Cache.ACC_lbl_writeDate' />",			width:'100',		align:'center'},	//작성일
												{	key:'NTSConfirmNum',		label:"<spring:message code='Cache.ACC_lbl_confirmNum' />",			width:'200',		align:'center'},	//승인번호
												{	key:'InvoicerCorpName',		label:"<spring:message code='Cache.ACC_lbl_invoiceCorpName' />",	width:'200',	align:'left'},		//공급자상호
												...
												{	key:'InvoiceeEmail1',		label:"<spring:message code='Cache.ACC_lbl_invoiceEmail' />",		width:'200',	align:'left'},		//공급받는자Email
												{	key:'Remark1',				label:'REMARK',														width:'100',	align:'left'},				
					...
					me.params.headerData = [	{	key:'chk',					label:'chk',			width:'20', 	align:'center',
												...
												{	key:'NTSConfirmNum',		label:'승인번호',			width:'200',		align:'center',
													formatter:function () {
														var rtStr =	""
																	+	"<a onclick='TaxInvoice.taxInvoicePop("+ this.item.TaxInvoiceID +"); return false;'>"
																	+		"<font color='blue'>"
												...
												{	key:'SupplyCostTotal',		label:"<spring:message code='Cache.ACC_lbl_supplyCost' />",			width:'100',	align:'right'},		//공급액          
												{	key:'TaxTotal',				label:"<spring:message code='Cache.ACC_lbl_taxType' />",			width:'100',	align:'right'},		//부가세          
												{	key:'SupplyCost',		label:"<spring:message code='Cache.ACC_lbl_itemSupplyCost' />",		width:'100',	align:'right'},		//품목 공급액          
												{	key:'Tax',				label:"<spring:message code='Cache.ACC_lbl_itemTaxType' />",		width:'100',	align:'right'},		//품목 세액          
												{	key:'InvoiceeEmail1',		label:"<spring:message code='Cache.ACC_lbl_invoiceEmail' />",		width:'200',	align:'left',
													formatter:function () {
														var invoiceMail = this.item.InvoiceeEmail1;
														if(this.item.InvoiceEmail2) invoiceMail += " / " + this.item.InvoiceEmail2;
														return invoiceMail;
													}
												},		//공급받는자Email   
												{	key:'TaxInvoiceActive',		label:"<spring:message code='Cache.ACC_lbl_processStatus' />",		width:'80',		align:'center'},	//처리상태
												{	key:'Remark1',				label:'REMARK',														width:'100',	align:'left'},		//비고		 
												{	key:'ModifyName',			label:"<spring:message code='Cache.ACC_lbl_modifyName' />",			width:'80',		align:'center', 	//수정/일반  
													formatter:function () {
														var rtStr =	""
			...
			searchList : function(YN){
				...
				var gridPanel		= me.params.gridPanel;
				var gridHeader		= me.params.headerData;
				var ajaxUrl			= "/account/taxInvoice/getTaxInvoiceList.do";
			...
			excelDownload : function(){
				...
				Common.Confirm("<spring:message code='Cache.ACC_msg_excelDownMessage'/>", "<spring:message code='Cache.ACC_btn_save' />", function(result){
		       		if(result){
						...
						var	locationStr		= "/account/taxInvoice/taxInvoiceExcelDownload.do?"
			...
			updateIsOffset : function(key){
				...
				if(key == 'Y'){
					msg = "<spring:message code='Cache.ACC_lbl_askOffsetY' />";
				}else if(key == 'N'){
					msg = "<spring:message code='Cache.ACC_lbl_askOffsetN' />";
				}else if(key == 'A'){
					me.offsetA();
					//msg = "완료처리 하시겠습니까?";
				}
				if(key == 'Y' || key == 'N'){
						...
						Common.Confirm(msg, "<spring:message code='Cache.ACC_lbl_offset' />", function(result){
				       		if(result){
								$.ajax({
			...
			offsetA : function(){
				...
				var chkList = me.params.gridPanel.getCheckedList(0);
				if(chkList.length == 0){
					Common.Inform("<spring:message code='Cache.ACC_msg_noselectdata' />");	//선택된항목이 없습니다.
					return;
				}
				
				var popupName	= "offsetAPopup";
				var popupID		= "offsetAPopup";
				var openerID	= "TaxInvoice";
				var popupTit	= "완료처리 - 전표등록"
				var popupYN		= "N";
				var callBack	= "offsetAPopup_CallBack";
				var url			= "/account/accountCommon/accountCommonPopup.do?"
								+ "popupID="		+ popupID	+ "&"
								+ "openerID="		+ openerID	+ "&"
								+ "popupYN="		+ popupYN	+ "&"
			...
			offsetAPopup_CallBack : function(info){
				var me = this;
				var params	= new Object();
				var chkList	= me.params.gridPanel.getCheckedList(0);
				...
				$.ajax({
					url			: "/account/taxInvoice/saveExpence.do",
					type		: "POST",
					data		: JSON.stringify(params),
					dataType	: "json",
					contentType	: "application/json",
			...
			cashBillSync : function(){
				$.ajax({
					url	: "/account/taxInvoice/cashBillSync.do",
					type: "POST",
					data: {
			...
			taxInvoiceSync : function(){
				$.ajax({
					url	: "/account/taxInvoice/taxInvoiceSync.do",
					type: "POST",
					data: {
						CompanyCode : accountCtrl.getComboInfo("companyCode").val()
					},
			...
			sendDelay: function() {//미상신 알림메일
				...
				Common.Confirm("<spring:message code='Cache.lbl_AccSendNotiTaxAlert' /><br>"+	//법인카드,세금계산서의 미상신건들에 대한 알림 메일을 전송합니다.
						"<spring:message code='Cache.msg_apv_ConfirmSend' /> ", "Confirmation Dialog", function (confirmResult) {
				if (confirmResult) {
					$.ajax({
						url			: "/account/doAutoDelayAlam.do",
						type		: "POST",
						data		:{"DirectSend"		: "Y", "SendType": "Tax", companyCode: accountCtrl.getComboInfo("companyCode").val()},
						success:function (data) {
							if(data.status == "SUCCESS"){
