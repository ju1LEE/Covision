...
<div class="cRConTop titType">
	...
	<div class="selBox" style="width: 184px;" id="bizDocSelectBox"></div>
...
<script>
	...
	var fnObj1 = {
		pageStart: function(){
			...
			if (len > 0) {
				if (schFrmSeBizID == "") {
					html = "<span class=\"selTit\" ><a id=\"seBizID\" onclick=\"clickBizSelectBox(this);\" value=\""+selBizList[0].code+"\" class=\"up\">"+CFN_GetDicInfo(selBizList[0].name)+"</a></span>"
				} else {
					html = "<span class=\"selTit\" ><a id=\"seBizID\" onclick=\"clickBizSelectBox(this);\" value=\""+schFrmSeBizID+"\" class=\"up\">"+CFN_GetDicInfo(schFrmSeBizNm)+"</a></span>"
				}
				html += "<div class=\"selList\" style=\"width:200px;display: none;\">";
				$(selBizList).each(function(i) {
					if (schFrmSeBizID == selBizList[i].code) {
						html += "<a class=\"listTxt select\" value=\""+selBizList[i].code+"\" onclick=\"clickBizSelectBoxList(this);\" id=\""+selBizList[i].code+"\">"+CFN_GetDicInfo(selBizList[i].name)+"</a>"
					} else {
						html += "<a class=\"listTxt\" value=\""+selBizList[i].code+"\" onclick=\"clickBizSelectBoxList(this);\" id=\""+selBizList[i].code+"\">"+CFN_GetDicInfo(selBizList[i].name)+"</a>"
					}
				});
				html += "</div>";	
			}
		 ...
		 var headerData1 =[
		                  ...
		                  {key:'FormSubject', label:'<spring:message code="Cache.lbl_apv_subject"/>', width:'200', align:'left',						// 제목
						   	  formatter:function () {
						   			...
						   			/* if(CFN_GetCookie("ListViewCookie") == "Y"){
						   				html +="<a href='#' onclick='onClickIframeButton(\""+ this.item.ProcessID +"\"); return false;'>"+this.item.FormSubject+"</a>";	
						   			}else{ */
		                  ...
		                  {key:'BusinessData3', label:'<spring:message code="Cache.ACC_billReqAmt"/>', width:'45', align:'right', formatter:"money"}, //금액
		                  {key:'BusinessData4', label:'<spring:message code="Cache.ACC_useHistory"/>', width:'100', align:'center'}, //적요 
...
/*					$('.bodyNode.bodyTdText').css('overflow','visible');*/
	...
	function onClickPopButton(ProcessID,FormPrefix,FormID,BusinessData1,BusinessData2){
		...
		CFN_OpenWindow("/approval/approval_Form.do?mode="+mode+"&processID="+ProcessID
				+"&workitemID=&performerID=&userCode=&gloct=BizDoc&admintype=&archived="
				+archived+"&usisdocmanager=true&subkind=T006"
				+"&ExpAppID="+((typeof BusinessData2!="undefined"&&BusinessData2!="undefined")?BusinessData2:"")
				, "", width, (window.screen.height - 100), "both");
	}
