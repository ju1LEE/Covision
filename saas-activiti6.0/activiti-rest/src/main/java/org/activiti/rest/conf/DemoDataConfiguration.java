 ...
 */
@Service
public class DemoDataConfiguration /*implements ActivitiEventListener*/ implements ApplicationListener<ApplicationEvent> {

	protected static final Logger LOGGER = LoggerFactory.getLogger(DemoDataConfiguration.class);

	protected static DemoDataConfiguration object = null;
	...
	public void init() {
		...
		object = this;
		identityService = org.activiti.engine.ProcessEngines.getDefaultProcessEngine().getIdentityService();
		taskService = org.activiti.engine.ProcessEngines.getDefaultProcessEngine().getTaskService();
		repositoryService = org.activiti.engine.ProcessEngines.getDefaultProcessEngine().getRepositoryService();
	...
	protected void createUser(String userId, String firstName, String lastName, String password, 
		...
		if (imageResource != null) {
			InputStream is = null;
			try {
				is = this.getClass().getClassLoader().getResourceAsStream(imageResource);
				byte[] pictureBytes = IoUtil.readInputStream(is, null);
	...
	protected void createModelData(String name, String description, String jsonFile) {
		...
		if (modelList == null || modelList.isEmpty()) {
			...
			try (InputStream svgStream = this.getClass().getClassLoader().getResourceAsStream("org/activiti/rest/demo/model/test.svg"))
			{
			...
			try (InputStream editorJsonStream = this.getClass().getClassLoader().getResourceAsStream(jsonFile))
			{
