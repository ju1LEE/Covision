/**
 * @Class Name : RequestBypassedExHandler.java
 * @Description : 
...
public class RequestBypassedExHandler implements TaskListener {

	private static final Logger LOG = LoggerFactory.getLogger(RequestBypassedExHandler.class);
	
	@Override
	public void notify(DelegateTask delegateTask) throws Exception{
		
		LOG.info("RequestBypassedEx");
		...
		try{
			...
			apvLineObj = CoviFlowWorkHelper.setActivity("bypass", delegateTask, ctx, "T006");
			...
			CoviFlowWorkHelper.processByPassedEx();
	
		}catch(Exception e){
			CoviFlowLogHelper.insertErrorLog(delegateTask, e, "ERROR");
			LOG.error("RequestBypassedExHandler", e);
			throw e;
		}
