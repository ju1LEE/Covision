/**
 * @Class Name : DraftEndExcutionListener.java
 * @Description : 
...
public class DraftEndExcutionListener implements ExecutionListener {
	
	private static final Logger LOG = LoggerFactory.getLogger(DraftEndExcutionListener.class);
	...
	public void notify(DelegateExecution execution) throws Exception {
		...
		try{
			...
			//알림 대상 - 결재단계의 모든 사람?
			JSONArray divisions = new JSONArray();
			divisions = CoviFlowApprovalLineHelper.getDivisions(apvLineObj);
			String usid = CoviFlowApprovalLineHelper.getInitiatorID(apvLineObj);
			for(int i = 0; i < divisions.size(); i++)
			{
				JSONObject division = (JSONObject)divisions.get(i);
				String processId = (String)division.get("processID");
				ArrayList<HashMap<String,String>> receivers = CoviFlowApprovalLineHelper.getDivisionMessageReceivers(division);
				if(receivers.size() > 0){
					CoviFlowWSHelper.invokeApi("MESSAGE", CoviFlowWorkHelper.makeMessageParams(fiid, formName, usid, formSubject, "COMPLETE", processId, receivers, apvLineObj));
				}	
			}
