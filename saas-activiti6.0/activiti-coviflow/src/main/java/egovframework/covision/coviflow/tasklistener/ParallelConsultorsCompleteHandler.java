...
public class ParallelConsultorsCompleteHandler implements TaskListener {

	private static final Logger LOG = LoggerFactory.getLogger(ParallelConsultorsCompleteHandler.class);
	
	@Override
	public void notify(DelegateTask delegateTask) throws Exception{
		
		LOG.info("ParallelConsultorsComplete");
		
		//CoviFlowWorkHelper.processResult();		
		...
		try{
			...
			for(int i = 0; i < ous.size(); i++)
			{
				...
				if(personTaskInfo.get("kind").toString().equalsIgnoreCase("substitute")){
					deputyCode = (String)((JSONObject)persons.get(0)).get("code");
				}
				...
				if(pendingOu.containsKey("taskid")){
					...
					if(taskid.equalsIgnoreCase(delegateTask.getId().toString())){
						...
						if(routeType.equalsIgnoreCase("consult")){//합의
							if(actionMode.equalsIgnoreCase("APPROVECANCEL")){//승인취소
							...
							} else if(actionMode.equalsIgnoreCase("AGREE")){
								//추가적으로 분석이 필요
								...
								if(isSubstituted){
									...
									apvLineObj = CoviFlowApprovalLineHelper.setOuTask(apvLineObj, activeDivisionIndex, pendingStepIndex, 0, taskid, "result", "agreed");
									...
									apvLineObj = CoviFlowApprovalLineHelper.setOuTask(apvLineObj, activeDivisionIndex, pendingStepIndex, taskid, CoviFlowVariables.APPV_AGREE);
								}
								
								if(delegateTask.getProcessDefinitionId().toString().contains("request")){
									
									if(delegateTask.hasVariable("g_request_approvalResult") && delegateTask.getVariable("g_request_approvalResult") != null){
										if(delegateTask.getVariable("g_request_approvalResult").toString().equalsIgnoreCase("agreed")
												||delegateTask.getVariable("g_request_approvalResult").toString().isEmpty()){
											delegateTask.setVariable("g_request_approvalResult", "agreed");	
										} else {
											delegateTask.setVariable("g_request_approvalResult", "disagreed");
										}
									} else {
										delegateTask.setVariable("g_request_approvalResult", "agreed");	
									}
									...
									if(delegateTask.hasVariable("g_basic_approvalResult") && delegateTask.getVariable("g_request_approvalResult") != null){
										if(delegateTask.getVariable("g_basic_approvalResult").toString().equalsIgnoreCase("agreed")
												||delegateTask.getVariable("g_basic_approvalResult").toString().isEmpty()){
											delegateTask.setVariable("g_basic_approvalResult", "agreed");	
										} else {
											delegateTask.setVariable("g_basic_approvalResult", "disagreed");
										}
									} else {
										delegateTask.setVariable("g_basic_approvalResult", "agreed");	
									}
							...
							} else if(actionMode.equalsIgnoreCase("DISAGREE")){
								//추가적으로 분석이 필요
								CoviFlowWorkHelper.processResult(apvLineObj, taskid, isMobile);
								
								if(isSubstituted){
									apvLineObj = CoviFlowApprovalLineHelper.setOuTask(apvLineObj, activeDivisionIndex, pendingStepIndex, 0, taskid, "status", "rejected");
									apvLineObj = CoviFlowApprovalLineHelper.setOuTask(apvLineObj, activeDivisionIndex, pendingStepIndex, 0, taskid, "result", "disagreed");
									apvLineObj = CoviFlowApprovalLineHelper.setOuTask(apvLineObj, activeDivisionIndex, pendingStepIndex, 0, taskid, "datecompleted", sdf.format(new Date()));
									//comment
									if(hasOldComment || StringUtils.isNotBlank(actionComment)){
										apvLineObj = CoviFlowApprovalLineHelper.setOuTask(apvLineObj, activeDivisionIndex, pendingStepIndex, 0, taskid, "comment", comment);
									...
									apvLineObj = CoviFlowApprovalLineHelper.setOuTask(apvLineObj, activeDivisionIndex, pendingStepIndex, 1, taskid, "status", "rejected");
									apvLineObj = CoviFlowApprovalLineHelper.setOuTask(apvLineObj, activeDivisionIndex, pendingStepIndex, 1, taskid, "result", "bypassed");
									apvLineObj = CoviFlowApprovalLineHelper.setOuTask(apvLineObj, activeDivisionIndex, pendingStepIndex, 1, taskid, "datecompleted", sdf.format(new Date()));
								}else{
									Map<String, Object> params = new HashMap<String, Object>();
									...
									apvLineObj = CoviFlowApprovalLineHelper.setOuTask(apvLineObj, activeDivisionIndex, pendingStepIndex, taskid, CoviFlowVariables.APPV_DISAGREE);	
								}
							...
							if(actionMode.equalsIgnoreCase("APPROVECANCEL")){//승인취소
								...
								if(isSubstituted){
									apvLineObj = CoviFlowApprovalLineHelper.setOuTask(apvLineObj, activeDivisionIndex, pendingStepIndex, 0, taskid, "status", "completed");
									apvLineObj = CoviFlowApprovalLineHelper.setOuTask(apvLineObj, activeDivisionIndex, pendingStepIndex, 0, taskid, "result", "completed");
									apvLineObj = CoviFlowApprovalLineHelper.setOuTask(apvLineObj, activeDivisionIndex, pendingStepIndex, 0, taskid, "datecompleted", sdf.format(new Date()));
									//comment
									if(hasOldComment || StringUtils.isNotBlank(actionComment)){
										apvLineObj = CoviFlowApprovalLineHelper.setOuTask(apvLineObj, activeDivisionIndex, pendingStepIndex, 0, taskid, "comment", comment);
								...
								if(isSubstituted){
									apvLineObj = CoviFlowApprovalLineHelper.setOuTask(apvLineObj, activeDivisionIndex, pendingStepIndex, 0, taskid, "status", "rejected");
									apvLineObj = CoviFlowApprovalLineHelper.setOuTask(apvLineObj, activeDivisionIndex, pendingStepIndex, 0, taskid, "result", "rejected");
									apvLineObj = CoviFlowApprovalLineHelper.setOuTask(apvLineObj, activeDivisionIndex, pendingStepIndex, 0, taskid, "datecompleted", sdf.format(new Date()));
									//comment
									if(hasOldComment || StringUtils.isNotBlank(actionComment)){
										apvLineObj = CoviFlowApprovalLineHelper.setOuTask(apvLineObj, activeDivisionIndex, pendingStepIndex, 0, taskid, "comment", comment);
						...
						if(StringUtils.isNotBlank(actionComment) &&
							...
							if(receivers.size() > 0){
								CoviFlowWSHelper.invokeApi("MESSAGE", CoviFlowWorkHelper.makeMessageParams(fiid, formName, usid, actionUserCode, formSubject, "COMMENT", Integer.toString(piid), receivers, apvLineObj, CoviFlowWorkHelper.base64Decode(actionComment)));
							}
						}
						
						if(actionMode.equalsIgnoreCase("AGREE") || actionMode.equalsIgnoreCase("DISAGREE")){
							//결재선 변경
						...
						if(!actionMode.equalsIgnoreCase("")){
							if(isLegacy){
								String personid = "";
			...
			LOG.error("ParallelConsultorsCompleteHandler", e);
