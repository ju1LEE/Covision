/**
 * @Class Name : DraftStartExcutionListener.java
 * @Description : 
...
public class DraftStartExcutionListener implements ExecutionListener {
	
	private static final Logger LOG = LoggerFactory.getLogger(DraftStartExcutionListener.class);
	
	@SuppressWarnings("unchecked")
	public void notify(DelegateExecution execution) throws Exception {
		
		LOG.info("draft Process Start.");
		/*
		 * draft 프로세스 시작 시 처리가 들어 올 곳
		 * */
		...
		try{
			...
			if(execution.hasVariable("g_context"))
			{
				...
				//파라미터 변수 처리
				ctxObj = parser.parse(new FileReader("D:/temp/appv_json/parameters3.json"));
			...
			global.put("g_draft_hasStepsFinished", "false");
			global.put("g_currentRouteType", "");
			global.put("g_currentUnitType", "");
			global.put("g_draft_approvalResult", "");
			global.put("g_draft_approver", "");
			//global.put("g_jfid", jfid);									//DRAFT 에서 비사용. 우선 주석 처리
			global.put("g_subject", formSubject);
			global.put("g_isLegacy", isLegacy);
			global.put("g_docNumber", "");
			//추가적인 분석 필요.
			//global.put("g_hasReceipt", "false");
			...
			LOG.error("DraftStartExcutionListener", e);
