/**
 * @Class Name : BasicLoopSteps.java
 * @Description : 
...
public class BasicLoopSteps implements JavaDelegate {

	private static final Logger LOG = LoggerFactory.getLogger(BasicLoopSteps.class);
			
	public void execute(DelegateExecution execution) throws Exception {
		
		LOG.info("BasicLoopSteps");
		
		try{
			String hasStepsFinished = "false";
			if(execution.hasVariable("g_basic_hasStepsFinished"))
			{
				hasStepsFinished = (String)execution.getVariable("g_basic_hasStepsFinished");
			}
			...
			if(approvalResult != null && (approvalResult.equalsIgnoreCase("rejected")||approvalResult.equalsIgnoreCase("cancelled")||approvalResult.equalsIgnoreCase("rejectedtodept"))){
			...
			//set variable
			execution.setVariable("g_basic_hasStepsFinished", hasStepsFinished);
		} catch(Exception e){
			CoviFlowLogHelper.insertErrorLog(execution, e, "ERROR");
			LOG.error("BasicLoopSteps", e);
			throw e;
		}
	}
}
