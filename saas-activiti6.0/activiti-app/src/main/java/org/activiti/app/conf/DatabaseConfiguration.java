...
public class DatabaseConfiguration {
	static {
		// [covision] for liquibase implements . 
		List<String> liquibasePackages = new ArrayList<String>();
		
        liquibasePackages.add("liquibase.change");
        liquibasePackages.add("liquibase.changelog");
        liquibasePackages.add("liquibase.database");
        liquibasePackages.add("liquibase.parser");
        liquibasePackages.add("liquibase.precondition");
        liquibasePackages.add("liquibase.datatype");
        liquibasePackages.add("liquibase.serializer");
        liquibasePackages.add("liquibase.sqlgenerator");
        liquibasePackages.add("liquibase.executor");
        liquibasePackages.add("liquibase.snapshot");
        liquibasePackages.add("liquibase.logging");
        liquibasePackages.add("liquibase.diff");
        liquibasePackages.add("liquibase.structure");
        liquibasePackages.add("liquibase.structurecompare");
        liquibasePackages.add("liquibase.lockservice");
        liquibasePackages.add("liquibase.sdk.database");
        liquibasePackages.add("liquibase.ext");
        
		liquibasePackages.add("covision.liquibase.database");
		
		System.setProperty("liquibase.scan.packages", StringUtils.join(liquibasePackages, ","));
	}
	...
	public DataSource dataSource() {
		...
		if (StringUtils.isNotEmpty(dataSourceJndiName)) {
			...
			String dataSourcePassword = CoviFlowPropHelper.getInstace().getDecryptedProperty( env.getProperty("datasource.password", "") );
	...
	public Liquibase liquibase() {
		...
		try {
			...
			Liquibase liquibase = new Liquibase("META-INF/liquibase/app-db-changelog.xml", new ClassLoaderResourceAccessor(), database);
			
			String dbSchemaUpdate = env.getProperty("app.db.schema.update", ProcessEngineConfiguration.DB_SCHEMA_UPDATE_FALSE);
			if("true".equals(dbSchemaUpdate)) {
				liquibase.update(new Contexts("activiti"), new LabelExpression(), false);
			}
			return liquibase;
