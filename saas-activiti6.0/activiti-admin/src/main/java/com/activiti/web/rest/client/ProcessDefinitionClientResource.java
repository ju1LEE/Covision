...
public class ProcessDefinitionClientResource extends AbstractClientResource {
	 ...
	 */
	@RequestMapping(value = "/rest/activiti/process-definitions/export/{definitionId}/{fileName}", method = RequestMethod.GET, produces = "application/json")
	public JsonNode exportProcessDefinition(@PathVariable String definitionId, @PathVariable String fileName, HttpServletResponse response) throws BadRequestException {
		ServerConfig serverConfig = retrieveServerConfig();
		try {
			return clientService.exportProcessDefinitionModelXml(serverConfig, definitionId, fileName, response);
		} catch (ActivitiServiceException e) {
			throw new BadRequestException(e.getMessage());
		}
	}
