...
public class ProcessDefinitionService {
	...
	/**
	 * covision. export process-definition. 22.1.27. hgsong 
	 * @param serverConfig
	 * @param definitionId
	 * @param response
	 * @return
	 */
	public JsonNode exportProcessDefinitionModelXml(ServerConfig serverConfig, String definitionId, String fileName, HttpServletResponse response) {
		HttpGet request = null;
		try {
			request = new HttpGet(clientUtil.getServerUrl(serverConfig, "repository/process-definitions/" + definitionId + "/resourcedata"));
			
			String xmlString = clientUtil.executeRequestAsString(request, serverConfig, 200);
			response.setHeader("Content-Disposition", "attachment; filename=\"" + fileName + "\"");
			
			response.getOutputStream().write(xmlString.getBytes("UTF-8"));
		} catch (Exception e) {
			if(request != null) {
				System.err.println("Requested resourcedata rest URL is " + request.getURI());
			}
			throw new ActivitiServiceException("Exception when requeste resourcedata rest", e);
		}
		return null;
	}
}
