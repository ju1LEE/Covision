...
//import com.activiti.security.CustomUserDetailService;
import com.activiti.security.UserDetailsService;
import com.activiti.service.UserService;
import com.activiti.service.activiti.persistent.PersistentTokenService;
...
public class CustomPersistentRememberMeServices extends AbstractRememberMeServices implements CustomRememberMeService {
	...
	public static final String COOKIE_NAME = "ACTIVITIADM_REMEMBER_ME";
	...
	protected void onLoginSuccess(HttpServletRequest request, HttpServletResponse response, Authentication successfulAuthentication) {
		...
		org.springframework.security.core.userdetails.User user = (org.springframework.security.core.userdetails.User) successfulAuthentication.getPrincipal();
		PersistentToken token = createAndInsertPersistentToken(user, request.getRemoteAddr(), request.getHeader("User-Agent"));
	...
	protected UserDetails processAutoLoginCookie(String[] cookieTokens, HttpServletRequest request, HttpServletResponse response) {
		...
		if (new Date().getTime() - token.getTokenDate().getTime() > tokenRefreshDurationInMilliseconds) {
			...
			try {
				...
				com.activiti.domain.User user = userService.getUser(token.getUser());
				
				token = persistentTokenService.createToken(user.getLogin(), request.getRemoteAddr(), request.getHeader("User-Agent"));
				addCookie(token, request, response);
...
//		return customUserDetailService.loadByUserId(token.getUser());
		return getUserDetailsService().loadUserByUsername(token.getUser());
	}
	...
	public PersistentToken createAndInsertPersistentToken(User user, String remoteAddress, String userAgent) {
		return persistentTokenService.createToken(user.getUsername(), remoteAddress, userAgent);
	}
}
